...
1. WHEN Người_dùng truy cập mục Thu chi từ thanh công cụ (Quản lý & Vận hành → Tài chính → Thu chi), THE Hệ_thống SHALL hiển thị Màn_thu_chi với breadcrumb "Tài chính > Thu chi"
2. THE Hệ_thống SHALL hiển thị phần thống kê phía trên danh sách với các thẻ: Tổng thu (tổng total_amount của tất cả Phiếu_thu khớp bộ lọc hiện tại), Tổng chi (tổng total_amount của tất cả Phiếu_chi khớp bộ lọc hiện tại), Chênh lệch (Tổng thu - Tổng chi), Tổng số phiếu
3. THE Hệ_thống SHALL hiển thị thanh công cụ với các nút: Thêm phiếu (+), Nhập dữ liệu (icon mũi tên lên), Tìm kiếm (search icon)
...
1. WHEN Người_dùng ấn nút dấu (+) tại Màn_thu_chi, THE Hệ_thống SHALL hiển thị dialog/form Thêm phiếu thu/chi với hai lựa chọn loại phiếu: Phiếu thu và Phiếu chi
2. WHEN Người_dùng chọn ô Phiếu thu, THE Hệ_thống SHALL hiển thị form với các trường: Căn hộ (*), Phòng, Giường, Khách hàng, Tên phiếu thu (*), Ngày thu (*), Ghi chú
...
6. WHEN Người_dùng ấn nút dấu (+) tại phần Hạng mục, THE Hệ_thống SHALL hiển thị dialog chọn Hạng_mục với danh sách Loại_thu_chi có sẵn (checkbox cho mỗi loại) và nút Thêm để tạo Loại_thu_chi mới
7. WHEN Người_dùng tích chọn một hoặc nhiều Hạng_mục và xác nhận, THE Hệ_thống SHALL hiển thị thông tin các Hạng_mục đã chọn ra màn hình Chi tiết Thu/chi với các trường cho mỗi hạng mục: Tên loại (readonly), Mô tả, Số lượng (mặc định 1), Đơn giá, Thành tiền (tự tính = Số lượng × Đơn giá)
8. THE Hệ_thống SHALL cho phép thêm nhiều Hạng_mục cho một Phiếu_thu, mỗi Hạng_mục có thông tin chi tiết riêng
9. WHEN Người_dùng điền đầy đủ thông tin bắt buộc và ấn nút Lưu, THE Hệ_thống SHALL tạo Phiếu_thu mới với Trạng_thái_duyệt là UNAPPROVED, database trigger tự động sinh Mã_phiếu định dạng PT{YYMM}{sequence}, và hiển thị thông báo "Dữ liệu đã được TẠO thành công"
10. IF Người_dùng không điền đầy đủ các trường bắt buộc, THEN THE Hệ_thống SHALL hiển thị thông báo lỗi validation tương ứng cho từng trường bị thiếu
11. IF Người_dùng không thêm ít nhất 1 Hạng_mục, THEN THE Hệ_thống SHALL hiển thị thông báo lỗi "Vui lòng thêm ít nhất 1 hạng mục"
...
1. WHEN Người_dùng chọn ô Phiếu chi tại dialog Thêm phiếu thu/chi, THE Hệ_thống SHALL hiển thị form với các trường: Căn hộ (*), Phòng, Giường, Khách hàng, Tên phiếu chi (*), Ngày chi (*), Ghi chú
2. THE Hệ_thống SHALL đánh dấu các trường bắt buộc bằng ký hiệu (*) gồm: Căn hộ, Tên phiếu chi, Ngày chi
3. THE Hệ_thống SHALL cho phép thêm nhiều Hạng_mục cho một Phiếu_chi, mỗi Hạng_mục có thông tin chi tiết riêng (Tên loại, Mô tả, Số lượng, Đơn giá, Thành tiền)
...
1. WHEN Người_dùng ấn nút Cập nhật tại cột Thao tác của một Phiếu_thu_chi có Trạng_thái_duyệt là UNAPPROVED, THE Hệ_thống SHALL hiển thị form chỉnh sửa với đầy đủ thông tin hiện tại bao gồm loại phiếu, Căn hộ, Phòng, Giường, Khách hàng, Tên phiếu, Ngày, Ghi chú, và danh sách Hạng_mục
...
6. IF Người_dùng muốn sửa phiếu đã duyệt, THEN THE Hệ_thống SHALL yêu cầu bỏ duyệt trước khi cho phép chỉnh sửa
...
1. WHEN Người_dùng ấn nút Xoá tại cột Thao tác của một Phiếu_thu_chi có Trạng_thái_duyệt là UNAPPROVED, THE Hệ_thống SHALL hiển thị hộp thoại xác nhận xoá
...
4. IF Người_dùng muốn xoá phiếu đã duyệt, THEN THE Hệ_thống SHALL yêu cầu bỏ duyệt trước khi cho phép xoá
...
1. WHEN Người_dùng click vào nút Lọc dữ liệu (icon 3 gạch ngang), THE Hệ_thống SHALL hiển thị thanh bộ lọc với các tiêu chí: Căn hộ (dropdown danh sách Căn_hộ), Phòng (dropdown lọc theo Căn hộ đã chọn), Sổ quỹ (dropdown), Loại phiếu (dropdown: Tất cả, Phiếu thu, Phiếu chi), Thời gian (date range picker: Từ ngày - Đến ngày), Trạng thái duyệt (dropdown: Tất cả, Đã duyệt, Chưa duyệt)
...
**User Story:** Là một Người_dùng, tôi muốn xem tổng số tiền đã thu và đã chi tại Căn hộ, để nắm bắt tình hình tài chính tổng quan.
...
### Yêu cầu 9: Nhập hàng loạt Phiếu thu/chi từ file Excel

**User Story:** Là một Người_dùng, tôi muốn nhập hàng loạt phiếu thu/chi từ file mẫu Excel, để tạo dữ liệu nhanh khi có số lượng phiếu lớn.

#### Tiêu chí chấp nhận

1. WHEN Người_dùng ấn nút Nhập dữ liệu (icon mũi tên lên) tại Màn_thu_chi, THE Hệ_thống SHALL hiển thị dialog nhập dữ liệu
2. THE Hệ_thống SHALL hiển thị link "Tải file mẫu tại đây" trong dialog nhập dữ liệu
3. WHEN Người_dùng click "Tải file mẫu tại đây", THE Hệ_thống SHALL tải xuống file Excel mẫu với các cột: Loại phiếu (*), Căn hộ (*), Phòng, Tên phiếu (*), Ngày (*), Hạng mục (*), Số tiền (*)
4. THE Hệ_thống SHALL hỗ trợ upload file bằng cách ấn nút "Chọn file" hoặc kéo thả file vào vùng upload, chấp nhận file .xlsx và .xls
...
1. WHEN Người_dùng ấn nút Thêm trong dialog chọn Hạng_mục khi lập phiếu, THE Hệ_thống SHALL hiển thị form tạo Loại_thu_chi mới với các trường: Tên loại (*), Loại (thu/chi), Mô tả
...
3. THE Hệ_thống SHALL hiển thị danh sách Loại_thu_chi trong dialog chọn Hạng_mục dưới dạng checkbox, cho phép tích chọn nhiều loại cùng lúc
