...
export function jsonResponse(body: unknown, status: number, extra: Record<string, string> = {}) {
  return jsonTextResponse(JSON.stringify(body), status, extra);
}

/** Return already-canonical JSON without parsing/stringifying it again. */
export function jsonTextResponse(body: string, status: number, extra: Record<string, string> = {}) {
...
export function emptyResponse(status: number, extra: Record<string, string> = {}) {
