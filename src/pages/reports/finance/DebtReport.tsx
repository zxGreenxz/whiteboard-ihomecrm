...
export default function DebtReport() {
  ...
  const stats = (
    <>
      <ReportCard
        ...
        icon={DollarSign}
        description="Tất cả hóa đơn chưa thanh toán"
      />
      <ReportCard
        title="Số hóa đơn nợ"
      ...
      <ReportCard
        ...
        value={criticalDebts}
        icon={Clock}
        description="Cần ưu tiên thu"
      />
      <ReportCard
        title="Trung bình/HĐ"
        value={formatCurrency(debts && debts.length > 0 ? totalDebt / debts.length : 0)}
        icon={TrendingDown}
  ...
  const exportData = debts?.map(debt => ({
    ...
    "Phòng": `${debt.rooms?.buildings?.name} - ${debt.rooms?.room_number}`,
                  ...
                  <TableBody>
                    {debts.map((debt) => (
                      <TableRow key={debt.id}>
                        <TableCell>
                          <Badge variant={
                            debt.aging_category === ">90" ? "destructive" :
                            debt.aging_category === "61-90" ? "destructive" :
