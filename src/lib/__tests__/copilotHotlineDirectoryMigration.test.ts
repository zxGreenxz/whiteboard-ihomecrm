...
const migration = boCommentSql(
  docSql('supabase/migrations/20260908135519_copilot_hotline_directory_v1.sql'),
);
...
describe('copilot hotline directory migration', () => {
  const body = () => functionBody(migration, 'copilot_hotline_directory_v1');
  ...
  it('is authenticated, stable, and denied by the dedicated hotline rollout by default', () => {
    ...
    expect(body()).toMatch(/copilot_page_flag_allows_v1\('copilot\.hotlines\.directory', p_organization_id\)/);
    expect(migration).toMatch(/'page',\s*'copilot\.hotlines\.directory',\s*'disabled'/);
  });

  it('requires hotline.view in the selected organization and cannot read another organization', () => {
    expect(body()).toMatch(/authorized_scope_v3\('hotline\.view', p_organization_id\)/);
    expect(body()).toMatch(/COALESCE\(v_org_wide, false\)/);
    expect(body()).toMatch(/h\.organization_id = p_organization_id/);
  });

  it('returns only a bounded public hotline directory, excluding descriptions and ownership fields', () => {
    ...
    expect(body()).toMatch(/'so_dien_thoai'/);
  ...
  it('grants the ABI only to authenticated callers and proves catalog properties', () => {
    expect(migration).toMatch(/REVOKE ALL ON FUNCTION public\.copilot_hotline_directory_v1\([^)]*\) FROM PUBLIC, anon, authenticated, service_role;/);
    expect(migration).toMatch(/GRANT EXECUTE ON FUNCTION public\.copilot_hotline_directory_v1\([^)]*\) TO authenticated;/);
