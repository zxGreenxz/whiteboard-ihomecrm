...
function OTien({
  ...
  if (gt === null) {
    return <td className="border-b border-[#f2f4f6] px-3 py-[var(--rp)]" />;
  }
  ...
  return (
    <td
      className={`border-b border-[#f2f4f6] px-3 text-right tabular-nums ${
        dongCon
          ? 'pb-2 pt-1.5 text-[length:var(--fs-sm)]'
          : 'py-[var(--rp)] text-[length:var(--fs)]'
      } ${className}`}
    >
...
function DongChinh({ d, onMo }: { d: DongTien; onMo: (id: string) => void }) {
  return (
    <tr className={d.daHuy ? 'text-gray-400' : undefined}>
      <td className="border-b border-[#f2f4f6] px-[var(--px)] py-[var(--rp)] text-[length:var(--fs)] font-medium">
        <span className={d.daHuy ? 'line-through' : undefined}>{d.ten}</span>
        {d.daHuy && (
          <span className="ml-2 rounded border border-gray-200 bg-gray-50 px-2 py-0.5 text-[11px] font-bold tracking-[.03em] text-gray-500">
            ĐÃ HUỶ
          </span>
        )}
      </td>
      <td className="border-b border-[#f2f4f6] px-3 py-[var(--rp)] text-[length:var(--fs-sm)] tabular-nums text-[#67737E]">
        {d.ky}
      </td>
      ...
      />
      <td className="border-b border-[#f2f4f6] px-2 py-[var(--rp)] text-center">
        {d.moHoaDonId && (
          <NutTron size={28} title="Xem chi tiết hoá đơn" onClick={() => onMo(d.moHoaDonId!)}>
...
function DongCon({ d }: { d: DongTien }) {
  return (
    <tr>
      <td className="border-b border-[#f2f4f6] py-1.5 pb-2 pl-[34px] pr-[var(--px)] text-[length:var(--fs-sm)] text-[#67737E]">
        {d.ma && <span className="font-mono text-[#4a5a52]">{d.ma}</span>}
        {d.ma && d.ten ? ' · ' : ''}
        {d.ten}
        {d.ghiChu && <span className="text-[#9ca3af]"> · {d.ghiChu}</span>}
      </td>
      <td className="border-b border-[#f2f4f6] px-3 py-1.5 pb-2 text-[length:var(--fs-sm)] tabular-nums text-[#67737E]">
        {d.ky}
      </td>
  ...
  return (
    <The id="s-taichinh" className="scroll-mt-[150px]">
      ...
      <DauThe icon={DollarSign} nhan="Tài chính">
        {tong.noHoaDon > 0 && (
          <span className="rounded-[5px] border border-red-200 bg-red-50 px-2.5 py-1 text-[13px] font-semibold tabular-nums text-[#dc2626]">
            Nợ hoá đơn {formatAmount(tong.noHoaDon)}
          </span>
        )}
        {tong.thieuCoc > 0 && (
          <span className="rounded-[5px] border border-orange-200 bg-orange-50 px-2.5 py-1 text-[13px] font-semibold tabular-nums text-orange-700">
            Thiếu cọc {formatAmount(tong.thieuCoc)}
          </span>
        )}
        {tong.chuaHoanKhach > 0 && (
          <span className="rounded-[5px] border border-[#f6e3e6] bg-[#fdf3f4] px-2.5 py-1 text-[13px] font-semibold tabular-nums text-[#9f1239]">
            Chưa hoàn khách {formatAmount(tong.chuaHoanKhach)}
          </span>
        )}
        {tong.noHoaDon === 0 && tong.thieuCoc === 0 && tong.chuaHoanKhach === 0 && (
          <span className="rounded-[5px] border border-[#cfe7db] bg-[#eef7f2] px-2.5 py-1 text-[13px] font-semibold text-[#12764a]">
        ...
        <table className="w-full min-w-[700px] border-collapse">
          <thead>
            <tr className="bg-[#fafbfc]">
              <th className="border-b border-[#e2e5ea] px-[var(--px)] py-2.5 text-left text-[length:var(--fs-xs)] font-bold uppercase tracking-[.06em] text-[#67737E]">
                Khoản mục
              </th>
              <th className="w-[130px] border-b border-[#e2e5ea] px-3 py-2.5 text-left text-[length:var(--fs-xs)] font-bold uppercase tracking-[.06em] text-[#67737E]">
                Kỳ / ngày
              </th>
              <th className="w-[112px] border-b border-[#e2e5ea] px-3 py-2.5 text-right text-[length:var(--fs-xs)] font-bold uppercase tracking-[.06em] text-[#67737E]">
                Số tiền
              </th>
              <th className="w-[112px] border-b border-[#e2e5ea] px-3 py-2.5 text-right text-[length:var(--fs-xs)] font-bold uppercase tracking-[.06em] text-[#67737E]">
                Đã thu
              ...
              <th className="w-[44px] border-b border-[#e2e5ea]" />
            </tr>
          </thead>
          <tbody>
            {nhom.map((g) => (
              <Fragment key={g.khoa}>
                <tr>
                  <td
                    colSpan={6}
                    className={`px-[var(--px)] py-2 text-[length:var(--fs-xs)] font-bold uppercase tracking-[.08em] ${
                      g.toDo
                ...
                {g.canhBao && (
                  <tr>
                    <td
                      colSpan={6}
                      className="border-b border-[#f2f4f6] bg-[#fffbfb] px-[var(--px)] py-2.5 text-[length:var(--fs-sm)] text-[#9f1239]"
                    >
                      {g.canhBao}
                    </td>
                  </tr>
          ...
          <tfoot>
            <tr className="bg-[#f7f9f8]">
              <td className="border-t border-[#e2e5ea] px-[var(--px)] py-3.5 text-[14px] font-bold uppercase tracking-[.04em] text-[#33443c]">
                Tổng cộng
              </td>
              <td className="border-t border-[#e2e5ea] px-3 py-3.5 text-[length:var(--fs-sm)] text-[#67737E]">
                {tong.soHoaDon} hoá đơn
                {contract.status === 'TERMINATED' && terminationInfo ? ' · 1 quyết toán' : ''}
              </td>
              <td className="border-t border-[#e2e5ea] px-3 py-3.5 text-right text-[16px] font-bold tabular-nums">
                {formatAmount(tong.soTien)}
              </td>
              <td className="border-t border-[#e2e5ea] px-3 py-3.5 text-right text-[16px] font-bold tabular-nums text-[#12764a]">
                {formatAmount(tong.daThu)}
              </td>
              <td
                className={`border-t border-[#e2e5ea] px-3 py-3.5 text-right text-[16px] font-bold tabular-nums ${
