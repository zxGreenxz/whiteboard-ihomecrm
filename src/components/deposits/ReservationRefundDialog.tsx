...
export function ReservationRefundDialog({ settlementId, amount, open, onOpenChange }: { settlementId: string | null; amount: number; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const submit = form.handleSubmit(({ accountId, paidOn, refundAttachments }) => settlementId && !uploading && !pay.isPending && pay.mutate({ settlementId, accountId, paidOn, refundAttachments }, {
    onSuccess: (result) => {
  ...
  return <Dialog open={open} onOpenChange={(next) => { if (!uploading && !pay.isPending) onOpenChange(next); }}><DialogContent className="max-w-md max-h-[90dvh] overflow-y-auto">
