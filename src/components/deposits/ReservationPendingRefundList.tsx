...
export function ReservationPendingRefundList({ buildingIds }: { buildingIds?: string[] }) {
  const query = useReservationSettlements({ buildingIds, refundState: "PENDING" });
  ...
  return <section className="space-y-3"><div><h2 className="font-bold">Chờ hoàn cọc</h2><p className="text-sm text-muted-foreground">Các khoản phải trả khách, không phụ thuộc ngày hẹn.</p></div>{rows.length === 0 ? <Card className="p-4 text-sm text-muted-foreground">Không có khoản nào đang chờ hoàn.</Card> : rows.map((row) => <Card key={row.id} className="flex flex-wrap items-center gap-3 p-4"><div className="min-w-0 flex-1"><b>{row.voucherCode || "Phiếu giữ chỗ"}</b><div className="text-sm text-muted-foreground">{row.payerName || "—"} · {row.buildingName}{row.roomName ? ` / ${row.roomName}` : ""}</div><div className="mt-1 font-bold text-amber-700">Còn phải hoàn {formatCurrency(row.refundRemaining)}</div></div><Button onClick={() => setPayTarget({ id: row.id, amount: row.refundRemaining })}>Hoàn tiền</Button></Card>)}<ReservationRefundDialog settlementId={payTarget?.id ?? null} amount={payTarget?.amount ?? 0} open={!!payTarget} onOpenChange={(next) => !next && setPayTarget(null)} /></section>;
}
