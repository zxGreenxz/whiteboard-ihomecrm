            ...
            {fleet.map((site) => {
              ...
              return (
                <TableRow key={site.buildingId}>
                  ...
                  <TableCell><strong>{arubaOnline}/{site.arubaNodes.length}</strong><small>Chỉ theo dõi</small></TableCell>
                  <TableCell><strong>{activeIncidents.length}</strong><small>đang mở</small></TableCell>
                  <TableCell><NetworkStatus kind={site.backupStatus} label={`${site.backupAgeHours} giờ`} /></TableCell>
          ...
          return (
            <article className="nc-mobile-site" key={site.buildingId}>
              ...
              <dl className="nc-mobile-site-grid">
                ...
                <div><dt>Aruba</dt><dd>{arubaOnline}/{site.arubaNodes.length} chỉ theo dõi</dd></div>
                <div><dt>Sự cố</dt><dd>{activeIncidents} đang mở</dd></div>
                <div><dt>Backup</dt><dd>{site.backupAgeHours} giờ · {site.backupStatus === "fresh" ? "mới" : "cũ"}</dd></div>
