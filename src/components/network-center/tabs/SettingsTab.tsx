  ...
  return (
    <section className="nc-panel">
      ...
      <div className="nc-settings-grid">
        <div className="nc-field"><Label htmlFor="polling-seconds">Chu kỳ kiểm tra (giây)</Label><Input id="polling-seconds" type="number" min={30} max={300} value={draft.pollingSeconds} onChange={(event) => setDraft((current) => ({ ...current, pollingSeconds: Number(event.target.value) }))} disabled={!executionAllowed} /></div>
        <div className="nc-field"><Label htmlFor="backup-hour">Giờ backup</Label><Input id="backup-hour" type="time" value={draft.backupHour} onChange={(event) => setDraft((current) => ({ ...current, backupHour: event.target.value }))} disabled={!executionAllowed} /></div>
        <div className="nc-field"><Label htmlFor="alert-sensitivity">Ngưỡng cảnh báo</Label><select id="alert-sensitivity" value={draft.alertSensitivity} onChange={(event) => setDraft((current) => ({ ...current, alertSensitivity: event.target.value as NetworkSettings["alertSensitivity"] }))} disabled={!executionAllowed}><option value="standard">Tiêu chuẩn</option><option value="strict">Nghiêm ngặt</option></select></div>
        <SettingSwitch label="Gom cảnh báo Aruba liên quan" checked={draft.dependencyGrouping} disabled={!executionAllowed} onChange={(checked) => setDraft((current) => ({ ...current, dependencyGrouping: checked }))} />
        <SettingSwitch label="Tạm dừng thay đổi" icon={<Pause />} checked={draft.changesPaused} disabled={!executionAllowed} onChange={(checked) => setDraft((current) => ({ ...current, changesPaused: checked }))} />
      </div>
