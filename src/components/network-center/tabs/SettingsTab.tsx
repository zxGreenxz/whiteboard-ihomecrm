  ...
  return (
    <section className="nc-panel">
      ...
      <div className="nc-settings-grid">
        <div className="nc-field"><Label htmlFor="polling-seconds">Chu kỳ kiểm tra (giây)</Label><Input id="polling-seconds" type="number" min={30} max={3600} value={draft.pollingSeconds} onChange={(event) => setDraft((current) => (current ? { ...current, pollingSeconds: Number(event.target.value) } : current))} disabled={!executionAllowed} /></div>
