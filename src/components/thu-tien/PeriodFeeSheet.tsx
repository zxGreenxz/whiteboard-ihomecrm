// =============================================================================
// PeriodFeeSheet — sheet MOBILE "Đóng tiền Tập trung theo Kỳ" (khung điện thoại).
// Port từ mockup TapTrungPhone.dc.html: header (bấm mở picker loại phí) + nội dung
// theo family (Tổng quan · Điện & Nước · GRID · Hoa hồng · Bảo trì) + bottom-sheet
// chọn loại phí. Backend dùng chung usePeriodFees / usePeriodFeeState / useUtilityPayState.
// =============================================================================

import { useMemo, useState } from 'react';
import { X, ChevronDown, Check, Camera, ArrowRight, LayoutGrid, Zap, Droplet, Calendar, HandCoins, Plus, Lock, Info, Edit3 } from 'lucide-react';
...
export function PeriodFeeSheet({ show, onClose, billingMonth, canRecordPayment }: Props) {
  ...
  const fam = category === 'overview' ? 'over' : cat?.family;

  const feeStatus = usePeriodFeeStatus(period, GRID_SERVER_KEYS, buildingIds, { enabled: show && buildingIds.length > 0 });
  ...
  const dueCountFor = (c: FeeCategory): number => {
    ...
    if (c.family === 'GRID') { const ids = c.elevatorGated ? buildingIds.filter((id) => elevatorIds.has(id)) : buildingIds; return ids.filter((id) => !(feeStatus.statusOf(id, c.serverKey)?.paidAmount)).length; }
    if (c.family === 'COMMISSION') return (commissions.data ?? []).filter((r) => r.status === 'unpaid').length;
    return 0;
  };

  const overview = useMemo(() => {
    let dueCount = 0, dueSum = 0; const nameOf = (id: string) => buildings.find((b) => b.id === id)?.name ?? id;
    const rows = FEE_CATEGORIES.map((c) => {
      let total = 0, paidN = 0, rowDue = 0; const dueList: string[] = [];
      if (c.family === 'EN') { for (const b of buildingIds) for (const k of ['dien', 'nuoc'] as const) { total++; const st = feeStatus.statusOf(b, k); if (st && st.paidAmount > 0) paidN++; else { rowDue += st?.expectedAmount ?? 0; if (!dueList.includes(nameOf(b))) dueList.push(nameOf(b)); } } }
      else if (c.family === 'GRID') { const ids = c.elevatorGated ? buildingIds.filter((id) => elevatorIds.has(id)) : buildingIds; for (const b of ids) { total++; const st = feeStatus.statusOf(b, c.serverKey); if (st && st.paidAmount > 0) paidN++; else { rowDue += st?.expectedAmount ?? 0; dueList.push(nameOf(b)); } } }
      else if (c.family === 'COMMISSION') { for (const r of commissions.data ?? []) { total++; if (r.status === 'paid') paidN++; else { rowDue += r.expectedAmount; if (!dueList.includes(r.buildingName)) dueList.push(r.buildingName); } } }
      else if (c.family === 'MAINTENANCE_BATCH') { const l = maintenance.data ?? []; total = l.length; paidN = l.length; }
      const dueN = total - paidN; dueCount += dueN; dueSum += rowDue;
      return { cat: c, total, paidN, dueN, dueList, dueSum: rowDue, pct: total ? Math.round((paidN / total) * 100) : 100, allPaid: dueN === 0 && total > 0 };
    });
    return { rows, dueCount, dueSum };
  ...
  return (
    <>
        ...
        <div className="sheet-scroll ptt-m-body">
          ...
          {fam === 'over' && (
            <div className="ptt-m-ov">
              <div className="ptt-m-ovtop">
                <div className="ptt-m-ovcard red"><div className="ptt-ov-lbl">Còn thiếu</div><div className="ptt-m-ovbig">{overview.dueCount}</div><div className="ptt-ov-sub">khoản chưa có phiếu</div></div>
              ...
              {overview.rows.map((r) => (
                <div className="ptt-m-ovrow" key={r.cat.key}>
                  <div className="ptt-m-ovrow-top">
                    ...
                    <span className="ptt-m-ovrow-lbl">{r.cat.label}</span>
          ...
          {fam === 'GRID' && (
            <div className="ptt-m-grid">
              <div className="ptt-gridstat" style={{ borderLeftColor: cat!.accent }}>
                ...
                <div className="ptt-gridstat-h"><div className="ptt-gridstat-title">{cat!.label}</div></div>
              </div>
              {cat!.restricted && <div className="ptt-note warn"><Lock /><span>Hạng mục hạn chế — chỉ người có quyền thấy phiếu.</span></div>}
              {cat!.elevatorGated && <div className="ptt-note info"><Info /><span>Chỉ hiện tòa có thang máy.</span></div>}
              <div className="ubc-list">
                {(cat!.elevatorGated ? buildings.filter((b) => elevatorIds.has(b.id)) : buildings).map((b) => {
                  const paid = S.paidOf(b.id); const isPaid = !!paid && paid.paidAmount > 0;
                  if (onlyDue && isPaid) return null;
                  ...
                  return (
                    <div className="ptt-m-card" key={b.id}>
                      <div className="ptt-m-cardhead"><span className="ud-bldcode">{b.name}</span><input className="ptt-m-code" placeholder={cat!.key === 'tien_nha' ? 'HĐ' : 'Mã NCC'} value={S.codeOf(b.id)} onChange={(e) => S.setField(b.id, { code: e.target.value })} onBlur={() => S.saveConfig(b.id)} /></div>
                      {cat!.multiPeriod && !isPaid && (
                        <div className="ptt-m-period"><Calendar /><span className="ptt-period-chips">{N_OPTIONS.map((v) => <button key={v} type="button" className={'ptt-period-chip' + (n === v ? ' on' : '')} onClick={() => S.setN(b.id, v)}>{v}</button>)}</span><span className="ptt-period-range">{rangeLabel(period, addMonths(period, n - 1))}</span></div>
                      )}
                      {cat!.multiPeriod && isPaid && paid!.coveredStart && (
                        <div className="ptt-cover self"><Calendar />Đã phủ {rangeLabel(paid!.coveredStart.slice(0, 7), (paid!.coveredEnd ?? paid!.coveredStart).slice(0, 7))}</div>
                      )}
                      {isPaid ? (
                        <div className="ubc-paid">
                          <span className="ubc-paid-pill"><span className="ubc-paid-txt"><span className="ubc-paid-a">Đã đóng {paid!.paidAmount.toLocaleString('vi-VN')}</span><span className="ubc-paid-m">{fmtDate(paid!.coveredStart)}</span></span></span>
                          {auto && <span className="ptt-auto">TỰ ĐỘNG</span>}
                          <span className="ptt-m-cardacts"><button type="button" className={'ptt-edit-btn' + (auto ? ' auto' : '')} onClick={() => S.openEdit(b.id)}><Edit3 /></button><button type="button" className="ubc-cancel" disabled={!canRecordPayment} onClick={() => S.requestCancel(b.id)}><X /></button></span>
                        </div>
                      ) : (
                        <>
                          <div className="ubc-pay">
                            <input className="ub-amt" type="text" inputMode="numeric" placeholder={def ? formatVN(def) : 'Số tiền'} value={formatVN(amount)} onChange={(e) => S.setAmount(b.id, parseVN(e.target.value))} />
                            <UtilityBookMenu accounts={S.myBooks} valueId={S.bookSel[b.id] ?? null} defaultId={S.defaultBookId} onPick={(id) => S.setBook(b.id, id)} compact disabled={!canRecordPayment} />
          ...
          {fam === 'COMMISSION' && (() => {
            ...
            return (
              <div className="ptt-m-comm">
                <div className="ptt-m-commstats"><div className="ptt-m-commcard"><div className="ptt-ov-lbl">Dự kiến</div><div className="ptt-m-commnum">{fmtFull(exp)}</div></div><div className="ptt-m-commcard green"><div className="ptt-ov-lbl">Đã chi</div><div className="ptt-m-commnum green">{fmtFull(paidSum)}</div></div><div className="ptt-m-commcard red"><div className="ptt-ov-lbl">Còn lại</div><div className="ptt-m-commnum red">{fmtFull(exp - paidSum)}</div></div></div>
                {rows.length === 0 ? <div className="c-empty"><div className="e-ic">📄</div><p>Không có HĐ ký trong kỳ.</p></div> : rows.map((r) => (
                  <div className="ptt-m-commrow" key={r.contractId}>
                    <div className="ptt-m-commr1"><span className="ud-mono2">{r.contractNumber ?? '—'}</span><span className="ptt-m-commroom">{r.buildingName} · {r.roomName ?? ''}</span>{r.status === 'paid' && <span className="ptt-comm-paid sm">Đã chi</span>}</div>
                    <div className="ptt-m-commr2"><div className="ptt-m-commtenant"><div>{r.tenantName}</div><div className="ptt-m-commmeta">Ký {fmtDate(r.signedDate)} · {r.months} th · bậc {r.tierPercent != null ? r.tierPercent + '%' : '—'}</div></div><span className={'ud-mono' + (r.status === 'paid' ? ' paid' : '')}>{fmtFull(r.expectedAmount)}</span></div>
                    {r.status === 'unpaid' && <button type="button" className="ptt-m-commbtn" disabled={!canRecordPayment || createComm.isPending} onClick={() => payCommission(r)}><HandCoins />Chi hoa hồng</button>}
                  </div>
                ))}
              </div>
            );
          ...
          {fam === 'MAINTENANCE_BATCH' && (
            <div className="ptt-m-batch">
              ...
              {maintenance.groups.length === 0 && maintenance.standalone.length === 0 && <div className="c-empty"><div className="e-ic">🔧</div><p>Kỳ này chưa có phiếu bảo trì.</p></div>}
            </div>
          )}
      ...
      <PeriodFeeEditModal target={S.editTarget} isAdmin={isAdmin} myBooks={S.myBooks} saving={S.saving} uploading={S.uploadingKey === '__edit__'} onAttach={S.onEditAttachClick} onClose={S.closeEdit} onSave={(args) => S.submitEdit({ isAdmin, ...args })} />
      ...
      <AttachmentLightbox attachments={EN.receiptView.attachments} index={EN.receiptView.index} onIndexChange={EN.setReceiptIndex} />
    </>
  );
}
