// =============================================================================
// PeriodFeePanel — panel DESKTOP "Đóng tiền Tập trung theo Kỳ" (cột trái, .tt-udesk).
// Ô chọn LOẠI PHÍ (bấm tiêu đề) → quyết family: Tổng quan · Điện & Nước (EN) ·
// GRID (đơn/đa kỳ) · Hoa hồng · Bảo trì (phiếu tổng). Port từ mockup
// "Đóng tiền Tập trung theo Kỳ.dc.html". Backend qua usePeriodFees + pay_period_fee.
// =============================================================================

import { useMemo, useRef, useState } from 'react';
import {
  ArrowLeft, X, ChevronDown, Check, Camera, ArrowRight, Lock, Info, Plus, Trash2,
  HandCoins, Edit3, Calendar,
} from 'lucide-react';
...
export function PeriodFeePanel({ billingMonth, onBillingMonthChange, onClose, canRecordPayment }: Props) {
  ...
  const isEN = cat?.family === 'EN';
  const isGrid = cat?.family === 'GRID';
  const isComm = cat?.family === 'COMMISSION';
  const isBatch = cat?.family === 'MAINTENANCE_BATCH';

  // ── Data (load-once, cache; enable theo nhu cầu) ──
  const feeStatus = usePeriodFeeStatus(period, GRID_SERVER_KEYS, buildingIds, { enabled: buildingIds.length > 0 });
  ...
  // GRID interaction state (chỉ cần khi cat là GRID)
  ...
  // ── Đếm "chưa đóng" cho badge dropdown ──
  const dueCountFor = (c: FeeCategory): number => {
    ...
    if (c.family === 'GRID') {
      const ids = c.elevatorGated ? buildingIds.filter((id) => elevatorIds.has(id)) : buildingIds;
      return ids.filter((id) => !(feeStatus.statusOf(id, c.serverKey)?.paidAmount)).length;
    }
    if (c.family === 'COMMISSION') return (commissions.data ?? []).filter((r) => r.status === 'unpaid').length;
    return 0;
  };

  // ── Tổng quan ──
  const overview = useMemo(() => {
    let dueCount = 0, slots = 0, dueSum = 0, paidSum = 0, paidCount = 0;
    const dueBld = new Set<string>();
    const nameOf = (id: string) => buildings.find((b) => b.id === id)?.name ?? id;
    const rows = FEE_CATEGORIES.map((c) => {
      let total = 0, paidN = 0, rowDueSum = 0, rowPaidSum = 0; const dueList: string[] = [];
      if (c.family === 'EN') {
        ...
        for (const b of ids) {
          ...
          if (st && st.paidAmount > 0) { paidN++; rowPaidSum += st.paidAmount; }
          else { rowDueSum += st?.expectedAmount ?? 0; dueList.push(nameOf(b)); dueBld.add(b); }
        }
      } else if (c.family === 'COMMISSION') {
        for (const r of commissions.data ?? []) {
          total++;
          if (r.status === 'paid') { paidN++; rowPaidSum += r.expectedAmount; }
          else { rowDueSum += r.expectedAmount; if (!dueList.includes(r.buildingName)) dueList.push(r.buildingName); }
        }
      ...
      dueCount += dueN; slots += total; dueSum += rowDueSum; paidSum += rowPaidSum; paidCount += paidN;
      return {
        cat: c, total, paidN, dueN, dueList, dueSum: rowDueSum,
        pct: total ? Math.round((paidN / total) * 100) : 100,
        allPaid: dueN === 0 && total > 0, empty: total === 0,
      };
    });
    return { rows, dueCount, slots, dueSum, paidSum, paidCount, dueBldCount: dueBld.size };
  }, [buildingIds, feeStatus.byKey, commissions.data, maintenance.data, elevatorIds, buildings]);

  // ── Category menu ──
  const pickCategory = (k: string) => { setCategory(k); setMenuOpen(false); setBldFilter('all'); setOnlyDue(false); };
  ...
  const gridBuildings = useMemo(() => {
    if (!isGrid) return [];
    let list = cat!.elevatorGated ? buildings.filter((b) => elevatorIds.has(b.id)) : buildings;
    if (bldFilter !== 'all') list = list.filter((b) => b.id === bldFilter);
    return list;
  }, [isGrid, cat, buildings, elevatorIds, bldFilter]);

  const gridStat = useMemo(() => {
    if (!isGrid) return { sum: 0, total: 0, paidN: 0, dueList: [] as string[] };
    const ids = cat!.elevatorGated ? buildingIds.filter((id) => elevatorIds.has(id)) : buildingIds;
    let sum = 0, paidN = 0; const dueList: string[] = [];
    for (const id of ids) { const st = feeStatus.statusOf(id, cat!.serverKey); if (st && st.paidAmount > 0) { sum += st.paidAmount; paidN++; } else dueList.push(buildings.find((b) => b.id === id)?.name ?? id); }
    return { sum, total: ids.length, paidN, dueList };
  ...
  const payCommission = async (r: PeriodCommissionRow) => {
    try {
      await createComm.mutateAsync({
        ...
        kind: 'broker', amount: r.expectedAmount, payer_name: r.tenantName || null,
  ...
  const saveBatch = async () => {
    try {
      await createBatch.mutateAsync({ payerName: payer, voucherDate: new Date().toISOString().slice(0, 10), accountId: S.defaultBookId ?? '', lines: batchLines.filter((l) => l.buildingId && l.amount > 0) });
      toast.success('Đã tạo phiếu tổng bảo trì');
      setCreateOpen(false); setBatchLines([]); setPayer('');
            ...
            <div className="ptt-menu">
              ...
              {FEE_GROUPS.map((g) => (
                <div key={g}>
                  ...
                  {FEE_CATEGORIES.filter((c) => c.group === g).map((c) => {
        ...
        <div className="ptt-scroll">
          <div className="ptt-ov-stats">
            <div className="ptt-ov-card red">
              ...
              <div className="ptt-ov-sub">trên {overview.slots} khoản · {overview.dueBldCount} tòa liên quan</div>
            </div>
            <div className="ptt-ov-card amber">
              ...
              <div className="ptt-ov-sub">theo số tiền dự kiến từng hạng mục</div>
            </div>
            <div className="ptt-ov-card green">
              ...
              <div className="ptt-ov-sub">{overview.paidCount} khoản đã có phiếu</div>
            </div>
          </div>
      ...
      {isGrid && (
        <div className="ptt-scroll">
          <div className="ptt-gridstat" style={{ borderLeftColor: cat!.accent }}>
            ...
            <div className="ptt-gridstat-h"><div className="ptt-gridstat-title">{cat!.label} · kỳ {fmtBillingMonth(period)}</div><div className="ptt-gridstat-sub">{gridStat.paidN}/{gridStat.total} tòa đã có phiếu</div></div>
          ...
          {cat!.restricted && (
            <div className="ptt-note warn mx"><Lock /><span>Hạng mục <b>hạn chế</b> — chỉ người có quyền xem/đóng thấy phiếu này. Phiếu ẩn khỏi báo cáo với người thiếu quyền.</span></div>
          )}
          {cat!.elevatorGated && (
            <div className="ptt-note info mx"><Info /><span>Chỉ hiển thị <b>{gridBuildings.length} tòa có thang máy</b>. Các tòa còn lại được ẩn cho hạng mục này.</span></div>
          )}
          ...
          <div className="ud-toolbar">
            <label className="ud-dd"><span>Tòa nhà</span>
              <select value={bldFilter} onChange={(e) => setBldFilter(e.target.value)}>
                ...
                {(cat!.elevatorGated ? buildings.filter((b) => elevatorIds.has(b.id)) : buildings).map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}
              </select>
            </label>
            ...
            {cat!.multiPeriod && <span className="ptt-multibadge"><Calendar />Trả trước nhiều kỳ — chi phí tự chia đều qua các tháng</span>}
          </div>

          <div className="ud-body">
            {loadingBld || feeStatus.isLoading ? <div className="ud-empty">⏳ Đang tải…</div> : gridBuildings.length === 0 ? <div className="ud-empty">🏢 Không có tòa nào cho hạng mục này.</div> : (
              <div className="ud-tablewrap">
                <table className="ud-table">
                  <thead><tr>
                    <th>Tòa</th><th>{cat!.key === 'tien_nha' ? 'Hợp đồng' : (cat!.multiPeriod ? 'Mã NCC' : 'Ghi chú')}</th><th>{cat!.key === 'tien_nha' ? 'Chủ tòa' : 'Đơn vị'}</th><th>Sổ quỹ ghi chi</th>
                    {cat!.multiPeriod && <th>Kỳ áp dụng</th>}<th className="num">Số tiền</th><th className="act">Thao tác</th>
                  </tr></thead>
                  <tbody>
                    {gridBuildings.map((b) => {
                      const paid = S.paidOf(b.id);
                      if (onlyDue && paid && paid.paidAmount > 0) return null;
                      const isPaid = !!paid && paid.paidAmount > 0;
                      const n = S.nOf(b.id);
                      const amount = S.amountOf(b.id);
                      const paying = S.payingKey === b.id;
                      const def = S.defaultAmountOf(b.id);
                      const auto = isPaid && paid!.accountIsEmpty;
                      return (
                        <tr key={b.id}>
                          <td className="ud-td-bld"><span className="ud-bldcode">{b.name}</span></td>
                          <td><input className="ud-code" placeholder={cat!.key === 'tien_nha' ? 'HĐ thuê' : 'Mã NCC'} value={S.codeOf(b.id)} onChange={(e) => S.setField(b.id, { code: e.target.value })} onBlur={() => S.saveConfig(b.id)} /></td>
                          <td><input className="ud-holder" placeholder={cat!.key === 'tien_nha' ? 'Chủ tòa' : 'Đơn vị'} value={S.holderOf(b.id)} onChange={(e) => S.setField(b.id, { holder: e.target.value })} onBlur={() => S.saveConfig(b.id)} /></td>
                          <td>{isPaid ? <span className={'ud-bookchip' + (auto ? ' empty' : '')}><BookIcon size={14} />{paid!.accountName || 'chưa có sổ'}</span> : <UtilityBookMenu accounts={S.myBooks} valueId={S.bookSel[b.id] ?? null} defaultId={S.defaultBookId} onPick={(id) => S.setBook(b.id, id)} disabled={!canRecordPayment} />}</td>
                          {cat!.multiPeriod && (
                            <td>
                              {isPaid ? (
                                <span className="ptt-cover"><Calendar />{paid!.coveredStart ? rangeLabel(paid!.coveredStart.slice(0, 7), (paid!.coveredEnd ?? paid!.coveredStart).slice(0, 7)) : fmtBillingMonth(period)}</span>
                              ) : (
                                <span className="ptt-period">
                                  <span className="ptt-period-chips">{N_OPTIONS.map((v) => <button key={v} type="button" className={'ptt-period-chip' + (n === v ? ' on' : '')} onClick={() => S.setN(b.id, v)}>{v}</button>)}</span>
                          ...
                          <td className="num">
                            {isPaid ? (
                              <div className="ud-paidamt"><span className="ud-paidamt-a">{fmtFull(paid!.paidAmount)}</span><span className="ud-paidamt-m">{paid!.coveredStart ? paid!.coveredStart.slice(0,10).split('-').reverse().join('/') : ''}</span></div>
                            ) : (
                              <>
                                <input className="ud-amt" type="text" inputMode="numeric" placeholder={def ? formatVN(def) : 'Số tiền'} value={formatVN(amount)} onChange={(e) => S.setAmount(b.id, parseVN(e.target.value))} />
                                {cat!.multiPeriod && n > 1 && amount > 0 && <div className="ptt-total">×{n} kỳ = <b>{fmtFull(amount * n)}</b></div>}
                              </>
                            )}
                          </td>
                          <td className="act">
                            {isPaid ? (
                              <span className="ud-acts">
                                {auto && <span className="ptt-auto">TỰ ĐỘNG</span>}
                                <span className="ud-check"><Check /></span>
                                <button type="button" className={'ptt-edit-btn' + (auto ? ' auto' : '')} title="Sửa phiếu" onClick={() => S.openEdit(b.id)}><Edit3 /></button>
                                <button type="button" className="ud-cancel" title="Hủy phiếu" disabled={!canRecordPayment} onClick={() => S.requestCancel(b.id)}><X /></button>
                              </span>
                            ) : (
                              <span className="ud-acts">
      ...
      {isComm && (
        <div className="ptt-scroll">
          {(() => {
            ...
            const exp = rows.reduce((s, r) => s + r.expectedAmount, 0);
            const paidSum = rows.filter((r) => r.status === 'paid').reduce((s, r) => s + r.expectedAmount, 0);
            const dueSum = exp - paidSum;
            const paidN = rows.filter((r) => r.status === 'paid').length;
            return (
              <>
                <div className="ptt-comm-stats">
                  <div className="ptt-comm-card"><div className="ptt-ov-lbl">HH dự kiến kỳ này</div><div className="ptt-comm-num">{fmtFull(exp)}</div><div className="ptt-ov-sub">{rows.length} hợp đồng ký trong kỳ</div></div>
                  <div className="ptt-comm-card green"><div className="ptt-ov-lbl">Đã chi</div><div className="ptt-comm-num green">{fmtFull(paidSum)}</div><div className="ptt-ov-sub">{paidN} hợp đồng</div></div>
                  <div className="ptt-comm-card red"><div className="ptt-ov-lbl">Còn phải chi</div><div className="ptt-comm-num red">{fmtFull(dueSum)}</div><div className="ptt-ov-sub">{rows.length - paidN} hợp đồng</div></div>
                </div>
                <div className="ud-body">
                  {commissions.isLoading ? <div className="ud-empty">⏳ Đang tải…</div> : rows.length === 0 ? <div className="ud-empty">📄 Không có hợp đồng nào ký trong kỳ.</div> : (
                    <div className="ud-tablewrap">
                      <table className="ud-table">
                        <thead><tr><th>Hợp đồng</th><th>Phòng · Khách thuê</th><th>Ngày ký</th><th className="ctr">Số tháng</th><th className="ctr">Bậc HH</th><th className="num">HH dự kiến</th><th className="act">Thao tác</th></tr></thead>
                        <tbody>
                          {rows.map((r) => (
                            <tr key={r.contractId}>
                              ...
                              <td className="ud-mono2">{r.signedDate ? r.signedDate.slice(0,10).split('-').reverse().join('/') : ''}</td>
                              ...
                              <td className="num"><span className={'ud-mono' + (r.status === 'paid' ? ' paid' : '')}>{fmtFull(r.expectedAmount)}</span></td>
                              <td className="act">
                                {r.status === 'unpaid' ? (
                                  <button type="button" className="ptt-comm-pay" disabled={!canRecordPayment || createComm.isPending} onClick={() => payCommission(r)}><HandCoins />Chi HH</button>
                                ) : (
                                  <span className="ptt-comm-paid">Đã chi</span>
                                )}
                              </td>
                            </tr>
                          ))}
                      ...
                      <div className="ptt-comm-note">HH dự kiến tính từ <b>bậc hoa hồng</b> của tòa theo số tháng hợp đồng. Bấm <b>Chi HH</b> tạo phiếu chi gắn hợp đồng — P&amp;L hiện dòng "thanh toán hoa hồng" theo phòng.</div>
                    </div>
                  )}
                </div>
              </>
              ...
              <div className="ptt-batch-list">
                ...
                {maintenance.standalone.length > 0 && (
                  <div className="ptt-batch-standalone">
                    <div className="ptt-batch-sohead"><span className="ptt-batch-sotitle">Phiếu lẻ (chưa gộp phiếu tổng)</span><span className="ptt-batch-sohint">tạo tay bên Thu chi · có thể gộp vào 1 phiếu tổng</span></div>
                    {maintenance.standalone.map((r) => (
                      <div className="ptt-batch-sorow" key={r.voucherId}>
                        ...
                        <span className="ptt-batch-someta">{r.voucherDate ? r.voucherDate.slice(0,10).split('-').reverse().join('/') : ''} · {r.accountName ?? '—'}</span>
