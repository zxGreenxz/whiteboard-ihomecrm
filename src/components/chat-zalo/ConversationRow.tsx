...
interface Props {
  ...
  /** menu chuột phải: ghim / tắt thông báo / đánh dấu chưa đọc / gắn hồ sơ */
...
/**
 * 1 dòng hội thoại (avatar + tên + preview + badge).
 * Click TRÁI = mở hội thoại; menu CHỈ mở bằng chuột phải, neo tại vị trí con
 * trỏ qua một trigger ẩn 1px — KHÔNG bọc row trong DropdownMenuTrigger vì
 * Radix coi click trái vào trigger là lệnh mở menu (bug đã gặp trên production:
 * bấm tin nhắn ra menu thay vì mở chat).
 */
export default function ConversationRow({ conv, active, onSelect, onTogglePin, onToggleMute, onToggleUnread, onLinkCrm }: Props) {
  ...
  return (
    <>
      ...
      {hasMenu && menuPos && (
        <DropdownMenu open onOpenChange={(v) => { if (!v) setMenuPos(null); }}>
          <DropdownMenuTrigger asChild>
            {/* trigger ẩn 1px neo menu tại vị trí chuột phải — không nhận click */}
            <span aria-hidden style={{ position: 'fixed', left: menuPos.x, top: menuPos.y, width: 1, height: 1, pointerEvents: 'none' }} />
          </DropdownMenuTrigger>
          <DropdownMenuContent align="start" className="w-56">
            {onTogglePin && (
              <DropdownMenuItem onClick={() => { setMenuPos(null); onTogglePin(conv); }}>
                <Pin className="mr-2 h-4 w-4" />
                {conv.pinned ? 'Bỏ ghim' : 'Ghim hội thoại'}
              </DropdownMenuItem>
            )}
            {onToggleMute && (
              <DropdownMenuItem onClick={() => { setMenuPos(null); onToggleMute(conv); }}>
                <BellOff className="mr-2 h-4 w-4" />
                {conv.muted ? 'Bật thông báo' : 'Tắt thông báo'}
              </DropdownMenuItem>
            )}
            {onToggleUnread && (
              <DropdownMenuItem onClick={() => { setMenuPos(null); onToggleUnread(conv); }}>
            ...
            {onLinkCrm && !conv.isGroup && (
              <DropdownMenuItem onClick={() => { setMenuPos(null); onLinkCrm(conv); }}>
