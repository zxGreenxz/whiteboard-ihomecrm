...
export function useContractSettlement(scope: SettlementReadScope, selection: SettlementSelection | null = null, enabled = true) {
 ...
 return {...query.data,rows:query.data?.rows??[],totals:query.isError?null:query.data?.totals??null,loading:query.isPending,fetching:query.isFetching,error,errorCode,partial:query.isError||query.data?.partial||false,pagination:query.data?.pagination??{complete:false,pages:0,nextCursor:null},selectionDetail,refresh:async():Promise<void>=>{const result=await query.refetch();if(result.error||result.data?.error||!result.data?.pagination.complete)throw new Error('Chưa tải lại được dữ liệu quyết toán. Vui lòng thử lại.');}};
}
