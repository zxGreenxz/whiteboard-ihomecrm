...
Bước:
1. Chọn tháng (MM-yyyy), toà = **`BuildingMultiSelect`** (chọn nhiều toà, nhóm theo khu vực — từ 9ad626d thay cặp SearchableSelect khu vực/toà cũ), loại Thu/Chi. Trang fetch `useBuildings({ includeVirtual: true })` truyền vào component nên chọn được cả toà ảo "Chung" (để soi phiếu chia LN); selection thành `building_ids` → cả `useIncomeExpenses`/`useIncomeExpenseStats` lẫn `useAccrualMonthReport` lọc `.in("building_id", ids)` **trực tiếp** (hết round-trip suy khu → toà). **Ô lọc phòng là dropdown chết** — options chỉ có đúng 1 mục "Tất cả phòng", không load danh sách phòng nào ([ProfitDistributionReport.tsx](src/pages/reports/finance/ProfitDistributionReport.tsx)).
