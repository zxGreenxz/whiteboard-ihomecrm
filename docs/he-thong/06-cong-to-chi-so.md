...
| `reading_code` | Mã chỉ số duy nhất **TOÀN CỤC** (unique index `idx_meter_readings_code` KHÔNG lọc theo user), format **`CSS{YYMM}{seq 5 chữ số}`** (VD `CSS250700001`). Auto-sinh bởi trigger nếu bỏ trống — generator đánh số **global MAX+1 theo prefix + advisory lock + SECURITY DEFINER** (xem §4.3, viết lại 2026-06-30/07-01). Riêng chốt điện lúc **thanh lý HĐ** dùng prefix riêng `TLY{YYYYMMDD}-…` để né generator (xem §6). |
...
flowchart TD
    ...
    B0 --> D["trigger trigger_auto_generate_reading_code<br/>→ reading_code = CSS{YYMM}{seq}<br/>(MAX+1 TOÀN CỤC theo prefix, advisory lock,<br/>SECURITY DEFINER — §4.3)"]
...
- ⚠️ **Gotcha:** trigger **GHI ĐÈ kể cả khi INSERT truyền giá trị** (không có check "fill-when-null"). `GenerateInvoiceDialog`/`ExcelInvoiceDialog` truyền `previous_reading` đã override tay nhưng giá trị lưu thật vẫn là reading gần nhất → lịch sử công tơ có thể lệch với hoá đơn; nếu prev override xuống thấp khiến `current < previous` thực thì CHECK fail và reading không được lưu trong khi hoá đơn vẫn tạo — `GenerateInvoiceDialog` chỉ `console.warn` (im lặng), còn `ExcelInvoiceDialog` từ `3181abe`/`e5dd8cc` **đã check `.error`** và toast đỏ "Chưa lưu được chỉ số điện" kèm tên phòng (xem §6).

### 4.3. Trigger `auto_generate_reading_code` (BEFORE INSERT) — đã viết lại 2 lần (2026-06-30 + 2026-07-01)

Phiên bản hiện hành (migration [`20260630000001_fix_reading_code_global_unique.sql`](supabase/migrations/20260630000001_fix_reading_code_global_unique.sql) + [`20260701000001_secdef_code_generators.sql`](supabase/migrations/20260701000001_secdef_code_generators.sql)):

- Bỏ qua nếu `reading_code` đã có giá trị (đường thanh lý `TLY…` đi qua nhánh này — §6).
- `prefix = 'CSS' || TO_CHAR(reading_date, 'YYMM')`.
- `PERFORM pg_advisory_xact_lock(hashtext(prefix))` — **khoá advisory theo prefix trong transaction**: các insert song song cùng tháng xếp hàng, không thể cùng tính ra một MAX.
- `seq` = **MAX toàn cục** phần số của các `reading_code` cùng prefix (KHÔNG lọc `user_id`) **+ 1**; kết quả `prefix || LPAD(seq, 5, '0')`. Dùng **MAX+1** (không phải COUNT+1) nên row xoá mềm/khoảng hở không bao giờ sinh lại số đã dùng.
- Hàm chạy **`SECURITY DEFINER` + `SET search_path = public`** — SELECT nội bộ bypass RLS, thấy TOÀN BỘ bảng khi tính MAX.

**Lịch sử 2 tầng bug (cùng một triệu chứng: nhân viên lưu chỉ số bị 23505 trùng mã, caller nuốt lỗi thì MẤT chỉ số):**

1. **Đánh số per-user vs UNIQUE toàn cục** (fix `e5dd8cc`, 2026-06-30): bản gốc `seq = COUNT(*)+1 WHERE user_id = NEW.user_id`, nhưng `idx_meter_readings_code` là unique **toàn cục** → 2 owner khác nhau cùng sinh ra `CSS260600001` → unique violation. Dialog Mode Excel (bản cũ) không check error insert nên chỉ số "bốc hơi" âm thầm, gãy chuỗi carry-forward chỉ số điện tháng-sang-tháng (dữ liệu mất đã được backfill; dialog nay đã check `.error` — xem §6).
2. **Generator SECURITY INVOKER trên bảng có RLS** (fix `13bf498`, 2026-07-01): sau fix 1 vẫn trùng mã khi **nhân viên** ghi — hàm chạy dưới RLS của caller, staff chỉ thấy reading của toà mình được gán (case thật: staff nathan thấy 8/90 row `CSS2607`) → MAX tính trên tập THIẾU → sinh `CSS260700009` đã tồn tại → 23505. **Chủ (is_admin) thấy hết nên test bằng chủ KHÔNG lộ lỗi.** Sửa: SECURITY DEFINER + search_path + advisory lock (mẫu chuẩn có sẵn: `generate_job_code`).

> **Bug class dùng chung toàn hệ thống:** mọi trigger sinh mã kiểu `MAX()/COUNT()` trên bảng-có-RLS đều dính lỗi 2 nếu để SECURITY INVOKER. Migration `20260701000001` đã áp secdef + lock cho **7 generator**: `auto_generate_reading_code` (CSS), `set_material_purchase_code` (MP), `set_material_usage_code` (MU), `set_material_adjustment_code` (MA), `auto_generate_voucher_code` (PT/PC — thu chi), `generate_template_code` (MT), `generate_invoice_number_v2` (INV — fallback, FE thường tự truyền số). Generator mới PHẢI theo mẫu này.
...
- **Hoá đơn** (`invoices`/`invoice_items`): liên kết quan trọng nhất — domain hoá đơn vừa **ĐỌC** vừa **GHI** `meter_readings`:
  - Đường chạy thật: **`GenerateInvoiceDialog`** + **`ExcelInvoiceDialog`** (đều mount trong [`InvoicesPage.tsx`](src/pages/invoices/InvoicesPage.tsx)) tự tra meter **ĐIỆN** của phòng (`meters` theo `room_id` + `meter_type='ELECTRICITY'`), prefill "chỉ số đầu" = `current_reading` của reading **APPROVED gần nhất**, và khi submit **INSERT một `meter_reading` mới `status='APPROVED'`** vào bảng ("chốt" chỉ số ngay lúc lập hoá đơn — Mode Excel có từ `3181abe`, 2026-06-30, để chỉ số đầu tháng sau carry-forward đúng). `GenerateInvoiceDialog` có guard skip-nếu-đã-có reading theo `settlement_month = billing_month`; `ExcelInvoiceDialog` **không** guard nhưng dùng `reading_date = ngày CUỐI kỳ billing_month` → trigger §4.1 suy `settlement_month` đúng kỳ, và reading luôn là "mới nhất" của meter. [`EditInvoiceDialog.tsx`](src/components/invoices/EditInvoiceDialog.tsx) cũng load meter điện của phòng để auto tính tiền điện theo chỉ số. (Perf: `ExcelInvoiceDialog` lấy "chỉ số gần nhất" bằng cách SELECT **toàn bộ** readings APPROVED của mọi meter điện trong toà rồi dedupe client-side — toà nhiều lịch sử sẽ tải rất nhiều row thừa.)
  - **Xử lý lỗi insert reading (đã sửa sau sự cố mất chỉ số):** bản cũ của `ExcelInvoiceDialog` **nuốt lỗi** insert (không check `.error`) — kết hợp bug generator trùng mã (§4.3) làm chỉ số điện "bốc hơi" âm thầm dù hoá đơn vẫn tạo; dữ liệu mất đã được **backfill** và dialog nay check `readingErr` từng phòng → hoá đơn vẫn tạo nhưng cuối lô toast đỏ **"Chưa lưu được chỉ số điện"** liệt kê tên phòng. `GenerateInvoiceDialog` vẫn chỉ `console.warn` (chấp nhận được vì có guard skip + generator đã sửa).
  ...
  - ⚠️ Gotcha trigger với luồng này: row insert truyền `settlement_month = billing_month` và `previous_reading` (đã override) nhưng trigger BEFORE INSERT **ghi đè cả hai** (§4.1, §4.2). Với `ExcelInvoiceDialog` hệ quả đã được vô hiệu bằng cách đặt `reading_date` = cuối kỳ (settlement thật = đúng kỳ); với `GenerateInvoiceDialog` (`reading_date = issue_date`) chọn kỳ khác tháng phát hành thì reading vẫn rơi lệch tháng → guard chống trùng check theo giá trị chưa-bị-ghi-đè. `previous_reading` thật luôn = reading gần nhất; nếu prev override thấp làm CHECK `current >= previous` fail thì hoá đơn vẫn tạo — `GenerateInvoiceDialog` bỏ qua im lặng (`console.warn`), `ExcelInvoiceDialog` toast đỏ báo phòng lỗi.
  - **Chốt điện khi THANH LÝ hợp đồng** (khu "Thu thêm" — [`20260627000001_termination_extra_charges.sql`](supabase/migrations/20260627000001_termination_extra_charges.sql)): RPC thanh lý (`terminate_*_impl` với `p_extra_charges`) khi có khoản loại **ELECTRIC** kèm `meter_id` sẽ **INSERT thêm 1 `meter_reading` APPROVED** chốt số cuối. Điểm khác đường hoá đơn: `reading_code` được RPC **tự cấp** với prefix riêng **`TLY{YYYYMMDD}-{8 ký tự ngẫu nhiên}`** (né trigger generator — thời điểm viết generator còn đánh số per-user dễ đụng mã) và insert bọc trong `BEGIN…EXCEPTION WHEN OTHERS THEN NULL` — **lỗi chèn chỉ số không bao giờ chặn thanh lý** (trade-off: fail thì cũng im lặng). Không có trigger nào tự sinh hoá đơn từ reading nên không trùng tiền. Chi tiết nghiệp vụ thanh lý: [16-thanh-ly-hop-dong.md](docs/he-thong/16-thanh-ly-hop-dong.md).
