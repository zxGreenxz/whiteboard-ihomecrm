...
Mục đích: ví tiền. Cột chủ chốt: `name`, `code` (auto `TK{6 số}`, unique), `bank_name`/`account_number`/`bank_account_holder`/`branch`, `description`, `is_default` (sổ mặc định — quyết định sổ nào được auto-pick khi user sở hữu **nhiều** sổ "…Thu", từ 25/09/2026 sổ nhận khi thu tiền do máy chủ quyết theo hình thức — xem §5.10), **`initial_amount`/`initial_date`** (số dư & ngày chốt đầu kỳ), **`lock_date`** (khoá sổ — chặn phiếu có `voucher_date ≤ lock_date`), `quick_default_building_id` → `buildings` (toà mặc định khi tạo phiếu nhanh), `user_id` (owner phụ trách sổ). FK ra: `buildings`. Được tham chiếu bởi: `income_expenses` (×3), `account_shared_users`, `buildings.default_account_id_tk/tt`.
...
### 4.8 Bổ sung chứng từ / ghi chú — chỉ thêm nội dung mới
...
Thao tác dùng được khi phiếu đã duyệt, đã thu/chi, đã huỷ hoặc thuộc kỳ đã chốt, bao gồm phiếu bỏ/hoàn cọc. Người dùng vẫn cần quyền xem và bổ sung vào đúng phiếu; ảnh của phiếu hạn chế được bảo vệ theo quyền xem phiếu. Không thay đổi số tiền, sổ quỹ, ngày phiếu hoặc trạng thái.

Phần bổ sung lưu riêng qua `append_income_expense_supplement_v1` trong `income_expense_supplements`, không cập nhật phiếu gốc. Ghi chú gốc còn được một số nghiệp vụ dùng để nhận diện giao dịch, nên được giữ nguyên từng nội dung. Các màn xem, bản in và ảnh hoàn cọc ghép thêm phần bổ sung khi hiển thị. RPC `annotate_income_expense_v1` cũ vẫn phục vụ các luồng ghi sổ/Copilot hiện hữu; màn bổ sung này không dùng nó và không còn ô đổi sổ quỹ.
...
2. *Sửa phiếu Chờ duyệt* (đợt 1 sửa phiếu, 25/09/2026) → `useReviseIncomeExpense` ([revisions.ts](src/hooks/income-expenses/revisions.ts)) → RPC `revise_pending_income_expense_v1(p_voucher, p_expected_approval_version, p_patch, p_items, p_reason, p_idempotency_key)`:
   - **Ai / phiếu nào:** người lập hoặc người có quyền sửa thu chi ở toà (super admin đi thẳng); chỉ phiếu Chờ duyệt, chưa ghi sổ, lập tay hoặc hoa hồng / trả khách thanh lý (hai loại sau chỉ đổi được tiền của hạng mục đang có + sổ/ngày/người nhận/tên/ảnh/ghi chú). Phiếu gắn hoá đơn, chia lợi nhuận, cặp bỏ cọc… sửa ở luồng gốc. Nút bút chì dùng `voucherEditAction` ([incomeExpenseRevision.ts](src/lib/incomeExpenseRevision.ts)); "Sửa phiếu (Super Admin)" cho phiếu đã duyệt/đã huỷ **đã bỏ**.
   - **Form** chỉ gửi ô đã đổi so với ảnh chụp lúc mở (`buildRevisionPatch`), hạng mục chỉ gửi khi đổi; hạng mục chưa ghi kỳ giữ kỳ trống; phiếu chưa có sổ được giữ sổ trống (`incomeExpenseNoAccountFormSchema`).
   - **Lý do ≥ 8 ký tự** khi đổi Thu/Chi, toà, sổ quỹ (từ sổ này sang sổ khác), KQKD, hoặc số/loại/đơn giá/kỳ hạng mục (`requiresRevisionReason`, cùng luật máy chủ).
   - **Chống sửa chồng:** gửi `approval_version` lúc mở; lệch ⇒ `PT409` (HTTP 409), form khoá nút Lưu và bảo mở lại phiếu. Không dùng `40001` cho lỗi này: PostgREST coi 40001 là lỗi tuần tự hoá và tự chạy lại giao dịch mãi (đo trên TEST: treo 125 giây rồi 504, còn giữ khoá org).
   - **Đổi Thu ↔ Chi** cấp **mã mới** đúng tiền tố PT/PC từ bộ đếm chung (`app_private.next_voucher_code_v1`); mã cũ bỏ, không tái dùng.
   - Mỗi lần sửa một dòng `income_expense_revisions` (ai, lúc nào, lý do, ảnh chụp trước/sau) — hộp Duyệt hiện "đã sửa N lần" + bảng lúc lập → hiện tại (`RevisionComparison`), chi tiết phiếu có "Lịch sử sửa". Kênh cũ `ie_compat_update_pending_v2` chỉ còn tên/ghi chú/ảnh (nối thêm) của phiếu đã duyệt.
   Để thêm mô tả hoặc ảnh ở mọi trạng thái, dùng nút **Bổ sung chứng từ / ghi chú** (§4.8), có cả với Admin. Nội dung cũ chỉ xem; nội dung mới được lưu riêng kèm người bổ sung và thời gian.
3. *Duyệt* → `useApproveVoucher({ id, expectedApprovalVersion })` → RPC `approve_pending_income_expense_checked_v1` (khoá org → phiếu → so phiên bản → thang ba bậc cặp bỏ cọc / canonical / legacy trên máy chủ). Org CANONICAL đi `approve_income_expense_v2` (cũng kèm phiên bản). Phiếu vừa bị sửa ⇒ "tải lại để xem thay đổi trước khi duyệt".
