...
- Sửa/huỷ/đính ảnh làm theo **từng voucher** qua `update_period_fee`, `cancel_period_fee`, `append_fee_attachment`; phiếu nhiều item bị hạn chế sửa tiền/kỳ và phiếu thuộc batch không được huỷ từ lưới. Bảo trì đọc các phiếu batch/đơn hiện có để tránh tạo lại. Căn cứ hoa hồng theo kỳ ký (`get_period_commissions`) nay chỉ khu Hợp đồng & quyết toán đọc (`useContractSettlement`); modal "Chi hoa hồng môi giới" cũ và hai sổ "chi thanh lý"/"thưởng Sale" đã gỡ khỏi trang 23/09/2026.
