...
> đã đổi (forfeit dùng `LEAST(total_deposit, deposit_paid)` + **cặp phiếu chờ duyệt** rồi tất
> toán qua `payment_method='CT'`; move-out cấn trừ nợ bằng `CT` + sổ ảo "Cấn trừ nội bộ" ngoài
> KQKD). Xem **[16 — Thanh lý hợp đồng (deep-dive)](16-thanh-ly-hop-dong.md)** cho dòng tiền &
> sơ đồ đầy đủ, đã đối chiếu định nghĩa hàm LIVE.
>
...
Đặc điểm:
- **Lọc & phân trang client-side**: lọc theo vòng đời (Đang ở / Thanh lý / Tất cả), stat card (Sắp hết hạn / Quá hạn / Đã thanh lý), tìm kiếm (mã HĐ / tên khách đại diện / SĐT / tên phòng), **toà nhà = `BuildingMultiSelect`** (chọn nhiều toà, nhóm theo khu vực — [ContractListFilters](../../src/components/contracts/ContractListFilters.tsx), thay cặp Khu vực → Toà cũ; lọc `buildingIds.includes(contract.room?.building_id)`), phòng (gộp theo TÊN, dropdown thu hẹp theo các toà đang chọn), tháng (overlap với kỳ HĐ). Sắp xếp gom theo toà rồi tên phòng.
- **Mặc định theo user**: nếu `full_name` là `joey`/`nathan` → tự **chọn sẵn toàn bộ toà** thuộc khu vực trùng tên (`setBuildingIds(...)` 1 lần khi mount — snapshot, không khoá).
...
>
> ⚠️ **Dialog còn lệch duy nhất**: [RegisterMoveOutDialog](../../src/components/contracts/RegisterMoveOutDialog.tsx) (Đăng ký chuyển đi) — **ý đồ**: UPDATE `expected_move_out_date` + **APPEND** `[Đăng ký chuyển đi]: ...` vào notes, nhưng vẫn gọi `mutate({ id, expected_move_out_date, notes })` **phẳng** trong khi `useUpdateContract` nhận `{ id, updates }` → `updates = undefined` → `.update(undefined)` lỗi PostgREST, **không ghi được gì**. Trang danh sách dùng `MoveOutDialog` → `useRegisterMoveOut` (hoạt động, nhưng có thể ghi đè trắng notes — §5.1). Chuyển phòng / Nhượng HĐ dùng `TransferRoomDialog` / `TransferContractDialog` (RPC `transfer_room` / `transfer_contract`) như danh sách.
