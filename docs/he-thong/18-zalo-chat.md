...
- **Áp dụng cho repo NÀY**: [ZALO-WORKER-SETUP.md](docs/zalo/ZALO-WORKER-SETUP.md) (hợp đồng bảng + vận hành worker — khớp code hiện tại) và [IMPLEMENTATION-STATUS.md](docs/zalo/IMPLEMENTATION-STATUS.md) (bản đồ tính năng, **chốt 2026-06-26 — đã lỗi thời một phần**: reaction/thu hồi/tải tin cũ/nhãn/broadcast/video ghi "còn thiếu" nhưng nay đã build, xem 7.1).
- **Tham khảo ý tưởng từ dự án KHÁC (n2store "Web 2.0" — Render + SSE + bảng `web2_zalo_*`), KHÔNG mô tả hệ này**: [ZALO-INTEGRATION.md](docs/zalo/ZALO-INTEGRATION.md), [ZALO-CHAT-BUILD-SPEC.md](docs/zalo/ZALO-CHAT-BUILD-SPEC.md), [ZALO-CHAT-IMPLEMENTATION-GUIDE.md](docs/zalo/ZALO-CHAT-IMPLEMENTATION-GUIDE.md), [ZALO-REBUILD-PLAN.md](docs/zalo/ZALO-REBUILD-PLAN.md) (plan rebuild 3-pane của **web2/zalo bên n2store**, không phải kế hoạch của trang `/chat-zalo` này), [SSE-REALTIME.md](docs/zalo/SSE-REALTIME.md), [MODAL-ANTI-LAG.md](docs/zalo/MODAL-ANTI-LAG.md), [UI-FIRST.md](docs/zalo/UI-FIRST.md). Repo này Supabase-native (Realtime thay SSE, RLS + RPC thay route Render) — đọc các file đó để hiểu bài học zca-js (kick 3000/3003, cookie ~7 ngày, feature matrix 148 method) nhưng đừng bê kiến trúc.
