...
`settings`, `document_templates`, `signature_templates`, `code_sequences`, `hotlines`, subscription tables đều gắn trigger `update_updated_at_column()` / `set_*_updated_at` để tự cập nhật `updated_at` mỗi lần UPDATE.
