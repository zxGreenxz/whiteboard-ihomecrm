...
- **Cơ cấu BĐS + danh mục** ([02](02-co-cau-toa-nha-phong-dich-vu.md), [14](14-cai-dat-danh-muc-tai-lieu.md)): phải có **khu vực → toà → tầng → phòng** (`areas → buildings → floors → rooms`) và **dịch vụ/định mức** (`services`, `building_services`, `service_quotas`). `code_sequences` + `generate_code` / `generate_next_code` là engine sinh mã **mồ côi** (FE/trigger không gọi — số HĐ/hoá đơn sinh bằng trigger riêng, xem [14] §4.5). `settings` (JSONB key-value) trên danh nghĩa là **công tắc hành vi** (`invoice_auto_approve`, `invoice_payment_deadline_days`, `contract_e_signing_enabled`…) nhưng ⚠️ hiện **chỉ `payment_auto_approve` có consumer thật** — 19/20 key là "cấu hình ma" ([14] §5.1). `seed_default_settings(p_user_id)` gieo ~20 key mặc định.
...
stateDiagram-v2
    ...
    note right of CONVERTED
        ...
        ⚠ BUG: nút "Tạo HĐ" set CONVERTED TRƯỚC rồi navigate
        /contracts/create (route KHÔNG tồn tại) → phiếu kẹt
        CONVERTED không HĐ + phòng bị nhả RESERVED dù chưa ký
        (xem [04] §5.4).
    end note
    CONVERTED --> [*]
    REFUNDED --> [*]
