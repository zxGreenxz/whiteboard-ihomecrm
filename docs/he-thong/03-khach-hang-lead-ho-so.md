...
- **Địa chỉ cũ và tìm nhanh:** `CustomerForm` và popup `CreateCustomerDialog` (mở từ Hợp đồng hoặc Khách hàng) cùng dùng `AddressCascadingDropdowns`, danh mục tỉnh → quận/huyện → phường/xã từ API địa chỉ cũ. Đổi cấp trên xóa các lựa chọn cấp dưới. Dropdown địa chỉ, giới tính, nhóm khách và loại phương tiện có ô tìm kiếm không phân biệt dấu. Lỗi tải danh mục phải hiển thị thao tác tải lại, không coi lỗi là danh sách rỗng hợp lệ.
- **Địa chỉ hành chính mới:** `CustomerAdministrativeAddress` hiển thị khung riêng dưới địa chỉ cũ. Mặc định lấy địa chỉ thường trú; nếu phần địa chỉ theo tỉnh/quận/phường khác, có bộ chọn nguồn. Bấm **Tra địa chỉ mới** gọi `POST /api/customer-address-conversion` với JWT phiên hiện tại và duy nhất chuỗi địa chỉ. Server xác thực JWT bằng GoTrue, dùng `GOONG_API_KEY` ở phía server, chỉ gọi Goong V2 với `has_deprecated_administrative_unit=true`. Hiển thị tỉnh/phường mới, chuỗi địa chỉ, và địa giới cũ để đối chiếu. Nhiều ứng viên được hiển thị riêng; không lấy V1 thay thế rồi gắn nhãn mới. Kết quả không ghi vào hồ sơ; sửa nguồn hoặc đóng form hủy tác vụ cũ. Deadline UI 15 giây bao gồm chờ phiên, server xác thực 4 giây và Goong 8 giây. Giới hạn 30 lần/phút/người dùng trong từng server instance là chống bấm lặp, không phải hạn mức tổng của tài khoản Goong.
