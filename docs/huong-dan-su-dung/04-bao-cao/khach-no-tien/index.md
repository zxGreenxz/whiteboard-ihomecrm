---
title: "Khách nợ tiền (đã chuyển)"
description: "Trang cũ được giữ làm biển chỉ đường tới luồng Thu tiền hiện hành."
kind: redirect
lifecycle: current
sidebar: false
routes: ["/reports/finance/customer-debt"]
redirect_to: "/thu-tien"
permissions: []
viewport: desktop
audience: [chu-nha, ke-toan, quan-ly-toa]
captured:
  date: "2026-07-20"
  account: production
status: published
---

# Khách nợ tiền (đã chuyển)

Trang báo cáo riêng **Khách nợ tiền** đã được gỡ khỏi trung tâm báo cáo. URL cũ `/reports/finance/customer-debt` hiện chuyển về **Thu tiền** (`/thu-tien`).

## Nơi làm việc hiện hành

Mở [Thu tiền](/01-bat-dau/quy-trinh-thu-tien/) để xem số phải thu, trạng thái kỳ và thực hiện thu theo hóa đơn. Đây là nguồn canonical cho việc xử lý công nợ.

Nếu liên kết cũ còn xuất hiện trong bookmark, chỉ cần mở lại URL; ứng dụng sẽ tự chuyển hướng.
