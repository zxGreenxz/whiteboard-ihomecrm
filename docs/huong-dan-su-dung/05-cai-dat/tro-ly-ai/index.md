...
2. Chọn model đang hiển thị trong danh sách. Khả dụng/chất lượng của model phụ thuộc provider và có thể thay đổi; nếu model lỗi, chọn model khác hoặc báo quản trị viên.
3. Viết yêu cầu có phạm vi rõ: tòa, tháng, trạng thái hoặc tên khách.
4. Kiểm tra link/kết quả trước khi hành động.

## Điều khiển giao diện

Chế độ UI-control chỉ hoạt động khi tài khoản được cấp riêng. Copilot có thể điều hướng, lọc và **điền dữ liệu vào form** trên các màn hình được cho phép, nhưng không tự bấm **Lưu**, **Xác nhận**, **Submit** hoặc hành động nguy hiểm. Mỗi lệnh là một phiên độc lập và vẫn chịu gate quyền của tài khoản.
...
Sau khi xác nhận, hệ thống hiện ghi audit, phiếu và hạng mục qua nhiều bước riêng, không phải một transaction duy nhất. Nếu có lỗi, hãy mở màn Thu chi kiểm tra phiếu/audit đã được tạo hay chưa trước khi yêu cầu lại để tránh trùng hoặc bỏ sót hạng mục.

::: warning
AI không thay thế người duyệt, không vượt RLS và không nên được cung cấp mật khẩu, token, CCCD hoặc thông tin ngân hàng không cần thiết. Cơ chế xác nhận hiện vẫn dựa vào cờ do model truyền; nếu Copilot tạo phiếu khi bạn chưa đồng ý rõ, dừng sử dụng write tool và báo quản trị viên kiểm tra audit.
:::
