...
- Quyền **Gửi** (`send`) để nhắn tin, thả cảm xúc, thu hồi và gửi hàng loạt; quyền **Quản lý tự động hoá** (`manage_automation`) để bật/tắt hai công tắc tự động hoá.
...
**Bước 1**: Mở **Kênh chat** => **Chat Zalo**. Màn hình hiện workspace 3 cột: **danh sách hội thoại** (bên trái), **khung chat** (giữa) và **panel thông tin** (phải). Trên điện thoại, ba cột này gộp lại thành từng màn: danh sách và khung chat chuyển qua lại, còn panel thông tin trượt ra từ phải.

![Workspace Chat Zalo 3 cột: danh sách hội thoại bên trái, khung chat ở giữa và panel thông tin bên phải](./images/buoc-01-man-hinh.webp)
...
Bật/tắt hai công tắc **Gửi ảnh phòng trống** / **Tự động trả lời** hiện chỉ **lưu trạng thái** — chưa có bộ chạy phía sau nên bật lên cũng chưa tự gửi gì. Con số "lượt tự động" ở chân danh sách là số minh hoạ. Hãy coi đây là tính năng đang chuẩn bị.
:::
...
<SandboxTry account="demo.sale" app-path="/chat-zalo" app-label="Mở màn Chat Zalo" view-only>

Đăng nhập bằng tài khoản `demo.sale` và mở màn **Chat Zalo**. Đây là bài **chỉ xem giao diện** — sandbox demo **chưa kết nối tài khoản Zalo thật** nên danh sách hội thoại sẽ trống; bạn cần kết nối một tài khoản Zalo thật (quét QR) mới có tin để nhắn. Bạn nên nhìn thấy:

- Bố cục **workspace 3 cột**: danh sách hội thoại bên trái, khung chat ở giữa, panel thông tin bên phải.
...
- [Phân quyền](/05-cai-dat/phan-quyen/) — cấp các quyền **Chat Zalo** (Xem / Gửi / Quản lý tự động hoá) cho nhân viên.
