...
The repository currently contains multiple SQL files that share the same leading migration version. The full-reset harness therefore enumerates every root `supabase/migrations/*.sql` file in bytewise UTF-8 order by its complete filename, records the original version and raw SHA-256 plus duplicate-version-group cardinality, and copies the exact unchanged SQL bytes into a safe disposable directory under monotonically increasing synthetic versions. Synthetic versions exist only inside that disposable reset project; production filenames, Git blobs, migration statements, and remote history are never renamed or rewritten. Any SQL failure in the resulting complete chain is a hard gate failure and must be corrected through separately reviewed forward-compatible repository work, never hidden by skipping, reordering, or mutating a source migration in the harness.
