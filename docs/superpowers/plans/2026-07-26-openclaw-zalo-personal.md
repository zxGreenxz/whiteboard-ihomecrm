...
- Upstream locks are immutable: npm integrity `sha512-klg0BOOTDv4xUykgA/pTZDsRrI9dzagq23OlPupCLrFijDOebPxGYaYdWDSPy4zBJAWjjnSrgyCB+5OuCMvZGw==`, shasum `ddd42ffa571e93a881ca5c95203eb7a49713f6c6`, git head `2d2ddc43d0dcf71f31283d780f9fe9ff4cc04fe4`, OCI index digest `sha256:6a31d44b2944e7adcd2b582bf6fb463111264ebca97a0201795b799135bd102c`, `linux/amd64` digest `sha256:165b4992f1b4b74ffdd7a02c887ba006f9f5dc951eca420eef573a8b233b543f`, and reference `linux/arm64` digest `sha256:38b611f494cb32e15aaf456d54c6b6be55db9098c90632aed0bfad4a70009707`.
- The vendor root is `services/openclaw-zalo-cell/vendor/zalouser-bridge/`. The published npm tarball is verified but is not treated as a TypeScript source/test distribution; the repo commits the exact `extensions/zalouser` source snapshot at the pinned git head, applies the committed patch series and bridge overlay, preserves licenses/notices, and emits the exact internal tgz.
- Docker installs only the verified internal tgz and never resolves registry ZaloUser during image build/runtime. Upstream and fork packages may not coexist. Evidence records upstream SRI, patch-series SHA-256, built-tgz SHA-256, installed-package digest/list, and deterministic architecture-specific image digest.
...
- `services/openclaw-zalo-cell/vendor/zalouser-bridge/upstream/{LICENSE.openclaw,package/**}` — `package/**` is the complete exact snapshot of every blob under `extensions/zalouser` at the pinned git head, including the top-level public entrypoints, `README.md`, `npm-shrinkwrap.json`, `tsconfig.json`, `test-api.ts`, manifests, and `src/**`.
- `services/openclaw-zalo-cell/vendor/zalouser-bridge/licenses/**`
...
- Create: `services/openclaw-zalo-cell/vendor/zalouser-bridge/upstream/{LICENSE.openclaw,package/**}` (the complete exact `extensions/zalouser` snapshot, not only `src/**`)
- Create: `services/openclaw-zalo-cell/vendor/zalouser-bridge/licenses/**`
...
Assert `UPSTREAM.json` contains the exact OpenClaw/ZaloUser version, npm SRI, shasum, git head, index digest, `linux/amd64` digest, reference `linux/arm64` digest, and a manifest/hash of every committed `upstream/package/**` source-snapshot file from `extensions/zalouser` at that git head. The manifest must include the top-level public entrypoints (`index.ts`, `api.ts`, `runtime-api.ts`, `channel-plugin-api.ts`, `contract-api.ts`, `doctor-contract-api.ts`, `secret-contract-api.ts`, `setup-entry.ts`, `setup-plugin-api.ts`) as well as the package-owned metadata and `src/**`. `SHA512SUMS` binds the verified registry tarball. The harness proves that package-owned entries in the published tarball outside bundled `package/node_modules/**` contain no TypeScript source/tests and that the tarball is never used as the fork source tree; bundled dependency contents are inventoried separately. `verify-upstream.mjs` compares the complete committed snapshot against the exact git commit and fails on an extra, missing, or changed source file.
...
`verify-upstream.mjs` downloads the exact npm tarball, verifies SRI/shasum/`gitHead`, proves that package-owned entries outside bundled `package/node_modules/**` lack TypeScript source/tests, inventories bundled dependency contents separately, and compares the complete committed `upstream/package/**` snapshot with every blob under `extensions/zalouser` at `2d2ddc43d0dcf71f31283d780f9fe9ff4cc04fe4`. `prepare.mjs` copies this verified committed snapshot into ignored `.work/` and applies `patches/series`; the build never reconstructs source from the npm tarball.

Apply the three committed patches plus `src/bridge/**` overlay deterministically. Preserve package name/version and plugin ID/channel. Patch the top-level `index.ts` entrypoint to register the private `zalouser.bridge.send` RPC and patch `monitor.ts`/`zalo-js.ts` for the awaited internal inbound listener. Patch `send.ts`, including link/reaction branches, plus `channel.adapters.ts` and `tool.ts` for private RPC context enforcement; classify typing/seen/delivery receipts in the third patch. Disable built-in OpenClaw/ZaloUser replies and pairing notifications with content. Run upstream-compatible tests plus the fork tests, compile with the fixed source epoch, use `npm pack --ignore-scripts`, and verify the tgz contains only runtime output, package metadata, root MIT license, bundled dependency licenses, and third-party notices--no package-owned `.ts`, source maps containing source content, fixtures, snapshots, or tests.
...
Expected: every command exits 0 and an injected native nonzero prevents every later sentinel. Evidence includes the exact clean-context manifest, upstream SRI/shasum/git head, patch-series SHA-256, built-tgz SHA-256, installed fork digest/list, helper SHA-256, pinned builder/exporter contract, source epoch/rewritten mtimes, both OCI archive hashes, matching manifest/config/layer digests, promoted archive hash/path, and deterministic `linux/amd64` image digest. Tasks 3-29 remain blocked until this gate proves the promoted archive exists with the recorded hash and the artifact-handoff contract consumes that proof; Task 29 later reproduces and promotes fresh bytes from the independently reviewed SHA for the actual deployment bundle.
