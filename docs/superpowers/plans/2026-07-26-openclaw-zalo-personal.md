...
- Internal `THIRD_PARTY_NOTICES.md` contains the upstream root notice verbatim plus the exact `38` package name/version/SPDX inventory, carrier paths, the pako exception, and the Spark-md5 selection. The internal tgz uses exact archive paths `package/LICENSE`, `package/THIRD_PARTY_NOTICES.md`, and `package/licenses/**`; it preserves runtime JS/JSON and exact legal carriers while pruning/rejecting dependency source, tests, fixtures, snapshots, docs, examples, all source maps, and inline `sourceMappingURL`. Verification fails on missing/extra/changed carriers, dependency-root mismatch, new notice files, path/case collisions, traversal, or symlinks.
- Docker installs only the verified internal tgz and never resolves registry ZaloUser during image build/runtime. Upstream and fork packages may not coexist. Evidence records the fixed tarball URL/size/entry count, SRI/SHA-1/attestation/SLSA/source locks, root license/notice hashes, exact 38-package/39-carrier compliance inventory, patch-series SHA-256, built-tgz SHA-256, installed-package digest/list, and deterministic architecture-specific image digest.
...
- Task 2 is a positive global architecture gate. No database, Edge, runtime, frontend, infrastructure, or rollout task may proceed until the vendored fork gate exits 0 with verified upstream/source locks, patch/tgz hashes, full inbound listener ordering, every outbound choke point fail-closed, control-traffic classification, deterministic `linux/amd64` image evidence, and internal-only installation.
- The owner approved the vendored integrity-pinned fork on 2026-07-27. Tasks 3-29 are open only after Task 2 passes; there is no alternate external adapter, owner wait checkpoint, or separate hook package in this plan.
...
Positive vendored-fork architecture gate
  Task 1 -> Task 2 (must PASS) -> Tasks 3-29
...
Assert `UPSTREAM.json` contains the exact OpenClaw/ZaloUser version; fixed tarball URL `https://registry.npmjs.org/@openclaw/zalouser/-/zalouser-2026.7.1.tgz`; byte size `2341459`; exactly `3169` regular `package/` entries; npm SRI; shasum/SHA-1; git head; npm attestation subject `pkg:npm/%40openclaw/zalouser@2026.7.1`; SLSA resolved commit equal to the git head where metadata is available; index digest; `linux/amd64` digest; reference `linux/arm64` digest; and a manifest/hash of the exact 75 committed `upstream/package/**` source-snapshot blobs from `extensions/zalouser` at that git head. The source manifest must include the top-level public entrypoints (`index.ts`, `api.ts`, `runtime-api.ts`, `channel-plugin-api.ts`, `contract-api.ts`, `doctor-contract-api.ts`, `secret-contract-api.ts`, `setup-entry.ts`, `setup-plugin-api.ts`) as well as package-owned metadata and `src/**`. `SHA512SUMS` binds the verified registry tarball. The tar harness requires exactly `25` package-owned regular files outside `package/node_modules/**`, `3144` bundled regular files, and exactly `38` dependency package roots. It proves the package-owned set has no TypeScript, tests, source maps, license, or notice files and that the tarball is never used as the fork source tree. `verify-upstream.mjs` compares the 75-blob committed snapshot against the exact git commit and fails on an extra, missing, or changed source blob.

`FORK.json` fixes package name/version `@openclaw/zalouser@2026.7.1`, plugin ID/channel `zalouser`, the ordered patch list, patch-series SHA-256, bridge-overlay manifest/SHA-256, built-tgz SHA-256, source epoch `1785062400`, and the internal artifact path. Compute patch-series SHA-256 from UTF-8 bytes for `"ihome-zalouser-patch-series-v1\\0"` followed, in `patches/series` order, by each relative path, NUL, raw patch bytes, and NUL. Compute the overlay SHA-256 from `"ihome-zalouser-bridge-overlay-v1\\0"` followed by each ordered `src/bridge/**` relative path, NUL, raw bytes, and NUL. Reject missing/reordered/unlisted patch or overlay files, changed source snapshot/overlay bytes, wrong package metadata, extra tgz files, TypeScript/tests in package-owned packed output, or an unrecorded root/dependency license/notice.

Add compliance fixtures that byte-compare upstream root `LICENSE` to SHA-256 `73571b25326281d369087f469842c02444fe39faaecebda4d82ed21ff3a1c29d` and root `THIRD_PARTY_NOTICES.md` to SHA-256 `c84200f7a9bb8b3abc8563520433316716a9eb83915cfe7c3063d5e6fce5e7ca`; require the notice to be copied verbatim as `upstream/THIRD_PARTY_NOTICES.openclaw.md`. Lock the exact 38 package name/version/SPDX inventory and all 39 bundled dependency carrier paths/hashes. Require `pako@2.2.0/LICENSE` and `pako@2.2.0/lib/zlib/README`; explicitly select the included Spark-md5 WTFPL branch and reject any network fetch or synthesized absent `LICENSE2`. Internal `THIRD_PARTY_NOTICES.md` must contain the upstream notice verbatim plus the exact inventory, carrier paths, pako exception, and Spark selection. Every carrier must exist at `licenses/<package>@<version>/<original-path>` with exact bytes. The verifier rejects missing, extra, or changed carriers; dependency-root mismatch; any new notice file; path/case collision; traversal; or symlink entry. The packed compliance entries must be exactly `package/LICENSE`, `package/THIRD_PARTY_NOTICES.md`, and `package/licenses/**`; tests retain required runtime JS/JSON but reject dependency source/tests/fixtures/snapshots/docs/examples, every source map, and every inline `sourceMappingURL`.
...
- [ ] **Step 3: Implement the reproducible source, patch, test, and pack pipeline**

`verify-upstream.mjs` downloads only `https://registry.npmjs.org/@openclaw/zalouser/-/zalouser-2026.7.1.tgz`; rejects redirects or a changed resolved URL; verifies byte size `2341459`, `3169` regular `package/` entries, SRI, SHA-1, `gitHead`, attestation subject, and SLSA resolved commit where npm metadata exposes them; and rejects traversal, symlinks, duplicate/case-colliding paths, or non-regular inventory entries. It requires the exact `25` package-owned/`3144` bundled split and exact `38` dependency roots, proves the package-owned set lacks TypeScript/tests/source maps/license/notice, and compares the committed `upstream/package/**` snapshot with exactly 75 blobs under `extensions/zalouser` at `2d2ddc43d0dcf71f31283d780f9fe9ff4cc04fe4`. `prepare.mjs` copies this verified committed snapshot into ignored `.work/` and applies `patches/series`; the build never reconstructs source from the npm tarball.

Copy upstream root `LICENSE` exactly to `upstream/LICENSE.openclaw` and internal `LICENSE`; copy upstream root `THIRD_PARTY_NOTICES.md` verbatim to `upstream/THIRD_PARTY_NOTICES.openclaw.md`. Generate internal `THIRD_PARTY_NOTICES.md` deterministically from that verbatim notice plus the locked exact 38 package name/version/SPDX inventory, all carrier paths, the `pako@2.2.0` two-carrier exception, and the explicit Spark-md5 included-WTFPL selection. Copy all 39 carriers byte-for-byte to `licenses/<package>@<version>/<original-path>`; never fetch or synthesize Spark `LICENSE2`. `verify-upstream.mjs` and `verify-artifact.mjs` reject a missing/extra/changed carrier, package-root mismatch, new notice file, path/case collision, traversal, symlink, or any hash mismatch.

Apply the three committed patches plus `src/bridge/**` overlay deterministically. Preserve package name/version and plugin ID/channel. Patch the top-level `index.ts` entrypoint to register the private `zalouser.bridge.send` RPC and patch `monitor.ts`/`zalo-js.ts` for the awaited internal inbound listener. Patch `send.ts`, including link/reaction branches, plus `channel.adapters.ts` and `tool.ts` for private RPC context enforcement; classify typing/seen/delivery receipts in the third patch. Disable built-in OpenClaw/ZaloUser replies and pairing notifications with content. Run upstream-compatible tests plus the fork tests, compile with the fixed source epoch, use `npm pack --ignore-scripts`, and verify the tgz retains required runtime JS/JSON plus exact legal carriers at `package/LICENSE`, `package/THIRD_PARTY_NOTICES.md`, and `package/licenses/**`. Prune and reject dependency source, tests, fixtures, snapshots, docs, examples, all source maps, every inline `sourceMappingURL`, extra license/notice files, traversal, symlinks, and case-colliding paths.

Build twice from the committed verified source snapshot into separate `.work/` directories and require byte-identical tgz SHA-256. Write the actual patch-series, bridge-overlay manifest/SHA-256, and tgz hashes to `FORK.json`; re-run `verify-artifact.mjs` against those values. The committed artifact is `artifacts/openclaw-zalouser-2026.7.1.tgz` and no other ZaloUser tgz is accepted.

- [ ] **Step 4: Build the immutable cell and pass the positive fork gate**
...
The Dockerfile validates the exact build argument and exposes it only to deterministic build/pack commands such as the session-crypto build; it must not declare `ENV SOURCE_DATE_EPOCH` or leave that value in the runtime environment. `.dockerignore` is deny-by-default and is validated as part of the image lock. To avoid an impossible self-hash, `image-lock.json` lists the exact normalized path and SHA-256 of every allowed context input **except itself**. During a baseline-producing gate (initial Task 2 or the intentional Task 19 entrypoint/image update), the helper hashes the exact working-tree lock bytes that the tests just validated and writes the new tracked baseline evidence. Every verification-only rerun after the Task 19 baseline commit requires the lock bytes to equal the selected reviewed Git blob. The helper computes a separate lock SHA-256 and a domain-separated context-root hash over that lock hash plus the ordered path/content hashes from the lock. It then assembles a fresh temp context and fails unless the lock file and every listed path/hash are exact and the context contains no extra path, symlink/reparse point, `node_modules`, `.work`, `.state`, `.release`, `build-evidence.json`, dist/coverage/log output, env/secret file, or untracked input. Thus host dependencies produced by the preceding `npm ci` can never enter either build. `install-vendored-zalouser.sh` verifies the local tgz hash from `FORK.json`, installs only that file, performs no npm metadata/tarball request, and fails unless `openclaw plugins list --json` shows exactly one approved `zalouser` package with the expected package/version/digest. The template contains `channels.zalouser`, `session.dmScope`, private listener/RPC configuration, built-in reply disablement, explicit tool/method deny rules, health binding, and render-time environment variable names; it contains no token, cookie, IMEI, phone number, user/group ID, R2 key, Supabase key, or model key.
...
The helper invokes `verify-image-lock.mjs --oci-a ... --oci-b ...` before cleanup. The verifier checks the base/index/architecture digests, fork lock, patch hash, tgz hash, installed package/list, absence of registry/upstream duplicates, private RPC registration, denied generic business-send surfaces, equal OCI manifest/config/layer digests, layer file mtimes equal to source epoch `1785062400`, and installed package metadata carrying that epoch. It atomically copies exactly gate A to explicit gitignored `-ReleaseArtifactPath services/openclaw-zalo-cell/.release/openclaw-zalo-cell-linux-amd64.oci.tar`, re-hashes/re-verifies the promoted bytes, and records reviewed lock SHA-256, context-root hash, buildx/BuildKit image/version, every exporter option, both OCI hashes, manifest/config/layers, deterministic image digest, package epoch, promoted archive path/hash, and helper SHA-256. Task 2 writes the reviewed baseline to tracked `build-evidence.json`; every later rerun writes to a release-scoped evidence file under `.release/` and compares all immutable contract/digest fields with that baseline before use. The promoted archive and later release-scoped evidence are ignored by Git but verified by exact path/hash and are not deleted before their evidence/bundle handoff; gate B and temp context are cleaned safely. Missing/tampered promoted bytes or any lock mismatch fails the gate.
...
Expected: every command exits 0 and an injected native nonzero prevents every later sentinel. Evidence includes the exact clean-context manifest; fixed tarball URL, `2341459` bytes, `3169` entries, SRI/SHA-1, attestation subject, SLSA resolved commit, exact 75-blob source manifest; root license/notice hashes; exact `25`/`3144` split, 38 dependency roots and 39 carrier hashes/paths; verbatim upstream notice, exact SPDX inventory, pako exception, Spark-md5 WTFPL selection; exact archive paths `package/LICENSE`, `package/THIRD_PARTY_NOTICES.md`, `package/licenses/**`; retained runtime JS/JSON; patch-series SHA-256; built-tgz SHA-256; installed fork digest/list; helper SHA-256; pinned builder/exporter contract; source epoch/rewritten mtimes; both OCI archive hashes; matching manifest/config/layer digests; promoted archive hash/path; and deterministic `linux/amd64` image digest. Negative fixtures prove missing/extra/changed carrier, dependency-root mismatch, new notice file, path/case collision, traversal, symlink, absent-`LICENSE2` fetch, dependency source/test/fixture/snapshot/docs/example, source map, or inline `sourceMappingURL` is rejected. Tasks 3-29 remain blocked until this gate proves the promoted archive exists with the recorded hash and the artifact-handoff contract consumes that proof; Task 29 later reproduces and promotes fresh bytes from the independently reviewed SHA for the actual deployment bundle.

- [ ] **Step 5: Commit the positive gate**
...
git add services/openclaw-zalo-cell/Dockerfile services/openclaw-zalo-cell/.dockerignore services/openclaw-zalo-cell/image-lock.json services/openclaw-zalo-cell/build-evidence.json services/openclaw-zalo-cell/README.md services/openclaw-zalo-cell/config services/openclaw-zalo-cell/scripts services/openclaw-zalo-cell/vendor services/openclaw-zalo-cell/session-crypto services/openclaw-zalo-bridge/test/upstream-contract.test.ts
if (Test-Path -LiteralPath 'services/openclaw-zalo-cell/.release/openclaw-zalo-cell-linux-amd64.oci.tar') { throw 'Release archive cleanup failed' }
if (git diff --cached --name-only | Select-String '(^|/)(node_modules|\.work|\.state|\.release|dist|coverage)(/|$)|\.log$|\.env') { throw 'Generated or secret residue was staged' }
git commit -m "feat(openclaw-zalo): vendor fork zalouser da pin integrity" -m "Co-Authored-By: Codex <noreply@openai.com>"
```

The Task 2 test/evidence consumer must verify the promoted archive's exact bytes and recorded hash before the exact cleanup command above removes it. The committed evidence preserves that completed gate; the production archive is rebuilt from the later independently reviewed SHA in Task 29 and must then survive through bundle creation and checked transfer. Contract tests inspect `git status --porcelain --untracked-files=all` and the staged path allowlist, rejecting any generated residue beyond intended source/evidence files. The ignored `.work/` path and host `node_modules` may exist for tests but can neither enter the clean context nor be staged.
...
The bundle creator exports source/config only from reviewed Git blobs, adds the exact promoted OCI archive plus content-free evidence/transfer manifest, reopens the final tar, and verifies archive hash/image digest before recording the bundle SHA-256. Build the other runtime images for `linux/amd64`; verify Task 2's base digest `sha256:165b4992f1b4b74ffdd7a02c887ba006f9f5dc951eca420eef573a8b233b543f`, upstream SRI/shasum/git head and committed source snapshot, patch-series SHA-256, built-tgz SHA-256, installed fork digest/list, deterministic cell OCI/image evidence, and architecture-specific bridge, maintenance, and egress-broker image digests. Reject tags, local mutable images, wrong architecture, registry ZaloUser resolution, duplicate packages, a vendor/installed directory hidden by a mount, missing/mismatched context/builder/exporter/image evidence, or a bundle created before the promoted archive.
