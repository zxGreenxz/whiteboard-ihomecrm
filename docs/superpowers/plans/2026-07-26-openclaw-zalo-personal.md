...
- Immutable Task 2 checkpoints are ordered: commit `M` contains the manifest/self-contained raw provenance inputs and is independently reviewed only after it exists, before any notice/carrier/artifact rendering; commit `R` is the final source/artifact/lock/tooling commit after every tracked vendor output, `FORK.json`, and `artifactMembers` is frozen, then independently reviewed at its exact SHA. For the qualifying positive/release gate after `R` is reviewed, preflight requires `HEAD` to equal exact `R`, `git status --porcelain=v1 --untracked-files=all` to be empty, the index to be empty, and no merge/rebase in progress before any dependency install or executable verifier/build; commands then export and execute only exact `R` bytes without tracked-file mutation. Pre-`R` TDD development is explicitly non-qualifying as described in the Task 2 steps. Successful OCI/evidence output creates evidence-only child commit `E` whose diff is exactly `services/openclaw-zalo-cell/build-evidence.json` and records/rechecks the exact raw-provenance path/size/SHA-256 set from `M`; only independent review of exact `E` unlocks Tasks 3-29. No pre-commit or plan-only `reviewed commit` claim is a gate.
