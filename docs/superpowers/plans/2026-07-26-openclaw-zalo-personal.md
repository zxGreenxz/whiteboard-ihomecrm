...
- `services/openclaw-zalo-cell/vendor/zalouser-bridge/upstream/{LICENSE.openclaw,package/package.json,package/openclaw.plugin.json,package/src/**}`
...
- Create: `services/openclaw-zalo-cell/vendor/zalouser-bridge/upstream/{LICENSE.openclaw,package/package.json,package/openclaw.plugin.json,package/src/**}`
...
`verify-upstream.mjs` downloads the exact npm tarball, verifies SRI/shasum/`gitHead`, proves that the published package lacks TypeScript source/tests, and compares the committed `upstream/package/**` snapshot with `extensions/zalouser` at `2d2ddc43d0dcf71f31283d780f9fe9ff4cc04fe4`. `prepare.mjs` copies this verified committed snapshot into ignored `.work/` and applies `patches/series`; the build never reconstructs source from the npm tarball.

Apply the three committed patches plus `src/bridge/**` overlay deterministically. Preserve package name/version and plugin ID/channel. Patch `monitor.ts`/`zalo-js.ts` for the awaited internal inbound listener; patch `send.ts`, including link/reaction branches, plus `channel.adapters.ts` and `tool.ts` for private RPC context enforcement; classify typing/seen/delivery receipts in the third patch. Disable built-in OpenClaw/ZaloUser replies and pairing notifications with content. Run upstream-compatible tests plus the fork tests, compile with the fixed source epoch, use `npm pack --ignore-scripts`, and verify the tgz contains only runtime output, package metadata, root MIT license, bundled dependency licenses, and third-party notices--no `.ts`, source maps containing source content, fixtures, snapshots, or tests.

Build twice from the committed verified source snapshot into separate `.work/` directories and require byte-identical tgz SHA-256. Write the actual patch-series and tgz hashes to `FORK.json`; re-run `verify-artifact.mjs` against those values. The committed artifact is `artifacts/openclaw-zalouser-2026.7.1.tgz` and no other ZaloUser tgz is accepted.
