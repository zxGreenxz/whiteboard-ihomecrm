...
Add artifact behavior tests: install the exact tgz into a clean offline temp project without registry resolution; load it through the pinned OpenClaw plugin loader and assert one `zalouser` plugin/channel plus the private RPC; run the upstream-compatible public API/config/adapter suite against the installed package; and run a differential corpus against the prepared build tree versus unpacked/installed tgz, allowing only the explicit patch-series behavior changes. Every runtime reachability allowlist member must be present and exercised or loaded; structural bans cannot be waived by the allowlist, category pruning cannot delete an allowlisted member, and no artifact member absent from `FORK.json.artifactMembers` may install or load.
...
Expected: every command exits 0 and an injected native nonzero prevents every later sentinel. This is a qualifying positive gate only when online attestation and SLSA verification succeeded in the same run; unavailable metadata/network or offline mode cannot write/update `FORK.json`, evidence, artifacts, or gate state. Evidence includes the exact redirect chain/final URL, `2341459` bytes, `3169` entries, SRI/SHA-1, attestation subject, SLSA resolved commit, exact 75-blob source manifest; reviewed `licenses/manifest.json` SHA/schema with exact 38 package selections and 39 source/size/SHA-256/output carrier records; root license/notice hashes; exact `25`/`3144` split; verbatim upstream notice, pako exception, Spark-md5 WTFPL selection; exact archive paths `package/LICENSE`, `package/THIRD_PARTY_NOTICES.md`, `package/licenses/**`; exact sorted `artifactMembers` path/type/mode/size/SHA-256 and runtime reachability allowlist; clean install/load, upstream-compatible and differential runtime results; patch-series SHA-256; built-tgz SHA-256; installed fork digest/list; helper SHA-256; pinned builder/exporter contract; source epoch/rewritten mtimes; both OCI archive hashes; matching manifest/config/layer digests; promoted archive hash/path; and deterministic `linux/amd64` image digest. Negative fixtures prove redirect overflow/downgrade/cross-organization, metadata/network failure, missing/extra/changed carrier or member, dependency-root mismatch, new notice file, path/case collision, traversal, symlink, absent-`LICENSE2` fetch, removed runtime-reachable member, dependency source/test/fixture/snapshot/docs/example, source map, inline `sourceMappingURL`, install/load failure, upstream incompatibility, or unexpected differential behavior is rejected. Tasks 3-29 remain blocked until a fresh independent review approves the license manifest, rendered legal outputs, artifact members, reachability allowlist, behavior evidence, and promoted archive proof; Task 29 later reproduces and promotes fresh bytes from the independently reviewed SHA for the actual deployment bundle.

- [ ] **Step 5: Obtain independent provenance, license, and artifact approval**

Use a fresh read-only `reviewer` agent with `fork_turns="none"`. Give it the exact reviewed commit, pinned npm metadata/attestation/SLSA response, 75-blob source manifest, `licenses/manifest.json`, root compliance hashes, rendered `LICENSE`/`THIRD_PARTY_NOTICES.md`/carrier tree, `FORK.json.artifactMembers`, runtime reachability allowlist, both tgz builds/hashes, install/load/upstream-compatible/differential results, and all negative-fixture results. It must independently recount 38 packages/39 carriers, compare every manifest record and artifact member, verify the Spark/pako choices, and return findings ordered by severity. Any finding, missing input, offline-only provenance, or non-approval leaves pack/image evidence provisional and Tasks 3-29 blocked. After zero findings, rerun `npm --prefix services/openclaw-zalo-cell/vendor/zalouser-bridge run verify` and require the accepted evidence to bind the reviewed commit plus reviewer-approval digest.

- [ ] **Step 6: Commit the positive gate**
