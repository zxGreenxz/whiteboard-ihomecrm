...
- Immutable Task 2 checkpoints are ordered: commit `M` contains the manifest/raw provenance inputs and is independently reviewed only after it exists, before any notice/carrier/artifact rendering; commit `R` is the final source/artifact/lock/tooling commit after every tracked vendor output, `FORK.json`, and `artifactMembers` is frozen, then independently reviewed at its exact SHA; all verification/build commands run strictly from clean `R` without tracked-file mutation; successful OCI/evidence output creates evidence-only child commit `E` whose diff is exactly `services/openclaw-zalo-cell/build-evidence.json`; only independent review of exact `E` unlocks Tasks 3-29. No pre-commit or plan-only `reviewed commit` claim is a gate.
...
Create only the immutable manifest/raw-provenance checkpoint first: `UPSTREAM.json`, `SHA512SUMS`, exact `upstream/LICENSE.openclaw`, exact `upstream/THIRD_PARTY_NOTICES.openclaw.md`, the exact 75-blob `upstream/package/**` snapshot, and `licenses/manifest.json`. Do not render internal `LICENSE`, internal `THIRD_PARTY_NOTICES.md`, `licenses/<package>@<version>/**`, patches/overlay output, `FORK.json`, tgz, image locks, or evidence yet. Commit these exact paths as commit `M`; only after the commit exists may a fresh read-only reviewer inspect exact `M`, independently validate provenance/attestation and the 38-package/39-carrier manifest, and approve it. Record `M=git rev-parse HEAD`; a pre-commit working tree, proposed hash, plan commit, or later descendant cannot be called reviewed `M`.

```powershell
git add services/openclaw-zalo-cell/vendor/zalouser-bridge/UPSTREAM.json services/openclaw-zalo-cell/vendor/zalouser-bridge/SHA512SUMS services/openclaw-zalo-cell/vendor/zalouser-bridge/upstream/LICENSE.openclaw services/openclaw-zalo-cell/vendor/zalouser-bridge/upstream/THIRD_PARTY_NOTICES.openclaw.md services/openclaw-zalo-cell/vendor/zalouser-bridge/upstream/package services/openclaw-zalo-cell/vendor/zalouser-bridge/licenses/manifest.json
git commit -m "chore(openclaw-zalo): khoa provenance manifest M" -m "Co-Authored-By: Codex <noreply@openai.com>"
$M = git rev-parse HEAD
```

Any `M` review finding requires a new commit and a new exact-SHA review before rendering begins.
...
npm --prefix services/openclaw-zalo-cell/session-crypto test
npm --prefix services/openclaw-zalo-cell/session-crypto run typecheck
if ((git rev-parse HEAD) -ne $R) { throw 'HEAD is not reviewed R' }
if (git status --porcelain --untracked-files=no) { throw 'Tracked tree/index is not clean at R' }
& services/openclaw-zalo-cell/scripts/build-reproducible-image.ps1 -Platform 'linux/amd64' -SourceDateEpoch '1785062400' -Dockerfile 'services/openclaw-zalo-cell/Dockerfile' -Context 'services/openclaw-zalo-cell' -EvidencePath 'services/openclaw-zalo-cell/.release/task2-build-evidence.json' -ReleaseArtifactPath 'services/openclaw-zalo-cell/.release/openclaw-zalo-cell-linux-amd64.oci.tar'
if (git status --porcelain --untracked-files=no) { throw 'Verification/build mutated tracked files from R' }
```
...
if (git status --porcelain --untracked-files=no) { throw 'R worktree/index is not clean' }
...
Use a fresh read-only `reviewer` agent with `fork_turns="none"`. Give it exact SHAs `M`, `R`, and `E`; both exact-SHA review reports for `M` and `R`; pinned npm metadata/attestation/SLSA response and exact subject digest binding; 75-blob source manifest; `licenses/manifest.json`; rendered legal tree; `FORK.json.artifactMembers`; static runtime-closure inputs/results; instrumented resolved-member trace; allowlist and legal/metadata exceptions; both tgz builds/hashes; install/load/upstream-compatible/differential results; negative fixtures; candidate evidence; promoted archive; and the `R..E` diff. It must prove `M` is an ancestor of `R`, `E` is the direct child of `R`, `R..E` changes exactly `build-evidence.json`, verification/build did not mutate `R`, and evidence hashes every accepted result. Findings are ordered by severity. Any finding, missing input, offline-only provenance, non-exact SHA, pre-commit review claim, or non-evidence diff keeps Tasks 3-29 blocked. Only a zero-finding approval of exact `E` completes the positive gate.
