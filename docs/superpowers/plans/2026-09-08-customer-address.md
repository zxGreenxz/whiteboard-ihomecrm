...
- Integration headless production-build smoke: both forms have 63 provinces, accent-insensitive search, descendant reset, keyboard selection, and wheel scrolling. Real Goong conversions 903/912 ms; original fields preserved; editing source invalidates result; zero customer/image writes or runtime errors. Nested modal scroll failed at 0px before optional modal mode; vehicle search matched MOTORBIKE before label-only opt-in. Shared selector defaults stay unchanged.
- GitNexus refreshed at 4637a3b1: SearchableSelect has 81 upstream impacts (critical breadth); new modal/searchByLabel options default false to preserve all existing non-customer consumers.
