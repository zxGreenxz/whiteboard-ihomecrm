...
- Vendor source nam tai `services/openclaw-zalo-cell/vendor/zalouser-bridge/`. Repo commit exact `75` blobs duoi `extensions/zalouser` tai git head tren vi published tarball khong chua TypeScript source/tests; build reproducible verify ca npm tarball va snapshot, ap dung committed patch series + bridge source overlay, va tao mot internal tgz duy nhat. Internal `THIRD_PARTY_NOTICES.md` chua verbatim upstream notice, exact 38-package inventory, carrier paths, pako exception va Spark selection; moi carrier duoc luu exact bytes tai `licenses/<package>@<version>/<original-path>`. Archive entry contract bat buoc la `package/LICENSE`, `package/THIRD_PARTY_NOTICES.md`, `package/licenses/**`; giu runtime JS/JSON va exact legal carriers, nhung prune/reject moi dependency source, test, fixture, snapshot, docs, example, source map va inline `sourceMappingURL`. Evidence ghi upstream tarball/attestation/source/license locks, patch-series SHA-256, built-tgz SHA-256 va image digest.
...
- Compliance test byte-compare upstream root `LICENSE`/`THIRD_PARTY_NOTICES.md` theo SHA-256 da khoa, bat buoc verbatim `upstream/THIRD_PARTY_NOTICES.openclaw.md`, exact 38-package name/version/SPDX inventory va tat ca 39 carrier paths/bytes duoi `licenses/<package>@<version>/<original-path>`. Pako `LICENSE` + `lib/zlib/README` deu bat buoc; Spark-md5 chon included WTFPL branch va cam fetch `LICENSE2`.
- Vendor verifiers fail closed khi thieu/thua/doi carrier, dependency root khong khop, xuat hien notice file moi, path/case collision, traversal hoac symlink; packed tgz phai chua exact archive paths `package/LICENSE`, `package/THIRD_PARTY_NOTICES.md`, `package/licenses/**`, giu runtime JS/JSON, va khong chua dependency source/test/fixture/snapshot/docs/example, source map hay inline `sourceMappingURL`.
...
|---|---|
| Fork | Fixed tarball URL/size/3169-entry/SRI/SHA-1/attestation/SLSA locks, exact 75-blob source snapshot, root license/notice hashes, exact 38 dependency roots + 39 carriers, internal notice/inventory/tgz allowlist, patch-series SHA-256, built-tgz SHA-256 va image digest khop; verifier reject carrier/root/notice/path/case/traversal/symlink drift; deny-by-default clean context; pinned BuildKit/buildx/exporter; two fresh no-cache/pull builds voi fixed epoch + rewritten timestamps tao OCI archives co manifest/config/layers giong nhau; exact promoted archive ton tai va hash/digest khop handoff manifest; listener/outbound choke-point tests xanh; image chi co internal fork `zalouser` |
...
- `services/openclaw-zalo-cell/vendor/zalouser-bridge/` cho fixed npm tarball/attestation lock, exact 75-blob source acquisition, committed patches, bridge overlay, tests, root `LICENSE`/verbatim upstream notice, exact 38-package inventory, `licenses/<package>@<version>/<original-path>` cho 39 carrier bytes, va built internal tgz co compliance allowlist
...
- Image production chi cai internal verified `@openclaw/zalouser` tgz giu ID/channel `zalouser`; fixed tarball URL/size/entry count/SRI/SHA-1/attestation/SLSA, exact 75-blob source, root license/notice hashes, 38 dependency roots, 39 exact carrier bytes, internal notice inventory, patch-series SHA-256, built-tgz SHA-256 va architecture-specific image digest khop immutable evidence, khong co registry ZaloUser song song.
- Internal tgz co dung archive entries `package/LICENSE`, `package/THIRD_PARTY_NOTICES.md`, `package/licenses/**` cho compliance files; notice chua verbatim upstream notice + exact 38 package name/version/SPDX inventory + carrier paths + pako exception + Spark-md5 WTFPL selection. Runtime JS/JSON bat buoc duoc giu; dependency source/test/fixture/snapshot/docs/example, moi source map va inline `sourceMappingURL` bi prune/reject. Missing/extra/changed carrier, dependency-root mismatch, notice moi, path/case collision, traversal hoac symlink deu fail gate.
