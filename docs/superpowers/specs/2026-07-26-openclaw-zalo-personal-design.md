...
- Vendor verifiers fail closed khi thieu/thua/doi carrier, dependency root khong khop, xuat hien notice file moi, path/case collision, traversal hoac symlink. `FORK.json.artifactMembers` phai la exact sorted path/type/mode/size/SHA-256 manifest cua moi tgz member; exact runtime reachability allowlist phai nam trong artifact, structural bans van bat buoc, pruning khong duoc xoa allowlisted runtime member, va unlisted member bi reject. Packed tgz phai chua exact archive paths `package/LICENSE`, `package/THIRD_PARTY_NOTICES.md`, `package/licenses/**`, giu runtime JS/JSON, khong chua dependency source/test/fixture/snapshot/docs/example, source map hay inline `sourceMappingURL`, va pass install/load/upstream-compatible/differential runtime tests.
