...
- Commit `M` chua `.gitattributes` voi vendor-wide `-text` + tgz `binary`, va exact raw bytes cua metadata, npm keys, attestations, Sigstore trusted root. Moi GET unauthenticated dung `Accept: application/json`, `Accept-Encoding: identity`, status 200, JSON content type, zero redirect, stream cap+1 abort: metadata `https://registry.npmjs.org/@openclaw%2Fzalouser/2026.7.1` 64 KiB; keys `https://registry.npmjs.org/-/npm/v1/keys` 64 KiB; attestations `https://registry.npmjs.org/-/npm/v1/attestations/@openclaw%2fzalouser@2026.7.1` 256 KiB; trust root `https://tuf-repo-cdn.sigstore.dev/targets/6494e21ea73fa7ee769f85f57d5a3e6a08725eae1e38c755fc3517c9e6bc0b66.trusted_root.json` 64 KiB, size `6787`, SHA-256 `6494e21ea73fa7ee769f85f57d5a3e6a08725eae1e38c755fc3517c9e6bc0b66`. `UPSTREAM.json` bind path/size/SHA-256/endpoint/cap; qualifying hashes doc tu Git blob bang `git ls-tree`/`git cat-file`, khong doc checkout hay reserialized JSON.
- Checkpoint bat buoc `M -> R -> E`: `R` freeze/review exact source tree; qualifying export dung Git object bytes va giu type/mode; candidate nam absolute duoi source `.release/`; detached child worktree tai exact `R` copy/re-hash mot file, commit `E`, prove `E^==R` va diff chi `build-evidence.json`, remove worktree roi fast-forward. Schema-v1 evidence embed exact canonical M/R review report bytes/base64 + size/SHA-256, reviewed SHA, reviewer role/identity/run ID, `APPROVED`, findings rong; E reviewer byte-compare va revalidate. Khong commit source cung tracked evidence.
...
- Vendor gate chay Node `>=24.15.0 <25`; Node `22.20` khong duoc tuyen bo supported. Package script la `vendor:prepare`, khong duoc dinh nghia lifecycle `prepare`. Task 2 phai exclude chinh xac immutable `upstream/package/**` va binary/generated artifact paths khoi root Vitest/ESLint ma khong an `vendor/zalouser-bridge/test/**` owned tests.
- Deterministic image gate dung exact `-ReviewedTree` Git blobs, context-root v2 record algorithm/golden root `925be74a4fe381076871348887a653659ada468fa21333d5d22585be9e381f4e`, va absolute `-BuildxPath`. Buildx exact `0.13.1` binary SHA-256 Windows `6b113e84cbc3cd645646aa82f00a7f7d3737cc10375b4341e0aca0de0c997c75`/Linux `3e2bc8ed25a9125d6aeec07df4e0211edea6288e075b524160ef3fd305d3d74c`; BuildKit exact pin hien co. Dockerfile multi-stage, moi install/build `RUN --network=none`, empty offline cache/no fallback, deterministic install timestamps/state, final rootfs chi chua runtime closure. `FORK.json.installedTree` bind path/type/mode/size/SHA-256 + counts/root; verify list/inspect/package.json/all discovery roots va reject duplicate/shadow. Hai builder phai tao byte-identical OCI archive va extracted layout/index/blob set. `build-evidence.schema.v1.json` closed moi object; PowerShell 7.3 wrapper self-contained va fail-fast.
