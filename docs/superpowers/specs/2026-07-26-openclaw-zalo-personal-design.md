...
- Commit `M` chua exact `87` blob inputs: `75` source-snapshot blobs; `.gitattributes` voi vendor-wide `-text` + tgz `binary`; `vite.config.ts` va `eslint.config.js` voi exclusion chinh xac cho immutable `upstream/package/**` + binary/generated vendor artifacts nhung khong an owned `vendor/zalouser-bridge/test/**`; `UPSTREAM.json`; `SHA512SUMS`; bon raw metadata/npm-key/attestation/Sigstore-trust blobs; hai root compliance blobs; va `licenses/manifest.json`. `UPSTREAM.json` bind inventory/count/canonical aggregate hash nay. Moi GET unauthenticated dung `Accept: application/json`, `Accept-Encoding: identity`, status 200, JSON content type, zero redirect, stream cap+1 abort: metadata `https://registry.npmjs.org/@openclaw%2Fzalouser/2026.7.1` 64 KiB; keys `https://registry.npmjs.org/-/npm/v1/keys` 64 KiB; attestations `https://registry.npmjs.org/-/npm/v1/attestations/@openclaw%2fzalouser@2026.7.1` 256 KiB; trust root `https://tuf-repo-cdn.sigstore.dev/targets/6494e21ea73fa7ee769f85f57d5a3e6a08725eae1e38c755fc3517c9e6bc0b66.trusted_root.json` 64 KiB, size `6787`, SHA-256 `6494e21ea73fa7ee769f85f57d5a3e6a08725eae1e38c755fc3517c9e6bc0b66`. Qualifying hashes doc tu committed Git blobs, khong doc checkout hay reserialized JSON.
- Checkpoint bat buoc `M -> R -> E`: `R` freeze/review exact source tree; bootstrap exporter tu exact `R` blob, verify blob type/object ID/size, roi dung `git ls-tree -rz --full-tree` + `git cat-file --batch` de preserve `100644`/`100755`, reject moi type/mode khac, ghi deterministic type/mode/size/Git-object/content-hash manifest va re-hash toan bo export truoc moi install/verifier/build. Working-tree read, checkout, JSON reserialization va `git archive` deu khong qualifying; archive chi duoc xuat hien trong negative/diagnostic rejection fixture. Candidate nam absolute duoi source `.release/`; detached child worktree tai exact `R` copy/re-hash mot file, commit `E`, prove `E^==R` va diff chi `build-evidence.json`, remove worktree roi fast-forward. Schema-v1 evidence embed exact canonical M/R review report bytes/base64 + size/SHA-256, reviewed SHA, reviewer role/identity/run ID, `APPROVED`, findings rong; E reviewer byte-compare va revalidate. Khong commit source cung tracked evidence. Task 27 lap lai dung lifecycle `R27 -> qualifying exact-R27 build -> evidence-only E27`; source-only preflight truoc `R27` khong duoc tao release archive/evidence, va downstream chi consume `E27` da independent review.
...
- Vendor gate chay Node `>=24.15.0 <25`; Node `22.20` khong duoc tuyen bo supported. Package script la `vendor:prepare`, khong duoc dinh nghia lifecycle `prepare`. Task 2 dat exclusion chinh xac cua immutable `upstream/package/**` va binary/generated artifact paths vao chinh `M`, de root Vitest/ESLint khong traverse 22 upstream test files nhung van chay owned `vendor/zalouser-bridge/test/**` tests.
- Deterministic image gate dung exact `-ReviewedTree` Git blobs da export/verify boi reviewed `export-reviewed-tree.mjs`, context-root v2 record algorithm/golden root `925be74a4fe381076871348887a653659ada468fa21333d5d22585be9e381f4e`, va absolute `-BuildxPath`. Buildx exact `0.13.1` binary SHA-256 Windows `6b113e84cbc3cd645646aa82f00a7f7d3737cc10375b4341e0aca0de0c997c75`/Linux `3e2bc8ed25a9125d6aeec07df4e0211edea6288e075b524160ef3fd305d3d74c`; BuildKit exact pin hien co. Dockerfile multi-stage, moi install/build `RUN --network=none`, empty offline cache/no fallback, deterministic install timestamps/state, final rootfs chi chua runtime closure. `FORK.json.installedTree` bind path/type/mode/size/SHA-256 + counts/root; verify list/inspect/package.json/all discovery roots va reject duplicate/shadow. Hai builder phai tao byte-identical OCI archive va extracted layout/index/blob set. `build-evidence.schema.v1.json` closed moi object; PowerShell 7.3 wrapper self-contained va fail-fast.
...
- Provenance test bat buoc `M` co `.gitattributes` va exact raw metadata, npm keys, attestation bundles, Sigstore trusted-root target; `UPSTREAM.json` bind path/endpoint/cap/size/SHA-256 cua ca bon. Verifier dung Git blob bytes de validate npm/Fulcio/Rekor/DSSE/SLSA va trust pins; refetch chi byte-compare, khong thay input. Evidence `E` recheck ca bon va embedded M/R review reports.
...
- Command-contract tests reject pre-review qualifying runs, working-tree/reserialized bytes, `git archive`/checkout source paths, wrong Git type/mode, an exporter that omits `git ls-tree -rz --full-tree`/`git cat-file --batch` or skips complete manifest/output re-hash before install, wrong context-root v2/golden vector, helper khong dung `-ReviewedTree` + absolute pinned `-BuildxPath`, source/evidence mixing, missing `RUN --network=none`/offline empty cache/no fallback/minimal final rootfs, mutable install state, wrong buildx/BuildKit, non-byte-identical OCI archive/layout/index/blob, open evidence schema, tampered review reports, unsafe cleanup, hoac native failure ma van chay sentinel sau do. Evidence candidate absolute duoi source `.release/`; E dung detached exact-R worktree va diff mot file. Task 27 source-only preflight is non-qualifying; exact `R27` review must precede the qualifying `-ReviewedTree $R27` build, and only independently reviewed evidence-only `E27` may be consumed downstream.
