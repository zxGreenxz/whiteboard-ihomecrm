...
- Immutable Task 2 checkpoints theo dung thu tu: commit `M` chua reviewed manifest + raw provenance inputs va chi duoc review sau khi commit, truoc moi notice/carrier/artifact rendering; commit `R` la final source/artifact/lock/tooling commit sau khi moi tracked vendor output, `FORK.json` va `artifactMembers` da freeze, roi duoc independent review tai exact SHA; moi verification/build chay strictly tu clean `R` ma khong mutate tracked file; positive OCI/evidence tao evidence-only child commit `E` co diff chinh xac mot file `services/openclaw-zalo-cell/build-evidence.json`; chi sau independent review cua exact `E` moi mo Tasks 3-29. Khong co pre-commit hay plan-only `reviewed commit` claim nao co gia tri gate.
...
2. Written spec review phai xac nhan lai trust boundaries, exact upstream tarball/attestation/75-blob source locks, root notice/license hashes, 38-package/39-carrier compliance inventory, vendor file ownership, inbound durability ordering, outbound choke-point coverage va artifact evidence.
