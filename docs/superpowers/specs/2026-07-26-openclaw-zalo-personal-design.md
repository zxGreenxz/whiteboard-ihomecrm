...
- Signed attestation subject SHA-512 phai bang exact SRI-decoded tgz digest `92583404e3930efe3153292003fa53643b11ac8f5dcda82adb73a53eea422eb1628c339e6cfc4661a61d58348fcb8cc12405a38e74ab832081fb93ae08cbd91b`; verifier chi tin trusted npm issuer chain, source repo `https://github.com/openclaw/openclaw`, workflow `.github/workflows/plugin-npm-release.yml`, release ref `refs/heads/release/2026.7.1`, build type `https://slsa-framework.github.io/github-actions-buildtypes/workflow/v1`, va resolved commit `2d2ddc43d0dcf71f31283d780f9fe9ff4cc04fe4`. Bat ky subject/issuer/repository/workflow/ref/build-type/commit/byte mismatch nao deu fail closed.
- Commit `M` phai tu chua exact raw verification inputs tai `upstream/provenance/npm-registry-metadata.json` va `upstream/provenance/npm-attestation-bundles.json`; file sau giu nguyen npm attestation response gom Sigstore bundle, DSSE envelope va SLSA statement duoc verifier dung, khong thay bang extracted projection. `UPSTREAM.json` bind exact path/byte size/SHA-256 cua tung file va signed subject name/SHA-512. Review exact `M` phai verify cryptographic signature/trusted issuer va exact source repo/workflow/ref/build type/resolved commit tu chinh committed bytes. Network refetch chi duoc byte/hash-compare, khong duoc ghi de hay thay the raw bytes da review trong `M`.
- Immutable Task 2 checkpoints theo dung thu tu: commit `M` chua reviewed manifest + self-contained raw provenance inputs va chi duoc review sau khi commit, truoc moi notice/carrier/artifact rendering; commit `R` la final source/artifact/lock/tooling commit sau khi moi tracked vendor output, `FORK.json` va `artifactMembers` da freeze, roi duoc independent review tai exact SHA. Truoc `R`, TDD development duoc phep chay local `npm ci`, test, compile, pack va provisional verifier de tao candidate bytes, nhung cac run nay khong duoc tao/cap nhat qualifying evidence, promote OCI archive hay mo Tasks 3-29. Sau khi `R` da commit va review, truoc bat ky dependency install hay executable verifier/build nao cua positive/release gate, preflight phai chung minh `HEAD` bang exact `R`, `git status --porcelain=v1 --untracked-files=all` rong, index rong, va khong co merge/rebase dang do; sau do chi export va chay tu bytes cua `R`, khong mutate tracked file. Positive OCI/evidence tao evidence-only child commit `E` co diff chinh xac mot file `services/openclaw-zalo-cell/build-evidence.json` va record/check exact raw-provenance path/size/SHA-256 set tu `M`; chi sau independent review cua exact `E` moi mo Tasks 3-29. Khong co pre-commit hay plan-only `reviewed commit` claim nao co gia tri gate.
...
- Vendor source nam tai `services/openclaw-zalo-cell/vendor/zalouser-bridge/`. Repo commit exact `75` blobs duoi `extensions/zalouser` tai git head tren vi published tarball khong chua TypeScript source/tests; build reproducible verify ca npm tarball va snapshot, ap dung committed patch series + bridge source overlay, va tao mot internal tgz duy nhat. Internal `THIRD_PARTY_NOTICES.md` chua verbatim upstream notice, exact 38-package inventory, carrier paths, pako exception va Spark selection; moi carrier duoc luu exact bytes tai `licenses/<package>@<version>/<original-path>`. `FORK.json` la external control metadata va khong duoc pack; vi vay `FORK.json.artifactMembers`/canonical array hash khong tu tham chieu va khoa exact sorted file-by-file path/type/mode/size/SHA-256 cho moi tgz member, con exact `runtimeReachabilityAllowlist` khoa moi runtime JS/JSON/package-metadata member bat buoc. Archive entry contract bat buoc la `package/LICENSE`, `package/THIRD_PARTY_NOTICES.md`, `package/licenses/**`; structural bans van fail closed, pruning khong duoc xoa runtime-reachable member, va khong member nao ngoai `artifactMembers` duoc phep. Install/load, upstream-compatible va differential runtime tests phai pass tren exact packed/installed bytes. Evidence ghi upstream tarball/attestation/source/license locks, exact raw registry/attestation bundle path/size/SHA-256 records tu `M`, manifest/artifact-member hashes, patch-series SHA-256, built-tgz SHA-256 va image digest.
...
- Vendored fork integrity: exact fixed tarball URL, toi da 3 HTTPS redirects chi trong `registry.npmjs.org`/direct subdomains, `2341459` bytes, `3169` regular `package/` entries, npm SRI/SHA-1, mandatory attestation subject + SLSA resolved commit, exact committed `75`-blob source snapshot, patch-series SHA-256, built-tgz SHA-256, internal-only install va deterministic `linux/amd64` image digest. Missing metadata/network hoac offline-only run khong duoc tao/cap nhat artifact/evidence/hash hay mo Tasks 3-29. Image test dung deny-by-default clean context, fixed build arg, pinned BuildKit/buildx contract, two fresh builders, no cache, pull base, two isolated OCI outputs voi `rewrite-timestamp=true`, verify source epoch trong package metadata, va promote mot exact verified archive cho rollout.
- Provenance test bat buoc commit `M` co exact raw `upstream/provenance/npm-registry-metadata.json` va `upstream/provenance/npm-attestation-bundles.json`; `UPSTREAM.json` bind path/byte size/SHA-256 cua ca hai va signed subject. Verifier phai parse va cryptographically validate trusted issuer/signature, source repo, workflow, ref, build type, resolved commit va tgz subject digest tu exact committed bytes. Network refetch chi so sanh voi `M`, khong duoc thay raw inputs; evidence `E` record va recheck exact raw-input hashes.
...
- Vendor verifiers fail closed khi thieu/thua/doi carrier, dependency root khong khop, xuat hien notice file moi, path/case collision, traversal hoac symlink. `FORK.json.artifactMembers` phai la exact sorted path/type/mode/size/SHA-256 manifest cua moi tgz member. Reachability runtime phai duoc kiem chung hai chieu va doc lap: analyzer inventory moi dynamic resolution/file-read site, gom non-literal `import()`, `require`/`createRequire`, `import.meta.resolve`, `exports`/`bin` indirection, computed filesystem read/asset/module resolution, cung recursive static import/require. Moi site phai resolve thanh exact member statically hoac map vao mot reviewed finite pattern co exhaustive expansion; site khong classified/resolved deu fail closed ke ca khi runtime tests khong exercise no. Instrumented install/load/upstream-compatible/differential tests ghi moi resolved member; bat buoc `derivedRuntimeSet == resolvedRuntimeSet == runtimeReachabilityAllowlist`. Moi runtime-classified artifact member phai nam trong allowlist; exact legal carriers va package metadata duoc liet ke ro la hai nhom ngoai le duy nhat. Negative fixture bat buoc chen mot unexercised/unclassified dynamic site va chung minh analyzer reject. Listed-unresolved, resolved-unlisted, artifact runtime member ngoai allowlist, hoac pruning xoa allowlisted runtime member deu fail closed; structural bans van bat buoc. Packed tgz phai chua exact archive paths `package/LICENSE`, `package/THIRD_PARTY_NOTICES.md`, `package/licenses/**`, giu runtime JS/JSON, khong chua dependency source/test/fixture/snapshot/docs/example, source map hay inline `sourceMappingURL`, va pass install/load/upstream-compatible/differential runtime tests.
- Command-contract tests phan biet pre-`R` TDD development voi positive/release gate: local development co the chay dependency install/test/build/provisional verify nhung khong duoc tao qualifying evidence, promote archive hay mo Tasks 3-29. Voi positive/release gate sau reviewed `R`, tests reject neu preflight khong chay truoc dependency install/executable verifier/build, `HEAD` khong bang exact reviewed `R`, `git status --porcelain=v1 --untracked-files=all` khong rong, index khong rong, merge/rebase dang do, hoac lenh chay tu working tree thay vi exact exported `R`. Generated ignored directories co the ton tai ngoai export nhung khong duoc vao exported context. Test cung reject neu thieu `.dockerignore`/clean-context allowlist, tao self-referential lock, khong bind reviewed lock hash vao context-root hash, cho host `node_modules`/dirty/untracked/secret/build-evidence vao context, thieu `ARG SOURCE_DATE_EPOCH`, dung persistent `ENV`, thieu pinned BuildKit digest/version, `--build-arg`, `--no-cache`, `--pull`, pinned OCI media/compression/compatibility options, `rewrite-timestamp=true`, fresh-builder isolation, distinct OCI destinations, manifest/config/layer comparison, package-metadata epoch verification, promoted archive hash/digest/handoff, release evidence lam dirty tracked files, safe exact-name cleanup, hoac fail-fast native-command handling.
