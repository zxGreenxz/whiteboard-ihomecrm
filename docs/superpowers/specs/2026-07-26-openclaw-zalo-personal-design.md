...
- Checkpoint bat buoc `M -> R -> E`: `R` freeze/review exact source tree; bootstrap exporter tu exact `R` blob, verify blob type/object ID/size, roi dung `git ls-tree -rz --full-tree` + `git cat-file --batch` de preserve `100644`/`100755`, reject moi type/mode khac, ghi deterministic type/mode/size/Git-object/content-hash manifest va re-hash toan bo export truoc moi install/verifier/build. Working-tree read, checkout, JSON reserialization va `git archive` deu khong qualifying; archive chi duoc xuat hien trong negative/diagnostic rejection fixture. Candidate evidence va OCI archive nam absolute duoi source `.release/`, khong reparse; moi evidence-child block tu rebind exact reviewed SHA, recompute canonical source/release/evidence/archive paths + hashes/types, reject reparse tren relevant ancestors, va khong consume variable chi co o block truoc. Detached child path canonical va contained trong canonical non-reparse temp root. Exact verifier trong child phai validate copied evidence, closed schema, exact reviewed tree va retained absolute archive truoc stage/commit. Chi commit mot `build-evidence.json`, prove `E^==R`; OCI archive van external. Cleanup dung checked `git worktree remove --force`; cleanup-only error phai fail, cleanup error dong thoi khong duoc mask primary error. Chi fast-forward sau khi directory va worktree registration deu mat. Schema-v1 evidence embed exact canonical M/R review report bytes/base64 + size/SHA-256, reviewed SHA, reviewer role/identity/run ID, `APPROVED`, findings rong; E reviewer byte-compare va revalidate. Khong commit source cung tracked evidence. Task 27 lap lai `R27 -> qualifying exact-R27 build -> evidence-only E27`. Final rollout lap lai `R29 -> qualifying -ReviewedTree R29 -> evidence-only direct child E29 -> independent E29 approval`; bundle/Edge/Worker/VPS chi consume approved E29, trong khi image evidence van bind R29 va `E29^==R29`.
- Task 2 co mot execution entrypoint dong. Root operator stage `approval-manifest-v1.json` tai `/opt/openclaw-tools/reviewed-task2-approvals/<R>/`, chay exact raw-R `install-reviewed-task2-launcher.mjs` tu fixed bootstrap path, va installer atomically promote `launch-reviewed-task2.mjs` + manifest thanh root-owned read-only tree `/opt/openclaw-tools/reviewed-task2/<R>/`. Launcher chay bang `openclaw-runner`; operator chi truyen phase, canonical repository root va exact installed manifest. M/R, review reports, Node/Git/PowerShell/npm/buildx/Docker/socket va sau raw-R helper bindings deu duoc doc tu closed manifest, reauthenticate truoc/sau execution, va embed vao `execution_authority` cua evidence. Inner PowerShell source duoc parse/execute in-memory voi logical path dung, khong `/dev/stdin`, profile, PATH lookup hay caller-selected authority.
...
- Build/gate entrypoints chay dependency-free stable Node assertion tren exact `process.version`: chi accept official stable `v24.<minor>.<patch>` matching `^v24\.(0|[1-9]\d*)\.(0|[1-9]\d*)$` voi minor integer `>=15`; reject `v22.20.0`, `v24.14.x`, `v25.x`, `v24.15.0-rc.1`, `v24.15.0-pre`, va moi suffix prerelease/build truoc first Node/npm/npx/vendor/helper action hay work/artifact/evidence. CI pin exact Node `24.15.0`, preflight truoc `npm ci`; vendor harness pin exact `packageManager: npm@11.12.1` va preflight dependency-free phai thay exact user agent `npm/11.12.1` truoc vendor `npm ci` hay owned command. Explicit reviewed-export options la mot binding atomic gom path/tree/SHA-256, khong duoc tron voi field thieu tu environment. Pin npm nay chi ap dung vendor harness, khong doi root package-manager contract. Planned `test:openclaw:services` prefix cung assertion; `build-reproducible-image.ps1` self-preflight truoc temp/builders/output. Node `22.20` khong supported cho gate. Rieng session-crypto package/lock van giu reviewed `engines.node >=22.13.0` nhu runtime/library minimum, khong doi theo build gate. Vendor script la `vendor:prepare`, khong co lifecycle `prepare`. Task 2 dat exclusion chinh xac cua immutable `upstream/package/**` va binary/generated artifact paths vao chinh `M`, de root Vitest/ESLint khong traverse 22 upstream test files nhung van chay owned tests.
...
- Host moi sau resize da duoc kiem tra read-only ngay 2026-07-28: Ubuntu 26.04, 2 vCPU, 3,474,336 KiB RAM nhin thay trong guest, 6 GiB swap va root disk 79,930,167,296 bytes. Owner cho phep production tren tier hien tai va se nang cap sau khi metric yeu cau. Production profile la mot organization/mot cell, worker concurrency `1`, `CPUQuota=180%`, `MemoryHigh=2200M`, `MemoryMax=2800M`, `MemorySwapMax=2G`, `TasksMax=512` va 20 GiB local disk. Capacity upgrade la trigger van hanh, khong phai hard gate Task 29.
...
- Dung VPS Vultr Seoul moi, chuyen dung cho OpenClaw. Baseline read-only sau resize ngay 2026-07-28: Ubuntu 26.04, 2 vCPU, 3,474,336 KiB RAM nhin thay trong guest, swapfile 6,710,882,304 bytes, root disk 79,930,167,296 bytes voi 59,544,616,960 bytes trong tai thoi diem kiem tra. Rootless Docker `29.1.3` khoi dong lai healthy va khong co workload container sau resize.
- Cau hinh hien tai duoc phep production theo low-tier profile da duyet. Moi pipeline cell, bridge, outbox, media va AI chi co concurrency `1`; queue/bounded spool hap thu burst thay vi spawn worker. Resource guard pause AI auto-send, automation va group-send truoc khi anh huong inbox/spool/manual non-AI. Swap la shock absorber, khong phai ly do de tang concurrency.
...
Implementation plan phai tao ba npm scripts `test:openclaw:services`, `test:openclaw:sql`, `test:openclaw:r2` truoc khi dung checklist nay. Stable-version Node assertion phai la first Node action in this checklist and in the service-script prefix; CI pins `24.15.0` before `npm ci`, va prerelease/build-suffixed versions fail. Session-crypto `verify:dist` must compare two clean builds against exactly three committed dist blobs while preserving package/lock engine `>=22.13.0`. Browser mac dinh headless; khong tu mo headed browser.
...
| VPS 2 vCPU/3.3 GiB guest RAM co the tang queue lag/swap | Production concurrency 1, CPU 180%, memory high 2200M/max 2800M, swap max 2G; pause AI/automation/group truoc, alert/upgrade khi swap >1 GiB trong 15 phut, memory pressure/OOM/restart, queue p95 >30 giay hoac disk >80% |
