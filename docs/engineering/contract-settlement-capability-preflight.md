...
3. Live freeze `a00_ie_owned_payload_freeze` thiếu `change_field_mask` trong lifecycle allowlist; request changes viết field này, resubmit clear. Kiểm tiếp bằng PostgreSQL 17.10 + PostgREST 16.3 local, với định nghĩa RPC/freeze trùng hash live, đã tái hiện resubmit phiếu owned trả `55000` khi clear mask. **Token có được tạo gián tiếp qua `finance_v2_begin_canonical_op`**; không được suy thiếu token chỉ vì body RPC không tạo trực tiếp. Sửa quyền chuyển các field review bằng purpose phù hợp, không mở guard tiền/ngân hàng. Đây là bằng chứng runtime local, chưa thay thế kiểm writer trên DEMO shared.
