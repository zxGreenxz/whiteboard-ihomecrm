...
| G-DEPLOYMENT (trước ghi là "T0b" — đổi ID, lý do như trên) | Deployment control | `PREPARED` | Workflow hardened (`workflow_dispatch`-only, exact commit/path/hash/recovery/window, environment `supabase-production`) đã nằm trên branch `security/authz-preparation` (đã push origin 2026-07-16). **NGUY HIỂM ĐÃ XÁC MINH:** live migration ledger chỉ có **20 dòng** trong khi repo có **364 file** `supabase/migrations/*.sql` (drift 344 file — phần lớn đã applied vào DB nhưng KHÔNG ghi ledger). `origin/main` vẫn chạy workflow CŨ (`push` → `supabase db push`). **Nếu merge/push branch này vào `main`, `db push` sẽ cố replay 344 migration lên production** → hỏng nặng. Vì vậy 2026-07-16 CHỈ push branch (không trigger gì; Vercel prod deploy từ main), KHÔNG merge main. | Trước khi merge workflow hardened vào main: (1) reconcile ledger (đánh dấu migration đã-applied bằng `supabase migration repair` hoặc insert ledger thủ công) HOẶC xác nhận workflow mới thay hẳn workflow cũ trong cùng PR để `db push` không bao giờ chạy; (2) cấu hình GitHub environment `supabase-production` + required reviewer + secret `SUPABASE_DB_URL`. Gate A chưa đạt trên remote. |
