...
| Database | `supabase/migrations/**` | Lịch sử DDL/RPC/RLS đang hoạt động (678 file + 15 trong `migrations-archive/`). Legacy KHÔNG replay được; số đếm sinh bằng `npm run catalog:capture`. |
...
| `.e2e-fleet/**` | 49 spec Playwright | Chạy LOCAL, cần `FLEET_PASS_*`, chỉ ghi vào org DEMO. Không phải CI gate — xem `tooling/test-matrix.json`. |
