...
| Database | `supabase/migrations/**` | Lịch sử DDL/RPC/RLS đang hoạt động (812 file + 15 trong `migrations-archive/`). Legacy KHÔNG replay được; số đếm sinh bằng `npm run catalog:capture`. |
