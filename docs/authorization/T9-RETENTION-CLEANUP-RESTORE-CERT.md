...
- **In scope:**
  ...
  - Final restore certification — **lifecycle 2 recovery ID, không mâu thuẫn trạng thái:** (1) recovery **trước-cleanup** phải đã `VERIFIED` từ trước (đây là gate mở DROP — không phải việc của T9; nếu recovery còn PARTIAL thì T1–T8 lẫn cleanup đều bị chặn từ upstream); (2) T9 cleanup từng batch; (3) T9 capture + certify một recovery ID **MỚI** (portable Auth/Storage-aware dump, exhaustive R2, independent blank restore) phản ánh catalog **sau** cleanup. T9 không "nâng" recovery cũ từ PARTIAL lên VERIFIED — nó chứng nhận một recovery mới sau khi dọn.
...
- **Final restore certification evidence (đặc thù T9):**
  ...
  - Independent **blank-target restore rehearsal**: restore vào target trắng, verify schema/ACL/security/money/object/browser; sinh recovery ID mới certified phản ánh catalog **sau** cleanup. **Target trắng KHÔNG bắt buộc là cloud** — tiêu chuẩn mô tả theo năng lực cần chứng minh: (1) target độc lập về môi trường (blank local PostgreSQL cluster/VM/Docker Supabase stack, hoặc project cloud tạm nếu owner mở phạm vi); (2) full coverage (DB gồm `auth`+`storage` metadata, object bytes, config); (3) quy trình tái lập được từ sealed artifact; (4) reviewer độc lập với người capture. Local-only vẫn đạt được nếu 4 điều kiện trên thỏa. **Tiến độ 2026-07-16:** dump portable qua session pooler (postgres role, đọc được `auth`/`storage`) đã restore thành công vào blank local cluster PG17 — 187/190 bảng khớp row-count, 3 lệch = drift online-capture (bảng hoạt động realtime), money sums khớp 100%, `rooms` phục hồi đủ sau khi pre-create 2 sort function (gap TOC-order của pg_dump với generated column); extension `vector`/`pg_cron`/`supabase_vault` không có trên Windows local → bảng phụ thuộc (AI embeddings, cron ledger, vault) cần target có extension (Docker/VM Linux hoặc project tạm) cho certification cuối.
  - Manifest counts/hash, **replica trên fault-domain vật lý thực sự khác nhau** (external SSD offline / máy thứ hai / NAS — 3 bản cùng ổ D: hiện tại KHÔNG đạt; "khác fault domain" không đồng nghĩa "phải cloud"), secret-scan finding đã rotation-review (2026-07-16: database password đã rotate qua Management API sau khi gửi qua chat, pooler login 3/3 node verify với password mới).
