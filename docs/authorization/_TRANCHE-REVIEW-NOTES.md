...
| (c) reorder §27.3/§27.5 | **Gap — cần owner** | §27.3 quy định T3 **trước** T6; §27 tự tuyên là nguồn chuẩn; §27.5 chỉ có dòng đơn T4/T6/T7+ | ⛔ **Owner quyết:** (1) giữ §27.3 (T3 trước T6a) hay chấp nhận reorder T6a-trước-T3; (2) sửa CHÍNH THỨC §27.3 + bảng §27.5 sang T4a/T6a/T6b. Việc *tách* sub-tranche được §27.5 lường trước; việc *đảo thứ tự tương đối* thì không — chỉ owner ghi đè được. |
...
| (d2-infra) schema feature-flag/canary | **Gap — cần owner** | §27.4 bước 5 + §16.6 chỉ *giả định* flag; không định nghĩa bảng, không giao tranche | ⛔ **Owner quyết:** chỉ định tranche sở hữu (đề xuất T5) + duyệt schema bảng flag; chốt nơi lưu cột `auto_approve_invoice`. |
| (d3) phân loại ~132 bảng | **Gap — cần owner + live** | §16.3 nhóm bảng, §16.8 stop-the-line null org money/PII, §11.3 TENANT/PLATFORM — nhưng **không** có danh sách từng bảng | ⛔ **Owner/T2 quyết:** chạy phân loại từ live `information_schema` rồi owner ký duyệt nhóm mỗi bảng; là tiền đề cho mọi quyết định NOT NULL của T6a. |
| (e) terminal-gate T9 | **Gap — cần owner** | STATUS: local capture chỉ tới `PARTIAL/CAPTURED_UNVERIFIED`; T9 §5 đòi blank-target restore + fault-domain thật; T9 §9 đẩy về owner | ⛔ **Owner quyết:** (1) MỞ LẠI blank-target restore trên target độc lập + fault-domain thật (off-D:, off-máy) — con đường DUY NHẤT plan cho phép lên `VERIFIED`; hoặc (2) giữ local-only và chấp nhận chương trình đứng NO-GO ở gate cuối. Lưu ý: yêu cầu "3 fault domain" đến từ master plan Phase B, chặt hơn §27.6 gốc (§27.6 chỉ đòi "xác nhận backup/PITR/restore procedure"). |

**Tóm tắt:** (a), (b), (d1), (d2-default) đã kéo về đúng plan trong lượt này. Còn lại **(c), (d2-infra), (d3), (e) là quyết định owner thật** — plan cố ý để mở hoặc §27 tự tuyên chỉ owner mới ghi đè được. Không mục nào là safety regression; production gate vẫn NO-GO.
