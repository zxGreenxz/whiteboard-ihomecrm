...
**Open items chặn prepare:** bốn permission key `income_expenses.self_approve_within_limit`, `cashbooks.post`, `approvals.emergency_override`, `income_expenses.reverse` **chưa nằm trong** tập đã seed ở `20260713110100_sprint2b_seed_permission_definitions.sql` (grep xác nhận có `income_expenses.approve/create/edit/cancel`, `thu_tien.collect/undo`, `cashbooks.create/edit/share`, nhưng KHÔNG có bốn key trên và không có namespace `approvals.*`). Đây **không phải quyết định owner**: theo `AUTHORIZATION-PLAN.md` §16.4 bước 1 ("Seed `permission_definitions` từ registry, freeze danh sách key") việc seed thuộc **T2** — nên mở rộng migration seed của T2/Sprint 2b thêm bốn key (`permission_domain='TENANT'`, resource/action đúng khuôn: `self_approve_within_limit`+`reverse` thuộc resource `income_expenses`, `cashbooks.post`, `approvals.emergency_override`) **trước** khi FK `approval_step_approvers.permission_key → permission_definitions.key` (§12.3) được tạo. Còn lại **cần owner**: versioned self-approve limit config (hạn mức), held-cashbook binding model, và nơi lưu org-setting classification — chưa có trong schema (xem §Open questions).
