...
- **In scope:**
  ...
  1b. **Forward migration seed 4 permission key cho T3** (T2 sở hữu registry theo §16.4): `income_expenses.self_approve_within_limit`, `cashbooks.post`, `approvals.emergency_override`, `income_expenses.reverse` — tạo **migration timestamp MỚI** (KHÔNG sửa `20260713110100` đã APPLIED — ledger production đã ghi version nên file sửa sẽ không chạy lại); `permission_domain='TENANT'`, resource/action đúng khuôn registry. Migration này là **hard dependency của T3** và phải áp trước khi T3 tạo FK `approval_step_approvers.permission_key → permission_definitions.key`. T3 chỉ consume, không seed.
