...
| [RUNBOOK-CUTOVER-PAYMENT.md](./RUNBOOK-CUTOVER-PAYMENT.md) | **`ĐÃ THỰC THI 2026-07-18 (canary)`** | Runbook cutover domain #1. P2–P4 đóng 17/07; P1 né bằng release branch `release/payment-w1`. Window W thực thi trọn theo ủy quyền owner — xem evidence dưới. |
| [W1-PAYMENT-CANARY-EVIDENCE-2026-07-18.md](./W1-PAYMENT-CANARY-EVIDENCE-2026-07-18.md) | **`APPLIED (canary demo) — observation`** | Window W1 trọn vẹn: dump `20260717T165216Z` + 7 SQL applied + merge main `e4eec85` + FLIP CANARY demo (cap 10/5tr/5tr) + e2e pre-flip fallback (v4 403→v3 200) + e2e CANONICAL 200 + replay-exact + over-cap 55000 + reconcile org thật khớp baseline từng đồng. 3 defect bắt+vá trong window (zombie session, ops-cap gap, guard-order replay), 2 phát hiện T6a (cross-org defaults demo→sổ/loại thu org thật — forward-fixed; RLS accounts cross-org read). t1b_01 final `5041ac2e…`, t1b_90 10/10 staging+local. |
