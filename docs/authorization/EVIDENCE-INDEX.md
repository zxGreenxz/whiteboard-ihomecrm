...
- **Giới hạn trung thực:** harness thiếu `vector`/`pg_cron`/`supabase_vault`; PostgREST/JWT thật chưa test (SET ROLE mô phỏng); các pass bind vào hash các file trên + artifact `caae2097…` — mọi edit làm evidence stale; đây là PREPARED evidence, KHÔNG phải production apply/activation. Approval v2 state machine (submit/decide/emergency/reverse), rollout CAS RPC, maker-checker/self-limit engine chưa nằm trong bộ này — vẫn IN_DESIGN.
