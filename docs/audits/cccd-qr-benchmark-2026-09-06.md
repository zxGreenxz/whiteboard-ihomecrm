...
**Trạng thái phát hành:** chưa push/promote tại lần cập nhật này; OCR đã qua rà soát độc lập và sửa ba vấn đề về tải HTTP nén, trộn thông tin hai thẻ và chữ bị cắt mép; camera cũng đã qua rà soát độc lập và sửa ba vấn đề; bản tích hợp còn rà soát toàn bộ và gate cuối. Clipboard hệ điều hành Windows, webcam thật, Android Chrome và iPhone Safari chưa được kiểm vật lý. Tốc độ tải model lần đầu trên mạng công cộng và bộ nhớ thiết bị di động chưa có số đo. Các phần bên dưới giữ số đo lịch sử và phương pháp để đối chiếu.
