...
WHERE key IN ('reports_finance.analysis', 'reports_finance.view')
  AND permission_domain = 'TENANT'
  AND scope_kinds IS DISTINCT FROM ARRAY['ORGANIZATION', 'AREA', 'BUILDING']::text[];
...
BEGIN
  ...
  IF NOT COALESCE(v_primary_allowed, false)
     AND (
       v_primary_reason IS NULL
       OR v_primary_reason NOT IN (
         'DEFAULT_DENY',
         'PERMISSION_INACTIVE_OR_MISSING'
       )
     ) THEN
    RETURN QUERY
  ...
  IF NOT COALESCE(v_fallback_allowed, false)
     AND (
       v_fallback_reason IS NULL
       OR v_fallback_reason NOT IN (
         'DEFAULT_DENY',
         'PERMISSION_INACTIVE_OR_MISSING'
       )
     ) THEN
...
COMMENT ON FUNCTION app_private.business_performance_analysis_decision_v1(uuid, uuid, uuid) IS
  'Private deny-wins analysis decision for one organization building. Legacy detail/view fallback is limited to canonical rollout-gap reasons.';

CREATE OR REPLACE FUNCTION app_private.business_performance_authorized_buildings_v1(
  p_organization_id uuid,
  p_include_restricted boolean DEFAULT true
)
RETURNS TABLE(building_id uuid, building_name text, restricted_allowed boolean, analysis_provenance jsonb, authorization_version bigint)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, app_private, public
AS $business_performance_authorized_buildings$
DECLARE
  v_actor uuid;
  v_authorization_version bigint;
  v_analysis_allowed boolean;
  v_analysis_version bigint;
  v_analysis_provenance jsonb;
  v_restricted_allowed boolean := false;
  v_restricted_version bigint;
  v_building record;
BEGIN
  ...
  SELECT app_private.lock_org_for_decision_v1(p_organization_id)
    INTO v_authorization_version;
  IF v_authorization_version IS NULL THEN
    RETURN;
  END IF;
  ...
  BEGIN
    SELECT actor.user_id
      INTO v_actor
    FROM app_private.resolve_finance_actor_v2(p_organization_id) AS actor;
  EXCEPTION
  ...
  FOR v_building IN
    SELECT
      b.id,
      COALESCE(NULLIF(btrim(b.name), ''), 'Unnamed building') AS name
    FROM public.buildings b
    WHERE b.organization_id = p_organization_id
      AND b.is_virtual = false
      AND b.deleted_at IS NULL
      AND COALESCE(public.can_access_building(b.id), false)
    ORDER BY lower(COALESCE(NULLIF(btrim(b.name), ''), 'Unnamed building')) COLLATE "C",
             COALESCE(NULLIF(btrim(b.name), ''), 'Unnamed building') COLLATE "C",
             b.id
  LOOP
    SELECT decision.allowed, decision.authorization_version, decision.analysis_provenance
      INTO v_analysis_allowed, v_analysis_version, v_analysis_provenance
    FROM app_private.business_performance_analysis_decision_v1(
      p_actor => v_actor,
      p_organization_id => p_organization_id,
      p_building_id => v_building.id
    ) AS decision;

    CONTINUE WHEN NOT COALESCE(v_analysis_allowed, false)
      OR v_analysis_version IS DISTINCT FROM v_authorization_version;

    v_restricted_allowed := false;
    v_restricted_version := v_authorization_version;
    IF COALESCE(p_include_restricted, false) THEN
      SELECT decision.allowed, decision.authorization_version
        INTO v_restricted_allowed, v_restricted_version
      FROM app_private.authorize_tenant_action_v3(
        p_actor => v_actor,
        p_organization_id => p_organization_id,
        p_permission_key => 'income_expenses.restricted_view',
        p_building_id => v_building.id,
        p_cashbook_id => NULL
      ) AS decision;
    END IF;
    ...
    restricted_allowed := COALESCE(v_restricted_allowed, false)
      AND v_restricted_version IS NOT DISTINCT FROM v_authorization_version;
    analysis_provenance := v_analysis_provenance;
    authorization_version := v_authorization_version;
    RETURN NEXT;
  END LOOP;
END;
$business_performance_authorized_buildings$;

REVOKE ALL ON FUNCTION app_private.business_performance_authorized_buildings_v1(uuid, boolean) FROM PUBLIC;
REVOKE ALL ON FUNCTION app_private.business_performance_authorized_buildings_v1(uuid, boolean) FROM anon;
REVOKE ALL ON FUNCTION app_private.business_performance_authorized_buildings_v1(uuid, boolean) FROM authenticated;
REVOKE ALL ON FUNCTION app_private.business_performance_authorized_buildings_v1(uuid, boolean) FROM service_role;
COMMENT ON FUNCTION app_private.business_performance_authorized_buildings_v1(uuid, boolean) IS
  'Private physical-building roster for an active finance actor. Requires normalized report authorization and legacy can_access_building visibility.';

CREATE OR REPLACE FUNCTION app_private.business_performance_exact_scope_v1(
  p_organization_id uuid,
  p_building_ids uuid[],
  p_require_restricted boolean DEFAULT false
)
RETURNS TABLE(authorization_version bigint, building_ids uuid[])
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, app_private, public
AS $business_performance_exact_scope$
DECLARE
  v_building_ids uuid[];
  v_roster jsonb;
  v_roster_count bigint;
  v_distinct_roster_count bigint;
  v_min_authorization_version bigint;
  v_max_authorization_version bigint;
  v_building_id uuid;
BEGIN
  IF auth.uid() IS NULL
     OR p_organization_id IS NULL
     OR p_building_ids IS NULL
     OR cardinality(p_building_ids) = 0
     OR array_position(p_building_ids, NULL::uuid) IS NOT NULL THEN
    RAISE EXCEPTION 'Business performance access denied'
      USING ERRCODE = '42501';
  END IF;

  SELECT array_agg(DISTINCT requested.building_id ORDER BY requested.building_id)
    INTO v_building_ids
  FROM unnest(p_building_ids) AS requested(building_id);

  SELECT
    count(*),
    count(DISTINCT roster.building_id),
    min(roster.authorization_version),
    max(roster.authorization_version),
    COALESCE(
      jsonb_object_agg(
        roster.building_id::text,
        jsonb_build_object('restricted_allowed', roster.restricted_allowed)
      ),
      '{}'::jsonb
    )
    INTO
      v_roster_count,
      v_distinct_roster_count,
      v_min_authorization_version,
      v_max_authorization_version,
      v_roster
  FROM app_private.business_performance_authorized_buildings_v1(
    p_organization_id,
    COALESCE(p_require_restricted, false)
  ) AS roster;

  IF v_roster_count = 0
     OR v_roster_count <> v_distinct_roster_count
     OR v_min_authorization_version IS DISTINCT FROM v_max_authorization_version THEN
    RAISE EXCEPTION 'Business performance access denied'
      USING ERRCODE = '42501';
  END IF;

  FOREACH v_building_id IN ARRAY v_building_ids LOOP
    IF NOT (v_roster ? v_building_id::text)
       OR (
         COALESCE(p_require_restricted, false)
         AND NOT COALESCE(
           (v_roster -> v_building_id::text ->> 'restricted_allowed')::boolean,
           false
         )
       ) THEN
      RAISE EXCEPTION 'Business performance access denied'
  ...
  RETURN QUERY
  SELECT v_max_authorization_version, v_building_ids;
END;
$business_performance_exact_scope$;

REVOKE ALL ON FUNCTION app_private.business_performance_exact_scope_v1(uuid, uuid[], boolean) FROM PUBLIC;
REVOKE ALL ON FUNCTION app_private.business_performance_exact_scope_v1(uuid, uuid[], boolean) FROM anon;
REVOKE ALL ON FUNCTION app_private.business_performance_exact_scope_v1(uuid, uuid[], boolean) FROM authenticated;
REVOKE ALL ON FUNCTION app_private.business_performance_exact_scope_v1(uuid, uuid[], boolean) FROM service_role;
COMMENT ON FUNCTION app_private.business_performance_exact_scope_v1(uuid, uuid[], boolean) IS
  'Private all-or-nothing validator for a nonempty same-organization physical building set. Duplicate IDs are normalized before authorization.';

CREATE OR REPLACE FUNCTION public.business_performance_organizations_v1()
...
SECURITY DEFINER
SET search_path = pg_catalog, app_private, public
AS $business_performance_organizations$
BEGIN
  ...
  RETURN QUERY
  WITH candidate_organizations AS MATERIALIZED (
    SELECT DISTINCT
      o.id AS organization_id,
      COALESCE(NULLIF(btrim(o.name), ''), NULLIF(btrim(o.slug), ''), 'Unnamed organization') AS organization_name
  ...
  roster_rows AS MATERIALIZED (
    SELECT
      ...
      roster.restricted_allowed,
      roster.analysis_provenance,
      roster.authorization_version
    FROM candidate_organizations candidate
    CROSS JOIN LATERAL app_private.business_performance_authorized_buildings_v1(
      candidate.organization_id,
      true
    ) AS roster
  )
  SELECT
    ...
    jsonb_agg(
      jsonb_build_object(
        'id', roster.building_id,
        'name', roster.building_name,
        'restricted_allowed', roster.restricted_allowed,
        'analysis_provenance', roster.analysis_provenance
      )
      ORDER BY lower(roster.building_name) COLLATE "C",
               roster.building_name COLLATE "C",
               roster.building_id
    ) AS authorized_buildings,
    count(*)::bigint AS authorized_physical_building_count,
    max(roster.authorization_version) AS authorization_version
  FROM roster_rows roster
  GROUP BY roster.organization_id, roster.organization_name
  HAVING count(*) > 0
     AND count(*) = count(DISTINCT roster.building_id)
     AND min(roster.authorization_version) = max(roster.authorization_version)
  ORDER BY roster.organization_name COLLATE "C", roster.organization_id;
END;
$business_performance_organizations$;
...
CREATE OR REPLACE FUNCTION public.business_performance_snapshot_v1(
  p_organization_id uuid,
  p_building_ids uuid[]
)
...
SECURITY DEFINER
SET search_path = pg_catalog, app_private, public
AS $business_performance_snapshot$
DECLARE
  v_building_ids uuid[];
...
BEGIN
  ...
  FOR v_row IN
    SELECT * FROM public.fa_snapshot_kpis(p_building_ids => v_building_ids)
  LOOP
    IF v_row.building_id IS NULL
    ...
    rooms_available := v_row.rooms_available;
    rooms_occupied := v_row.rooms_occupied;
    rooms_reserved := v_row.rooms_reserved;
    rooms_maintenance := v_row.rooms_maintenance;
    rooms_unavailable := v_row.rooms_unavailable;
    vacancy_loss_month := v_row.vacancy_loss_month;
    active_contracts := v_row.active_contracts;
    avg_rent := v_row.avg_rent;
    deposit_held := v_row.deposit_held;
    receivable_total := v_row.receivable_total;
    aging_not_due := v_row.aging_not_due;
    aging_1_30 := v_row.aging_1_30;
    aging_31_60 := v_row.aging_31_60;
    aging_61_90 := v_row.aging_61_90;
    aging_over_90 := v_row.aging_over_90;
    RETURN NEXT;
  END LOOP;
...
BEGIN
  ...
  FOR v_row IN
    ...
    WHERE (vacancy.eff_end - p_as_of_date)
      BETWEEN 0 AND GREATEST(p_window_days, 0)
...
REVOKE ALL ON FUNCTION public.business_performance_organizations_v1() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.business_performance_organizations_v1() FROM anon;
REVOKE ALL ON FUNCTION public.business_performance_organizations_v1() FROM authenticated;
REVOKE ALL ON FUNCTION public.business_performance_organizations_v1() FROM service_role;
GRANT EXECUTE ON FUNCTION public.business_performance_organizations_v1() TO authenticated;
COMMENT ON FUNCTION public.business_performance_organizations_v1() IS
  'Lists active organizations and canonical physical buildings available to the authenticated Business Performance caller.';
...
COMMENT ON FUNCTION public.business_performance_pnl_v1(uuid, text, date, date, uuid[]) IS
  'Org-bound Business Performance P&L for authorized physical buildings. Requires analysis and restricted-finance access.';

REVOKE ALL ON FUNCTION public.business_performance_snapshot_v1(uuid, uuid[]) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.business_performance_snapshot_v1(uuid, uuid[]) FROM anon;
REVOKE ALL ON FUNCTION public.business_performance_snapshot_v1(uuid, uuid[]) FROM authenticated;
REVOKE ALL ON FUNCTION public.business_performance_snapshot_v1(uuid, uuid[]) FROM service_role;
GRANT EXECUTE ON FUNCTION public.business_performance_snapshot_v1(uuid, uuid[]) TO authenticated;
COMMENT ON FUNCTION public.business_performance_snapshot_v1(uuid, uuid[]) IS
  'Org-bound Business Performance finance snapshot for authorized physical buildings. Requires analysis and restricted-finance access.';
...
COMMENT ON FUNCTION public.business_performance_occupancy_snapshot_v1(uuid, date, uuid[]) IS
  'Org-bound occupancy snapshot for an exact authorized physical building set.';
...
COMMENT ON FUNCTION public.business_performance_upcoming_vacancy_v1(uuid, date, integer, uuid[]) IS
  'Org-bound upcoming-vacancy rows for an exact authorized physical building set.';
...
COMMENT ON FUNCTION public.business_performance_occupancy_monthly_v1(uuid, date, date, uuid[]) IS
  'Org-bound monthly occupancy trend for an exact authorized physical building set.';
