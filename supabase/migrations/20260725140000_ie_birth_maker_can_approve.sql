-- =============================================================================
-- Luật mới: NGƯỜI LẬP PHIẾU CÓ QUYỀN DUYỆT ⇒ PHIẾU TỰ DUYỆT NGAY
--
-- Chỉ đạo chủ sở hữu 2026-07-25:
--   "người lập phiếu nếu có quyền duyệt thì sẽ auto duyệt phiếu đó luôn"
--
-- TRƯỚC                                   SAU
--   hạng mục đặc biệt      -> Nháp          người lập CÓ quyền duyệt -> Duyệt luôn
--   CHI >= ngưỡng          -> Nháp          hạng mục đặc biệt        -> Nháp
--   còn lại                -> Duyệt         CHI >= ngưỡng            -> Nháp
--                                           còn lại                  -> Duyệt
--
-- Vì sao cần: sau khi gỡ income_expenses.approve khỏi vai trò "Quản Lý Tòa"
-- (20260725130000), chỉ chủ sở hữu còn quyền duyệt. Nếu chính chủ lập phiếu vượt
-- ngưỡng thì phiếu vào Nháp mà không còn ai khác duyệt được — luật này gỡ đúng
-- điểm đó, và đồng thời là quy tắc chung: ai được giao quyền duyệt thì phiếu của
-- người đó không phải chờ.
--
-- ⚠ HỆ QUẢ CẦN BIẾT: luật đặt TRƯỚC nhánh hạng mục đặc biệt, nên phiếu thuộc
--   nhóm force_approval (hoàn cọc/thanh lý/hoa hồng/thưởng...) do người CÓ quyền
--   duyệt lập cũng sẽ tự duyệt. Hiện chỉ chủ sở hữu có quyền này. Nếu muốn nhóm
--   đặc biệt luôn phải qua Nháp kể cả với chủ sở hữu, đảo thứ tự hai nhánh đầu.
--
-- Quyền được kiểm bằng mô hình canonical (app_private.can_v3 / has_any_scope_v3),
-- KHÔNG đọc JSONB legacy — nên luật này đã đứng trên hệ mới ngay từ bây giờ.
--
-- Sinh từ pg_get_functiondef của bản ĐANG CHẠY; chỉ thay đúng khối quyết định
-- trạng thái sinh và thêm 1 biến. Không đổi gì khác trong hàm.
-- =============================================================================
