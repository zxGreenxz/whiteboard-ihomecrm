-- =============================================================================
-- get_public_available_rooms: thêm phòng "khách SẮP CHUYỂN ĐI" vào danh sách
-- (status_public='soon'), giống "sắp hết hạn hợp đồng".
-- =============================================================================
-- BASE: 20260617100100 (bản mới nhất — có pass + pass_avail_date). Chỉ đổi LOGIC
-- "soon" + "avail_date" để gộp thêm HĐ đã ĐĂNG KÝ NGÀY CHUYỂN ĐI:
--
--   "Sắp chuyển đi" = HĐ còn ACTIVE nhưng khách đã đăng ký expected_move_out_date
--   (RegisterMoveOutDialog → contracts.expected_move_out_date). Đây là tín hiệu
--   trống phòng CHẮC CHẮN (khách đã hẹn ngày đi), thậm chí mạnh hơn "sắp hết hạn".
--
-- Quy ước mới (áp cho cả EXISTS phân loại lẫn subquery tính avail_date):
--   1 phòng là 'soon' nếu có HĐ ACTIVE/EXTENDED thoả MỘT trong hai:
--     (a) expected_move_out_date >= CURRENT_DATE   → khách sắp chuyển đi
--         (KHÔNG giới hạn cửa sổ v_soon: đã hẹn ngày đi thì luôn hiện, kèm ngày).
--     (b) COALESCE(actual_end_date,end_date) trong [hôm nay; hôm nay + v_soon]
--         → sắp hết hạn (giữ NGUYÊN logic cũ).
--   avail_date = ngày phòng dự kiến trống: ưu tiên expected_move_out_date (nếu hợp
--   lệ), ngược lại COALESCE(actual_end_date,end_date). Lấy MIN qua các HĐ thoả.
--
-- FE không đổi: status_public='soon' đã hiển thị "Sắp trống <ngày>" sẵn.
-- KHÔNG sửa migration cũ — chỉ CREATE OR REPLACE (giữ nguyên grants anon/auth).
-- =============================================================================
  ...
  WITH rms AS (
    SELECT
      ...
      CASE
        ...
        WHEN EXISTS (
          ...
          WHERE c.room_id = rm.id
            ...
            AND (
              -- (a) khách đã đăng ký ngày chuyển đi (sắp chuyển đi)
              (c.expected_move_out_date IS NOT NULL AND c.expected_move_out_date >= CURRENT_DATE)
              -- (b) sắp hết hạn hợp đồng trong cửa sổ v_soon
      ...
      (
        SELECT MIN(
          CASE
            WHEN c.expected_move_out_date IS NOT NULL AND c.expected_move_out_date >= CURRENT_DATE
