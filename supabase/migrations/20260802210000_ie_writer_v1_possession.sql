-- WRITER TẠO PHIẾU TAY CHUYỂN SANG POSSESSION
--
-- `create_income_expense_v1` là đường ghi tiền cuối cùng còn kiểm quyền sổ bằng
-- bảng cũ `account_shared_users`. Chú thích trong chính thân hàm đã tự thú:
-- "Cashbook ownership/sharing remains semantically authoritative until the
-- normalized cashbooks.post resolver is complete" — nên KNOWER/CUSTODIAN hoàn
-- toàn vô hình với nó, và FE phải đi đường vòng compat (mutations.ts:98-117).
--
-- Đo trên prod trước khi vá:
--   * 25 ô (người × sổ) có possession CUSTODIAN mà v1 từ chối oan — chủ yếu là
--     chủ tổ chức trên chính các sổ mình giữ.
--   * 23 ô mà joey/nathan đang ghi được NHỜ dòng share cũ; xoá bảng cũ trước khi
--     vá hàm này là cắt mất quyền tạo phiếu của họ. Vì vậy vá hàm TRƯỚC, xoá sau.
--
-- Vá theo NEO (3 khối), không chép tay 3.6k dòng — thân hàm lấy tươi từ prod
-- bằng pg_get_functiondef (án lệ "thân hàm prod lệch file migration").
-- Luật giữ khớp với `ie_compat_insert_v2` §9.2 để hai đường tạo phiếu không
-- lệch nhau: CUSTODIAN/OPERATOR làm mọi loại, KNOWER chỉ Phiếu thu.
--
-- Bỏ `FOR SHARE` trên dòng phân quyền: hệ possession thu hồi bằng cách đóng
-- `valid_to` chứ không xoá dòng, nên hai lần hậu kiểm phía dưới đã đủ chặn
-- revoke chạy song song — và tránh luôn án lệ 25006 (khoá dòng trong hàm đọc).
--
