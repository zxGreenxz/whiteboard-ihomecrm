-- =============================================================================
-- get_public_available_rooms: nhánh "sắp chuyển đi" CŨNG tuân cửa sổ soon_days
-- =============================================================================
-- BASE: 20260627120000_public_rooms_include_moveout.sql
--
-- ĐỔI Ý so với bản trước: trước đây HĐ có expected_move_out_date >= CURRENT_DATE
-- thì LUÔN hiện "Sắp trống" bất kể còn bao xa. Nay theo yêu cầu, nhánh (a) chỉ
-- tính 'soon' khi ngày chuyển đi NẰM TRONG cửa sổ [hôm nay; hôm nay + v_soon],
-- giống nhánh (b) sắp hết hạn. Tức phòng chỉ lên công khai khi sắp trống thật
-- trong vòng soon_days ngày.
--
--   (a) expected_move_out_date BETWEEN CURRENT_DATE AND CURRENT_DATE + v_soon
--   (b) COALESCE(actual_end_date, end_date) BETWEEN ... (giữ nguyên)
--
-- FE không đổi. KHÔNG sửa migration cũ — chỉ CREATE OR REPLACE (giữ grants).
-- =============================================================================
      ...
      CASE
        ...
        WHEN EXISTS (
          ...
          WHERE c.room_id = rm.id
            ...
            AND (
              -- (a) khách đã đăng ký ngày chuyển đi, TRONG cửa sổ soon_days
...
-- =============================================================================
-- get_my_available_rooms: đồng bộ y hệt thay đổi nhánh (a) ở trên (bản in-app).
-- =============================================================================
