-- =============================================
-- Migration: get_invoice_statistics hỗ trợ staff
-- Created: 2026-05-16
-- Description:
--   Trước đây RPC luôn lọc theo i.user_id = p_user_id (FE truyền auth.uid()).
--   Với staff, auth.uid() ≠ owner_id (invoices thuộc về owner), nên stats về 0.
--
--   Sửa: RPC tự suy ra "effective owner":
--   - Super admin → dùng p_user_id (cho phép xem owner bất kỳ).
--   - Nếu caller chính là owner (có invoice user_id = caller) → dùng caller.
--   - Ngược lại, lookup staff_assignments WHERE staff_id = caller → user_id (owner).
--   - Đồng thời giới hạn theo các building_id được gán cho staff (NULL = tất cả).
-- =============================================
...
BEGIN
  ...
  -- Caller NULL (service_role / Management API) → fallback dùng p_user_id trực tiếp.
  ...
  ELSE
    ...
    IF v_is_super THEN
      ...
      v_staff_buildings := NULL;
    ELSE
    -- Owner? Có ít nhất 1 invoice do chính caller tạo.
    IF EXISTS (SELECT 1 FROM invoices WHERE user_id = v_caller LIMIT 1) THEN
      v_owner := v_caller;
      v_staff_buildings := NULL;
    ELSE
      -- Tìm owner qua staff_assignments
      ...
      IF v_owner IS NULL THEN
        -- Không phải owner, không phải staff → trả 0
        RETURN json_build_object(
          'total_amount', 0, 'total_paid', 0, 'total_remaining', 0,
          'total_refunded', 0, 'total_count', 0,
          'rent_amount', 0, 'electric_amount', 0, 'water_amount', 0, 'pdv_amount', 0,
          'total_collected', 0, 'payment_tm', 0, 'payment_tk', 0, 'payment_tt', 0,
          'change_amount', 0
        );
      END IF;

      -- Staff: nếu có 1 assignment building_id IS NULL → toàn quyền owner's buildings
      IF EXISTS (
  ...
  -- 2) Aggregates trên bảng invoices (đã filter)
  ...
  -- 3) Breakdown Điện/Nước/PDV theo description (khớp UI bảng)
  SELECT
  ...
  -- 4) Tổng tiền thu + chia theo phương thức (TM/TK/TT)
  SELECT
  ...
  -- 5) Tiền Thối = SUM(income_expenses.change_amount)
...
COMMENT ON FUNCTION get_invoice_statistics IS
  'Invoice stats với hỗ trợ staff: tự suy ra effective owner & building scope từ staff_assignments. Super admin xem toàn bộ. Owner xem của mình. Staff bị giới hạn theo building được gán (building_id IS NULL = full).';
