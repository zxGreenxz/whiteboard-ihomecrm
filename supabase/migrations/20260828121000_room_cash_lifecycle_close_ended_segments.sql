-- =====================================================================
-- Forward-fix cho 20260828120000 — hai lỗi lộ ra khi smoke trên phòng thật:
--
-- 1. SEGMENT KHÔNG ĐÓNG KHI HỢP ĐỒNG KẾT THÚC. get_room_residence_segments_v1
--    (Task 0) chỉ mô hình mốc CHUYỂN PHÒNG; hợp đồng thanh lý/hết hạn không có
--    transfer nào nên segment cuối mãi mãi to_date NULL = "đang còn ở". Phòng
--    401 (org thật): 6 hợp đồng ĐÃ kết thúc mà timeline vẽ 6 thanh chồng nhau
--    chạy tới hôm nay, 0 khoảng trống. Fix: to_date NULL thì đóng bằng
--    actual_end_date, hoặc end_date khi hợp đồng đã TERMINATED/EXPIRED.
--
-- 2. VACANCY GÃY VỚI SEGMENT CHỒNG LẤN. lead(from_date) bắt cặp với to_date
--    của ĐÚNG DÒNG TRƯỚC — hai segment chồng nhau (dữ liệu bẩn) sẽ sinh khoảng
--    trống ma. Fix: chuẩn island — running max của to_date (NULL coi là
--    infinity), chỉ nhận gap khi next_from vượt mọi to_date đã thấy.
-- =====================================================================
BEGIN;
...
BEGIN
  IF to_regprocedure('public.get_room_cash_lifecycle_v1(uuid,date,date)') IS NULL THEN
    RAISE EXCEPTION 'Thiếu get_room_cash_lifecycle_v1 — chạy 20260828120000 trước. DỪNG.';
  END IF;
END
$preflight$;
...
BEGIN
  ...
  WITH seg AS (
    ...
    -- có mốc chuyển đi" — nhưng hợp đồng đã kết thúc thì thanh phải ĐÓNG tại
    -- ngày kết thúc, không phải chạy tới vô cùng (fix #1).
  ...
  vac AS (
    SELECT ss.run_max_to AS from_date, ss.next_from AS to_date,
