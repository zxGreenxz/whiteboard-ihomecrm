-- 20260727120000_ie_income_auto_approve_and_kqkd_guard.sql
--
-- MỤC TIÊU (chủ sở hữu chốt 27/07/2026)
--   1. MỌI phiếu THU tự duyệt + ghi sổ ngay khi tạo, kể cả hạng mục "Tiền Cọc"
--      bên thu. Phiếu CHI giữ nguyên: cờ force_approval (hoàn cọc/thanh lý/hoa
--      hồng/lương/LN) và ngưỡng chi (app_private.ie_auto_approve_config).
--   2. Hạng mục thu "Tiền Cọc" KHÔNG phải luôn sinh từ hợp đồng => system_only=false.
--   3. Hạng mục tạo tay từ form Thu/Chi bị thiếu organization_id => phiếu dùng nó
--      luôn kẹt "Chờ duyệt". Backfill + trigger tự gắn tổ chức.
--
-- VÌ SAO CẦN (đo trên production 27/07/2026)
--   Phiếu "thay phao bơm bồn phụ 405pvb" 250.000đ < ngưỡng 300.000đ mà vẫn Chờ
--   duyệt. Tái hiện được: hạng mục "thay phao bơm" tạo lúc 05:18 có
--   organization_id = NULL => create_income_expense_v1 raise 42501 "Loại hạng mục
--   1 không thuộc tổ chức hoặc sai chiều thu/chi" => FE nuốt lỗi, rơi sang
--   ie_compat_insert_v2 (ép UNAPPROVED, không có bước xét ngưỡng).
--
--   Ngoài ra guard "trường tài chính suy diễn" của v1 tính kỳ vọng SAI: khi caller
--   truyền p_business_result_accounting = NULL nó giả định (counts=true,
--   kqkd=total), trong khi public.recompute_ie_business_result suy KQKD từ
--   accounting_class của item (hạng mục cọc => DEPOSIT => kqkd=0, counts=false).
--   Mọi phiếu có hạng mục cọc đều văng 23514 => đó là lý do "Tiền Cọc" từng bị
--   gắn system_only để né writer canonical. Sửa guard cho khớp thì bỏ được cờ đó.

BEGIN;

-- ---------------------------------------------------------------------------
-- 1. Backfill hạng mục mồ côi tổ chức (3 dòng trên production 27/07/2026)
-- ---------------------------------------------------------------------------
UPDATE public.income_expense_types t
  ...
  FROM (
    SELECT t2.id,
           COALESCE(
             (SELECT ie.organization_id
                FROM public.income_expense_items i
                JOIN public.income_expenses ie ON ie.id = i.income_expense_id
               WHERE i.income_expense_type_id = t2.id
                 AND ie.organization_id IS NOT NULL
               ORDER BY ie.created_at
               LIMIT 1),
             (SELECT (array_agg(DISTINCT m.organization_id))[1]
                FROM public.organization_memberships m
               WHERE m.user_id = t2.user_id AND m.status = 'ACTIVE'
...
-- ---------------------------------------------------------------------------
-- 2. Lưới an toàn: mọi đường INSERT hạng mục đều được gắn tổ chức.
--    _autofill_org đã generic (suy từ user_id khi user thuộc đúng 1 org ACTIVE);
--    income_expense_types có cột user_id nên dùng lại được nguyên trạng.
-- ---------------------------------------------------------------------------
DROP TRIGGER IF EXISTS trg_autofill_org ON public.income_expense_types;
CREATE TRIGGER trg_autofill_org
  BEFORE INSERT ON public.income_expense_types
  FOR EACH ROW EXECUTE FUNCTION public._autofill_org();

-- ---------------------------------------------------------------------------
-- 3. "Tiền Cọc" bên THU không bắt buộc sinh từ hợp đồng.
--    CHỈ đụng hạng mục type='income'; bên chi (Hoàn cọc thanh lý, Cấn cọc
--    chuyển doanh thu, Hoa hồng môi giới…) giữ nguyên system_only.
-- ---------------------------------------------------------------------------
UPDATE public.income_expense_types
   SET system_only = false
 WHERE type = 'income' AND system_only IS TRUE;

COMMIT;

-- ---------------------------------------------------------------------------
-- 4. create_income_expense_v1 — vá tại chỗ, sinh từ định nghĩa sống 27/07/2026.
--    Diff so với bản cũ: (a) thêm v_stored_pnl_sum, (b) SELECT đối chiếu tính
--    tổng PNL, (c) kỳ vọng kqkd/counts mirror recompute_ie_business_result,
--    (d) thang sinh trạng thái thêm nhánh p_type = 'INCOME' -> APPROVED.
-- ---------------------------------------------------------------------------
