-- =============================================================================
-- PANALYTICS-C02 (P2) — re-anchor bảo mật 02/09/2026: `log_public_room_events`
-- cấp EXECUTE cho `anon` và chỉ có trần 50 sự kiện MỖI LỜI GỌI — không có trần
-- theo token/ngày, không có retention. Người ngoài lặp lời gọi để bơm bảng
-- `public_room_events` lớn vô hạn (tốn dung lượng, làm chậm mọi báo cáo `pra_*`).
--
-- VÁ, hai lớp:
--   1. NGÂN SÁCH THEO TOKEN/NGÀY — bảng đếm `app_private.public_room_event_budgets`
--      + kiểm trần ngay trong logger. Vượt trần → trả 0 (im lặng như mọi nhánh
--      từ chối khác của hàm này; không lộ nội bộ cho anon).
--      Trần 5.000 dòng/token/ngày. Đo prod 02/09: 16.329 dòng trong 73 ngày trên
--      3 token ≈ 75 dòng/token/ngày ⇒ trần rộng gấp ~60 lần mức thật, không cắt
--      nhầm ai. Ngày tính theo giờ Việt Nam để khớp cách báo cáo đọc dữ liệu.
--   2. RETENTION 90 NGÀY — hàm `app_private.pra_prune_public_room_events_v1(days)`
--      + cron job hằng ngày. Đo prod: dữ liệu cũ nhất 21/06 (73 ngày) ⇒ lần chạy
--      đầu xoá 0 dòng; đây là chốt chặn tương lai, không phải cú xoá lịch sử.
--
-- Chữ ký logger GIỮ NGUYÊN `(text, jsonb)` ⇒ ACL definer baseline không đổi.
-- Thân hàm chép NGUYÊN VĂN bản 20260831023937, chỉ CHÈN phần ngân sách.
-- =============================================================================

BEGIN;
SET LOCAL lock_timeout = '15s';

-- ── 1. Bảng đếm ngân sách ────────────────────────────────────────────────────
...
-- Cron 03:41 hằng đêm (giờ máy chủ) — lệch giờ với 6 job sẵn có để không chồng tải.
SELECT cron.unschedule('pra_prune_public_room_events_v1')
 WHERE EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'pra_prune_public_room_events_v1');
