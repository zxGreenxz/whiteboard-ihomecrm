-- Forward-only correction: org_wide must never bypass a scoped DENY.
BEGIN;
...
comment on function app_private.authorized_scope_v3(text, uuid) is
  'Shared permission scope resolver: scoped DENY keeps org_wide false and returns only effective dimensions.';
