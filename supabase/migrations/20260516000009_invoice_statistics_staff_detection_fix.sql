-- =============================================
-- Migration: Fix staff detection trong get_invoice_statistics
-- Created: 2026-05-16
-- Description:
--   Trước đó RPC dùng "EXISTS invoices WHERE user_id = v_caller" để xác định
--   caller có phải owner hay không. Sai khi caller từng có invoice (soft-deleted)
--   trong quá khứ — caller bị nhận diện nhầm là owner và stats về 0.
--
--   Fix: đảo ưu tiên — kiểm tra staff_assignments trước.
--   - Nếu caller có row trong staff_assignments với user_id ≠ caller
--     → caller là STAFF của owner đó.
--   - Ngược lại → caller là owner chính dữ liệu của mình.
-- =============================================
...
BEGIN
  ...
  IF v_caller IS NULL THEN
    -- Service role / Management API: trust p_user_id.
    v_owner := p_user_id;
  ...
  ELSE
    ...
    IF v_is_super THEN
      ...
      v_staff_buildings := NULL;
    ELSE
      -- Ưu tiên detect STAFF: caller có row staff_assignments với user_id ≠ caller
      ...
      IF v_owner IS NOT NULL THEN
        ...
        END IF;
      ELSE
        -- Không phải staff → caller là chủ dữ liệu của mình.
        v_owner := v_caller;
        v_staff_buildings := NULL;
      END IF;
    END IF;
...
COMMENT ON FUNCTION get_invoice_statistics IS
  'Invoice stats. Detect staff trước (theo staff_assignments với user_id ≠ caller), nếu không phải staff thì caller là owner. Tránh false-positive khi owner-test invoice trước đó đã soft-delete.';
