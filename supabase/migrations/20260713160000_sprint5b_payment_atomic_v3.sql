-- =============================================================================
-- Sprint 5b — record_invoice_payment_v3: payment + invoice + voucher + items ATOMIC.
-- (AUTHORIZATION-PLAN §8.1 usePayments/useInvoicePayments, §13 record_invoice_payment_atomic)
--
-- Vá double-payment: v2 chỉ atomic payment+invoice; FE mirror voucher/items NGOÀI
-- RPC ⇒ RPC ok + mirror lỗi + retry = payment ĐÔI. v3 gộp mọi thứ 1 transaction +
-- idempotency_key ⇒ retry sau commit trả kết quả cũ, không nhân đôi. Split
-- revenue/deposit vẫn do FE tính (logic đã kiểm), truyền qua p_voucher/p_items.
--
-- SECURITY DEFINER ⇒ guard cột tài chính (Sprint 5a) bypass (owner) nên set
-- approved_by=auth.uid() hợp lệ. RBAC qua can_do_on_building invoices.edit.
-- =============================================================================
...
BEGIN
  ...
  IF p_idempotency_key IS NULL OR length(btrim(p_idempotency_key)) < 8 THEN
    RAISE EXCEPTION 'idempotency_key bắt buộc (>=8 ký tự)';
  END IF;

  -- Idempotency: nếu đã có voucher với key này (retry sau commit) → trả kết quả cũ.
  SELECT ie.id AS voucher_id, ie.payment_id INTO v_existing
  FROM income_expenses ie
  WHERE ie.idempotency_key = p_idempotency_key AND ie.deleted_at IS NULL
  LIMIT 1;
  IF FOUND THEN
    RETURN json_build_object('payment_id', v_existing.payment_id, 'voucher_id', v_existing.voucher_id, 'idempotent', true);
  END IF;

  -- Lock invoice + RBAC.
  SELECT id, user_id, building_id, contract_id, total_amount, paid_amount, status
  INTO v_inv FROM invoices
  ...
  IF v_new_paid >= v_inv.total_amount THEN
    ...
    IF v_excess > 0 THEN
      ...
      VALUES (v_inv.contract_id, v_excess,
  ...
  -- Voucher (income) — chỉ khi có account_id (mirror phiếu thu). Payload từ FE.
  IF p_account_id IS NOT NULL AND p_voucher IS NOT NULL THEN
    INSERT INTO income_expenses (
      ...
      creator_name, business_result_accounting, change_amount, change_account_id,
      rounding_amount, rounding_account_id, idempotency_key
    ) VALUES (
      v_caller, 'INCOME',
      COALESCE(p_voucher->>'name', 'Thu tiền hoá đơn'),
      v_inv.building_id,
      NULLIF(p_voucher->>'room_id','')::uuid,
      v_inv.contract_id,
      p_account_id,
      p_invoice_id,
      v_payment_id,
      p_payment_date,
      p_voucher->>'payer_name',
      p_voucher->>'notes',
      COALESCE(p_voucher->'attachments', '[]'::jsonb),
      'APPROVED', v_caller, now(),
      p_voucher->>'creator_name',
      ...
      COALESCE((p_voucher->>'change_amount')::numeric, 0),
      NULLIF(p_voucher->>'change_account_id','')::uuid,
      COALESCE((p_voucher->>'rounding_amount')::numeric, 0),
      NULLIF(p_voucher->>'rounding_account_id','')::uuid,
      p_idempotency_key
    ...
    IF p_items IS NOT NULL THEN
      FOR it IN SELECT * FROM jsonb_array_elements(p_items) LOOP
        ...
        VALUES (v_voucher_id, (it->>'income_expense_type_id')::uuid, it->>'description',
                COALESCE((it->>'quantity')::numeric,1), (it->>'unit_price')::numeric,
                NULLIF(it->>'start_date','')::date, NULLIF(it->>'end_date','')::date);
      END LOOP;
    END IF;
  END IF;

  RETURN json_build_object('payment_id', v_payment_id, 'voucher_id', v_voucher_id,
    'new_paid_amount', v_new_paid, 'new_status', v_status, 'excess_amount', v_excess);
END;
$fn$;

REVOKE ALL ON FUNCTION public.record_invoice_payment_v3(uuid,numeric,payment_method,date,text,uuid,text,text,jsonb,jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.record_invoice_payment_v3(uuid,numeric,payment_method,date,text,uuid,text,text,jsonb,jsonb) TO authenticated;
