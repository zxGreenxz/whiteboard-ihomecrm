-- =============================================
-- Migration: Thêm ô "Khuyến mãi" cho phòng (rooms.sale_note) + expose ở RPC public.
-- sale_note = chính sách sale/promo riêng của phòng (vd "Thưởng sale 500k",
-- "HĐ 14 tháng giảm 300k suốt hợp đồng"). Tách khỏi description (mô tả/ghi chú phòng).
-- CREATE OR REPLACE get_public_available_rooms để trả thêm rm.sale_note (qua to_jsonb).
-- =============================================

ALTER TABLE public.rooms ADD COLUMN IF NOT EXISTS sale_note text;
