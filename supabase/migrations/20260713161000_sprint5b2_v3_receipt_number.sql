-- =============================================================================
-- Sprint 5b2 — Thêm p_receipt_number vào record_invoice_payment_v3 (để hợp nhất
-- useCreatePayment/CollectPaymentDialog vốn set receipt_number). DROP+CREATE vì
-- đổi signature. Named-param call cũ (useInvoicePayments) vẫn chạy (default null).
-- =============================================================================
BEGIN;

DROP FUNCTION IF EXISTS public.record_invoice_payment_v3(uuid,numeric,payment_method,date,text,uuid,text,text,jsonb,jsonb);
...
BEGIN
  ...
  WHERE ie.idempotency_key = p_idempotency_key AND ie.deleted_at IS NULL LIMIT 1;
  IF FOUND THEN
  ...
  IF p_account_id IS NOT NULL AND p_voucher IS NOT NULL THEN
    INSERT INTO income_expenses (
      ...
      COALESCE((p_voucher->>'rounding_amount')::numeric, 0), NULLIF(p_voucher->>'rounding_account_id','')::uuid,
      p_idempotency_key
...
REVOKE ALL ON FUNCTION public.record_invoice_payment_v3(uuid,numeric,payment_method,date,text,uuid,text,text,jsonb,jsonb,text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.record_invoice_payment_v3(uuid,numeric,payment_method,date,text,uuid,text,text,jsonb,jsonb,text) TO authenticated;
