-- =============================================================================
-- Profit Close V2 forward fix: ignore rules owned by inactive/deleted managers.
--
-- Production contains an active legacy salary rule and mappings for a manager
-- who is already soft-deleted, with legacy NULL organization_id values. The
-- established calculation excludes that manager. This migration aligns preview
-- validation without changing manager/rule/mapping data.
--
-- Missing managers still fail. For an active, non-deleted manager, the manager,
-- active rule and mapping must all have the current organization_id.
-- =============================================================================
...
BEGIN
  ...
  -- Active rules that reference no manager are structurally corrupt. Rules for
  -- an existing inactive/soft-deleted manager are legacy history and ignored,
  -- including their legacy NULL organization_id fields. A live manager's active
  -- rule and mapping must all be explicitly scoped to this organization.
  IF EXISTS (
    SELECT 1
    ...
    RAISE EXCEPTION
      'Management salary configuration contains a missing manager or live cross/null-organization rows'
      USING ERRCODE = '55000';
  END IF;
...
COMMENT ON FUNCTION public._profit_close_preview_core_v2(
  uuid,date,uuid[],jsonb,text,boolean
) IS 'Profit Close V2 canonical preview core. Inactive/deleted manager rules and inactive/deleted shareholder mappings are ignored; missing IDs and live cross/null-org configuration fail closed.';
