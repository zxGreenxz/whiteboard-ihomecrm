-- =============================================
-- Migration: award_job_bonus tra them cot `time_context` cho popup thuong
--   chon SKIN theo ngu canh thoi gian hoan thanh viec:
--     'AFTER_HOURS' (ngoai gio thuong ngay, sau afterHourMark) → popup 4A (dem/indigo)
--     'SUNDAY'      (Chu Nhat / cuoi tuan)                       → popup 4B (huan chuong)
--     'HOLIDAY'     (ngay Le / Tet)                              → popup 4C (tri an, do)
--     'NORMAL'      (gio hanh chinh ngay thuong)                 → popup don xanh (mac dinh)
--   Uu tien HOLIDAY > SUNDAY > AFTER_HOURS > NORMAL. Tra tren CA 2 dong (JOB + DAY_BONUS),
--   cung 1 gia tri (cung job/ngay). Combo (>=2 khoan) GIU thiet ke vang, bo qua time_context.
--   Doi return signature → DROP truoc roi CREATE. Logic thuong GIU NGUYEN (20260630000003).
-- =============================================
