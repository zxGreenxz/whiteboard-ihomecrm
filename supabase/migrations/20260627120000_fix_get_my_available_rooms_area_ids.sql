-- get_my_available_rooms: đồng bộ với get_public_available_rooms.
--
-- Lý do: hàm cũ còn tham chiếu buildings.area_id (đã DROP khi chuyển sang N-N
-- area_buildings) → RPC trả 400, tab "Phòng trống" in-app rỗng. Đồng thời hàm cũ
-- thiếu các nâng cấp đã có ở bản công khai: overlay phòng khách nhờ sale ('pass'),
-- suy 'soon' theo expected_move_out_date, các field liên hệ/elec_rate, area_ids N-N.
--
-- Bản này GIỮ phần suy owner từ caller (owner/super → chính mình; staff →
-- staff_assignments.user_id) rồi dùng CÙNG khối truy vấn với get_public_available_rooms.
