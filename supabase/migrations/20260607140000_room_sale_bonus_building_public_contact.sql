-- Phòng trống / Sale: tách "Thưởng sale" khỏi "Khuyến mãi" + liên hệ & chỉ đường riêng từng tòa
--  • rooms.sale_bonus_note   : ô "Thưởng sale" (nội bộ sale) — TÁCH khỏi sale_note (Khuyến mãi khách)
--  • buildings.public_contact_name / public_contact_phone : liên hệ QL riêng từng tòa (gọi/Zalo)
--  • buildings.public_map_url : link Google Maps "Chỉ đường" riêng từng tòa
-- Cập nhật RPC public get_public_available_rooms để trả thêm các field trên.

ALTER TABLE public.rooms
  ADD COLUMN IF NOT EXISTS sale_bonus_note text;

ALTER TABLE public.buildings
  ADD COLUMN IF NOT EXISTS public_contact_name  text,
  ADD COLUMN IF NOT EXISTS public_contact_phone text,
  ADD COLUMN IF NOT EXISTS public_map_url       text;
...
BEGIN
  ...
  WITH rms AS (
    SELECT
      ...
      rm.description,
      rm.sale_note,
      rm.sale_bonus_note,                                 -- NEW: Thưởng sale (nội bộ)
      rm.room_type,
      CASE
        WHEN EXISTS (
  ...
  blds_j AS (
    SELECT jsonb_agg(jsonb_build_object(
      ...
      'public_contact_name',  b.public_contact_name,      -- NEW: liên hệ QL riêng tòa
      'public_contact_phone', b.public_contact_phone,
      'public_map_url',       b.public_map_url            -- NEW: link Chỉ đường riêng tòa
