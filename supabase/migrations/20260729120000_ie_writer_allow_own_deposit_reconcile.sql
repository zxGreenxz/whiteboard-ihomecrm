-- =============================================================================
-- Phiếu thu CỌC bị writer canonical tự bắn 23514 — nới đúng 2 khe hậu kiểm
-- =============================================================================
-- BỆNH (NATHAN báo 29/07/2026, phòng 405 toà 1392QT):
--   Thu chi → Thêm phiếu → hạng mục "Tiền Cọc" + chọn phòng → bấm Lưu ra toast
--   "Writer phiếu nháp tổng quát đã tác động trạng thái giữ phòng" và KHÔNG có
--   phiếu nào được tạo. Không phải thao tác sai — đây là hồi quy trong code.
--
-- CHUỖI NHÂN QUẢ (đã tái hiện thật trên production bằng transaction rollback):
--   1. 20260727120000_ie_income_auto_approve_and_kqkd_guard bỏ cờ system_only
--      của hạng mục "Tiền cọc" => lần đầu tiên phiếu cọc ĐI QUA create_income_expense_v1.
--   2. Item cọc làm trigger trg_ie_items_reconcile_room chạy
--      recompute_room_reservation => phòng AVAILABLE -> RESERVED (đúng thiết kế
--      của 20260608000000: cọc giữ chỗ thì khoá phòng).
--   3. Nhưng writer có HAI hậu kiểm viết từ thời phiếu cọc CHƯA BAO GIỜ vào được
--      đây: "rooms.status/updated_at không được đổi" và "contracts.deposit_paid/
--      updated_at không được đổi". Chính hiệu ứng bắt buộc của phiếu cọc bị hậu
--      kiểm coi là writer đụng nhầm state cũ => RAISE 23514 => rollback cả phiếu.
--   FE (mutations.ts) coi 23514 là lỗi THẬT (đúng), nên nổi thẳng thành toast.
--
-- ĐO ĐƯỢC trên production 29/07/2026 (probe rollback, không ghi gì):
--   A. item cọc + phòng AVAILABLE          -> ERR 23514 "…giữ phòng"        (ca của NATHAN)
--   B. item thường + phòng AVAILABLE       -> OK
--   C. item cọc + phòng OCCUPIED (có HĐ)   -> OK  (recompute return sớm)
--   D. item cọc + không chọn phòng         -> OK
--   E. item cọc + có gắn HĐ                -> ERR 23514 "…tiền cọc hợp đồng"
--   F. item thường + có gắn HĐ             -> OK
--   => hỏng ĐÚNG 2 ca A và E. Ba màn đang chết vì A: Thu chi/Thêm phiếu,
--      "Tạo phiếu cọc giữ chỗ" (CreateDepositDialog), "Cọc nhanh" ở Phòng trống
--      (QuickDepositModal). Ca E chết ở Thu chi khi phòng còn HĐ THIẾU cọc
--      (form tự gắn HĐ vào phiếu).
--
-- CÁCH VÁ — nới ĐÚNG khe, không tháo hậu kiểm:
--   • Phòng: cho phép DUY NHẤT AVAILABLE -> RESERVED, và chỉ khi phiếu này là
--     phiếu THU có item hạng mục cọc, chưa gắn HĐ, VÀ trạng thái mới khớp
--     predicate chuẩn public.room_has_holding_deposit(). RESERVED -> AVAILABLE,
--     đụng OCCUPIED/MAINTENANCE, hay chỉ bump updated_at: vẫn abort như cũ.
--   • Hợp đồng: cho phép deposit_paid đổi khi phiếu có item cọc, VÀ giá trị sau
--     cùng phải bằng SỐ DẪN XUẤT chuẩn (không phải số tuỳ ý).
--   Phiếu KHÔNG có item cọc: hành vi y hệt trước, cấm tuyệt đối (ca B/F).
--
--   Để hậu kiểm hợp đồng khỏi chép lại công thức (2 bản dễ lệch), tách số dẫn
--   xuất thành helper public.contract_deposit_paid_derived() — đúng mẫu
--   room_has_holding_deposit() đã tách ở 20260727120000_public_rooms_hide_held_by_deposit.
--   recompute_contract_deposit_paid gọi helper: HÀNH VI KHÔNG ĐỔI (đã đối chiếu
--   toàn bộ 330 hợp đồng trên production trước/sau: 0 dòng lệch).
--
-- BASE: pg_get_functiondef bản ĐANG CHẠY (md5 02425b2cb5506fd7c1ab043a227ec102);
--       chỉ đổi 4 chỗ (5 biến mới, 1 cột trong câu SELECT tổng hợp item, 2 khối
--       hậu kiểm). Không đụng gì khác. KHÔNG sửa migration cũ.
-- =============================================================================

BEGIN;

-- ---------------------------------------------------------------------------
-- 1) Helper: SỐ CỌC DẪN XUẤT của hợp đồng (tách y nguyên khỏi
--    recompute_contract_deposit_paid — một nguồn sự thật cho cả writer lẫn recompute).
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.contract_deposit_paid_derived(p_contract_id uuid)
RETURNS numeric
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT GREATEST(
   ...
   WHERE p_contract_id IS NOT NULL
     AND (voucher.contract_id = p_contract_id OR link_row.id IS NOT NULL);
$$;

COMMENT ON FUNCTION public.contract_deposit_paid_derived(uuid) IS
'Số "Cọc đã thu" DẪN XUẤT của hợp đồng: tổng item accounting_class=DEPOSIT trên
các phiếu ĐÃ DUYỆT gắn hợp đồng (trực tiếp hoặc qua contract_deposit_links), THU
cộng / CHI trừ, chặn dưới 0. Nguồn sự thật dùng chung cho
recompute_contract_deposit_paid (ghi cột) và hậu kiểm của create_income_expense_v1.';

-- Nội bộ: chỉ gọi từ trong function SECURITY DEFINER (án Sprint 0).
REVOKE ALL ON FUNCTION public.contract_deposit_paid_derived(uuid) FROM PUBLIC, anon, authenticated;

-- ---------------------------------------------------------------------------
-- 2) recompute_contract_deposit_paid dùng helper (HÀNH VI KHÔNG ĐỔI).
--    Vẫn giữ nguyên điều kiện "không có dòng cọc nào thì KHÔNG ghi" — cột giữ
--    số cũ, đúng như bản đang chạy.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3) create_income_expense_v1 — hai hậu kiểm biết phân biệt phiếu cọc.
-- ---------------------------------------------------------------------------
