-- STATUS: BLOCKED / IN_DESIGN — do not apply or grant client/service EXECUTE.
-- T5 domain slice #2 — draft design for manually-entered income/expense vouchers.
-- This artifact is parked behind unfinished T2 normalized authorization and T3
-- provenance-scoped approval containment. Its legacy-derived authorization/cashbook
-- bridge, missing canonical-draft marker, mutable-resource replay semantics, rollout
-- governance, and durable audit are not activation-ready. Nullable-account handling,
-- generic contract trigger sequencing, tenant-only authority, and initial idempotency
-- claim ordering are local design fixes only; they do not lift these blockers. There is
-- no frontend route, flag flip, approval wiring, direct-DML revoke, or legacy change.
-- Keep the feature OFF and the function revoked until a replacement exact-source
-- artifact passes the T2/T3/T4 gates and independent PostgreSQL evidence.
--
-- Contract:
--   * actor/profile/organization/status/amount/audit are server-owned;
--   * header + all items + audit + idempotency response commit atomically;
--   * only non-recurring, non-deposit UNAPPROVED drafts are created;
--   * every foreign resource is live and closes to the building organization;
--   * building-create and cashbook-use authorization are independent and both required;
--   * same key/same canonical payload replays; same key/different payload conflicts.
BEGIN;
...
-- Missing/OFF remains LEGACY and the endpoint rejects without effects. This row does
-- not activate the writer; a later grant/canary tranche must provide release identity
-- and positive count/VND caps before an authenticated caller can mutate through it.
INSERT INTO app_private.server_feature_flags (
  feature_key, domain, risk_class, mode,
  max_operation_count, max_single_amount_vnd, max_total_amount_vnd,
  reason
) VALUES (
  'income_expense.create_draft.v1', 'income_expenses', 'MONEY', 'OFF',
  0, 0, 0,
  'Blocked income/expense draft design; requires canonical T2/T3 replacement and fresh exact-source evidence'
)
ON CONFLICT (feature_key) DO NOTHING;

-- This bridge is private and fail-closed. It deliberately does not reuse the
-- legacy public helpers: every permission witness is correlated to the same
-- organization, role owner, active OWNER grantor, and exact JSON boolean leaf.
-- buildings.user_id and role names are audit/legacy metadata, never authority.
CREATE OR REPLACE FUNCTION app_private.authorize_income_expense_on_building(
  p_actor uuid,
...
REVOKE ALL ON FUNCTION app_private.authorize_income_expense_on_building(
  uuid, uuid, text, uuid
) FROM PUBLIC, anon, authenticated, service_role;

-- Replace the infrastructure's provisional evaluator in a forward migration,
-- rather than editing its already-reviewed historical file. ON is an explicit,
-- release-identified uncapped route. CANARY alone requires enrollment, a finite
-- half-open window, and positive caps; amount admission is checked by the writer.
...
CREATE OR REPLACE FUNCTION public.create_income_expense_v1(
  p_type text,
  p_name text,
  p_building_id uuid,
  p_room_id uuid,
  ...
  p_contract_id uuid,
  p_payer_name text,
  p_receive_bank_account text,
  p_receive_bank_name text,
  p_account_id uuid,
  p_attachments jsonb,
...
BEGIN
  ...
  SELECT COALESCE(NULLIF(btrim(p.full_name), ''), NULLIF(btrim(p.email), ''), u.email, 'Người dùng')
    INTO v_actor_name
    FROM auth.users u
  ...
  INSERT INTO public.income_expenses (
    ...
    'NONE', false, 0, false,
    0, NULL,
    p_voucher_date, 'UNAPPROVED', NULL, NULL,
    v_payload_hash, NULL
  )
...
COMMENT ON FUNCTION public.create_income_expense_v1(
  text, text, uuid, uuid, uuid, uuid, text, text, text, uuid, jsonb,
  boolean, text, date, jsonb, text
) IS
  'BLOCKED design artifact for manually-entered, non-recurring, non-deposit UNAPPROVED income/expense drafts. Local nullable-account, generic trigger sequencing, tenant-only authority, validation, and claim-before-admission fixes are not activation evidence. Canonical T2 resolver/cashbook possession, a T3 canonical-draft marker plus legacy-transition containment, immutable replay semantics, rollout governance, and durable audit remain prerequisites. Keep OFF, revoked, unrouted, and unapplied.';

REVOKE ALL ON FUNCTION public.create_income_expense_v1(
  text, text, uuid, uuid, uuid, uuid, text, text, text, uuid, jsonb,
  boolean, text, date, jsonb, text
) FROM PUBLIC, anon, authenticated, service_role;
