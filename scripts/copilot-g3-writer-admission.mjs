...
export function collectG3Journals(directory) {
  ...
  for (const entry of readdirSync(directory, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) journals.push(...collectG3Journals(join(directory, entry.name)));
      ...
      ensure(Object.keys(bundle).length === 9 && bundle.kind === 'g3-writer-receipts-v1' && HASH.test(bundle.admissionDigest) && Array.isArray(bundle.journals) && bundle.journals.length === 2, 'g3_artifact_receipts_schema');
...
export async function prepareG3Writer({ current, reviewedSourceSha, token, directory, reconciliation, reconciliationDigest, fetch }) {
  ...
  if (!reconciliation) {
    ...
    const zip = join(resolve(directory, '..'), 'g3-prior.zip'); writeFileSync(zip, Buffer.from(await archive.arrayBuffer()), { mode: 0o600 });
    try { receipts = JSON.parse(execFileSync('unzip', ['-p', zip, 'writer-receipts.json'], { encoding: 'utf8', maxBuffer: 1024 * 1024 })); } catch { throw new Error('g3_prior_artifact_invalid'); }
  }
...
async function cli() {
  ...
  if (process.argv[2] === 'prepare') {
    ...
    await prepareG3Writer({ current, reviewedSourceSha: env.COPILOT_G3_REVIEWED_SOURCE_SHA, token: env.GH_TOKEN, directory, reconciliation, reconciliationDigest: env.COPILOT_G3_RECONCILIATION_DIGEST, fetch });
