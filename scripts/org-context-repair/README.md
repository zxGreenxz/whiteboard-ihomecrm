...
- Finish runtime verification and exact-release UI tests. Clean CI on `1ffddb61` passed typecheck, lint, strict islands, build, Node regressions and 7,337 Vitest cases, but failed one upload-signature test, the error-swallow gate and two stale documentation counts. The follow-up corrects those issues and expands coverage; its CI result must be recorded separately. The source inventory completed on CI: 1,081 source files and 550 API/storage call sites (candidates for review, not 550 verified boundaries).
...
## Follow-up business boundary audit

- Thirteen previously unfilled business tables now have guarded insert/update triggers: area/building links; asset handovers, movements, maintenance and warehouses; auto-debt configuration; job groups/types; hotlines; lead activities; salary ledger snapshots; service quotas/tiers. Parent references must agree on one company; a root insert uses validated working-company context. Existing non-null company labels cannot be reassigned by these triggers. The generator verifies each parent foreign key and the exact baseline trigger set before installing anything.
- The existing strict resolver changes only the authenticated root `settings` case. Its other eleven table paths retain their current behavior. The salary resolver now prioritizes the persisted monthly parent and validates new interactive periods through the salary-context helper; unclassified background writes with multiple active salary companies fail instead of choosing the first configuration.
- Legacy salary lock preflight checks every persisted period and every commission voucher before its first write. It attaches company identity to monthly and snapshot rows and surfaces snapshot errors. This preserves the legacy path's existing multi-request transaction limitation; the canonical writer remains atomic.
- Five nullable-company tables remain account-level/shared compatibility surfaces: `ai_copilot_entitlements` (primary key/user-based entitlement lookup), `push_subscriptions` (device endpoint and user), `user_subscriptions` (user subscription), `invoice_generation_settings` (unique user settings), and `public_room_settings` (owner primary key). They must not be blindly relabelled to the selected company. The direct public-room settings hook writes one row per owner; its owner-only RLS and both desktop/mobile descriptions already identify these settings as shared across that account’s links. Copilot and worker consumers retain their existing organization filters; the selector does not split this legacy owner-wide setting into separate company rows. The Copilot entitlement UI now states that enabling/disabling the feature applies to the account across companies; data permissions remain company-specific.
- A disposable live-catalog verification at 15:56 UTC passed 44 patched definitions plus all thirteen business-table cases (SHA-256 `b33a589b2688a20bde9677c347bdd096b34156d27207f855ec769a7ea1fb0c4c`). The same business boundary regression passed with a deliberate removed-guard mutation detected. The final 45-function review then passed at 16:06 UTC (SHA-256 `523ac150bfb503c358cb27ab91e4c999ad22ee2108e956930ae8df83a5b27b95`), and the salary-trigger regression independently passed historical-parent, explicit-conflict and ambiguous-background cases.
- High-risk graph freshness on the relocated worktree currently fails: GitNexus is missing and UA is stale. No conclusion is based on that graph. Refresh and rerun the high-risk check before promotion; catalog/function/trigger proofs above are separate evidence, not a graph-pass claim.


## Selected-company member visibility

A final source review found that `current_admin_org_v1` used `has_any_scope_v3('users.view')`, whose witness may come from any active company. The selected-company resolver now calls a private clone limited to that company. The original global helper is untouched; emergency denies, scope kinds, member overrides, role timing, possession rules and the existing platform-admin bypass are retained. Apply also checks the live digest of the original permission helper, so a newer permission rule cannot silently be overwritten by an older cloned rule.

At 16:53 UTC the full disposable catalog verification passed all 45 patched definitions and 13 business boundaries, including permission in A not admitting member visibility in B, emergency deny preservation and platform-admin behavior (review SHA-256 `f00f5a29c4af3c8835024a316da325c91f1abffbbba7e9a2a56441bad5dce5cd`). Actual production-role checks remain required after a backed-up application.

Clean CI run `34503921512` on `4a4ed5bf28ac5dee5cd01358d562f54e8699f3ac` passed quality, strict islands, timezone, secret and knowledge/realtime jobs. Production-only jobs were skipped on this draft PR. The additional member-visibility change needs its own CI rerun. The latest local salary/context runtime run passed 15 tests, and removing the client batch-company guard produced the expected assertion failure before the source was restored.


## Release preparation at 2026-09-10 17:30 UTC

PR #61 head `e704a2c6b99e48b6879dbcd845a42f88c9f3ec57` passed all six code/test/source-impact jobs in CI run `34507138723`. The clean runner built GitNexus at the exact head and passed high-risk freshness; the older UA graph is still not used as architecture evidence. Source impact reports 597 upstream references for the shared request client, and change detection reports 99 files/296 symbols/50 flows. These source measurements do not establish deployed SQL/RLS or real-role behavior.

The backup prerequisite passed PR #62 CI run `34507388279` and is on main at `8a83ece946983197639e0e012f348405e7a03e27`. Its ten tests cover password byte preservation, pgpass escaping and invalid line rejection; an escaping mutation was detected. A new encrypted backup was dispatched at 17:28 UTC. Run `34508482559` then failed at 17:29 UTC with PostgreSQL password authentication rejected, even after the pgpass correction; no backup artifact was produced. Application production remains at the previous release; no schema or image repair has run.

The allocated forward file `20260910171935_repair_explicit_working_organization.sql` is prepared locally from the verified review, but is not included in the reviewed commit. Provenance generation stopped because local Supabase PAT access is unavailable; the existing Dashboard session currently times out. Current live preconditions, backup eligibility, provenance and actual-role/HTTP checks are still prerequisites for application.
