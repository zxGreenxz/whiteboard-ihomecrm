-- t5_24 — PHƯƠNG ÁN DUYỆT ĐẦY ĐỦ (phần thực thi) — đi kèm t5_23
-- 1) create_income_expense_v1: quyết định trạng thái sinh theo (a) cờ
--    force_approval trên hạng mục (hoàn cọc/thanh lý/lương/LN/HH/thưởng/HHMG/
--    Tiền thối...) và (b) ngưỡng phiếu CHI trong app_private.ie_auto_approve_config
--    (cài ở trang Cài đặt). Dưới ngưỡng + hạng mục thường → TỰ DUYỆT (như t5_21);
--    ngược lại → NHÁP chờ duyệt tay. Claim t5_22 đã chấp nhận cả 2 hình dạng.
-- 2) app_private.submit_financial_request_v1: v_is_force nhận thêm cờ
--    force_approval → engine không bao giờ AUTO_POST hạng mục đặc biệt.
-- Sinh từ pg_get_functiondef bản sống 2026-07-19 với thay đổi tối thiểu.
