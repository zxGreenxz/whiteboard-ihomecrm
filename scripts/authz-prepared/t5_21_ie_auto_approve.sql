-- t5_21 — Phiếu thu/chi TỰ ĐỘNG DUYỆT khi tạo (khôi phục phương án đã chốt)
-- BUG: create_income_expense_v1 hard-code 'UNAPPROVED' → mọi phiếu thành nháp
-- chờ duyệt tay — SAI phương án owner đã chốt: "tất cả phiếu thu chi tự động
-- duyệt, trừ hạng mục chi đặc biệt đã liệt kê" (bằng chứng legacy: cột
-- approval_status DEFAULT 'APPROVED' + comment recurring.ts "phiếu mặc định
-- APPROVED khi tạo"; luật engine P200 'Bắt buộc duyệt (HH/thưởng/hoàn/lương/LN)').
-- FIX: phiếu sinh ra APPROVED + approved_by=người tạo + approved_at=now().
-- Danh sách đặc biệt KHÔNG bị ảnh hưởng vì không đi qua writer này:
--   · cọc / hoa hồng / thưởng Sale → 0A000 (flow chuyên biệt/legacy)
--   · lương / lợi nhuận / chi quản lý → engine writers (PENDING_APPROVAL → duyệt)
--   · hoàn tác thu tiền → reverse_invoice_payment_v3 (thu_tien.undo)
-- Sinh từ pg_get_functiondef bản sống 2026-07-19, đúng 2 thay đổi:
--   1) INSERT: 'UNAPPROVED', NULL, NULL → 'APPROVED', v_actor, now()
--   2) audit event: status APPROVED + reason "TỰ DUYỆT"
...
BEGIN
  ...
  INSERT INTO public.income_expenses (
    ...
    p_voucher_date, 'APPROVED', v_actor, now(),
  ...
  PERFORM app_private.append_income_expense_event_v1(
    ...
    NULL, 'APPROVED', 'Tạo phiếu TỰ DUYỆT (phương án org: auto-duyệt trừ hạng mục đặc biệt) qua create_income_expense_v1');
