...
begin
  select organization_id into v_org from public.organization_memberships
   ...
   having count(*) filter (where member_type is not null) >= 2 limit 1;
  ...
  -- T10: grant possession + within limit → self-approve posts
  ...
  raise notice 'ALL 10 APPROVAL V2 TESTS PASSED';
end;
$tests$;
