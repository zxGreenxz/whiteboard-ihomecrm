...
| Đăng nhập | Tên tài khoản như production, **mật khẩu TEST riêng** (dòng `TEST_PASS <email> <mật khẩu>` trong vault). Tài khoản fixture `demo.*` giữ mật khẩu production để bộ E2E chạy được |
...
3. **Nạp** `auth.users`/`auth.identities`, dựng bucket, chép **dòng** `storage.objects` với đúng id
   (khoá ngoại `app_private.ie_supplement_objects` trỏ vào đó).
4. **Trung hoà default privileges** của `postgres` rồi `pg_restore` GIỮ ACL — Supabase mặc định cấp
   ...
   này thì mọi hàm production đã REVOKE khỏi anon sẽ mở lại trên TEST.
...
6. **Kiểm**: ~8.000 object (thân hàm, ACL bỏ grantor, policy, trigger, index, constraint, cột, kiểu,
   ...
   tuyệt đối. Lệch một chỗ ⇒ exit 1, kết quả ghi `kiem.json` trong thư mục làm việc.
7. **Hậu kỳ**: thay ref production trong thân hàm (chỉ `append_income_expense_supplement_v1` ghim
   host storage), xoá `push_subscriptions` (thiết bị thật), đặt mật khẩu TEST, dựng lại cron (trừ
...
và xoá khi xong. Không có kênh gửi ra ngoài nào nối vào TEST: worker Zalo và Network Center chỉ
